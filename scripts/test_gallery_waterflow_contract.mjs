...
assertIncludes(waterfallCard, 'private tagRowIndexes(remainder: number): number[]', 'Waterfall tag strip must split tags into two independent odd/even rows');
assertIncludes(waterfallCard, 'i % 2 === remainder', 'Waterfall tag strip must distribute tags by index parity instead of fixed columns');
assertIncludes(waterfallCard, 'private TagRow(remainder: number)', 'Waterfall tag strip must render each row independently');
assertIncludes(waterfallCard, 'this.TagRow(0)', 'Waterfall tag strip must render the first parity row');
assertIncludes(waterfallCard, 'this.TagRow(1)', 'Waterfall tag strip must render the second parity row');
assertIncludes(waterfallCard, 'Scroll(this.tagScroller)', 'Waterfall tag strip must use a native horizontal Scroll so each row can keep its own left-aligned width');
...
assertNotIncludes(waterfallCard, 'private tagColumnIndexes(): number[]', 'Waterfall tag strip must not regress to column-paired rows that force top/bottom tag alignment');
assertNotIncludes(waterfallCard, '.listDirection(Axis.Horizontal)', 'Waterfall tag strip must not use horizontal List columns for the two-row layout');
