...
ok('Index /s/ deep links pass parsed fileCount to Reader', /new ReaderParams\([\s\S]*target\.gid,[\s\S]*target\.token,[\s\S]*target\.index,[\s\S]*target\.fileCount/.test(indexSrc))
ok('Search /s/ jumps pass parsed fileCount to Reader', /new ReaderParams\([\s\S]*target\.gid,[\s\S]*target\.token,[\s\S]*target\.index,[\s\S]*target\.fileCount/.test(searchSrc))
