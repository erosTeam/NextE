...
// contract targets.
{
  ...
  ok('keeps a gallery showKey cache', /this\.cache\.getShowKey\(ref\.gid\)/.test(svc))
  ok('fast path calls showPage', /EhApiPhpService\.showPage\(/.test(svc))
  ok('learns showKey from /s/ resolve', /this\.cache\.putShowKey\(ref\.gid, r\.showKey\)/.test(svc))
