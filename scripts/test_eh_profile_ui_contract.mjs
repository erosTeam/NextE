...
// Options are parsed from the page (no hard-coded value→label list to go stale, e.g. resolutions).
ok('no hard-coded option-label lists remain', !/new UcOption\(5, '2400x'\)/.test(page) && !/uhOptions\(/.test(page))
...
ok('options come from the page (parsed), not a hard-coded list; oi gated by field presence', /pageOptions\(/.test(page) && /radioGroups\.get\(code\)/.test(page) && /hasField\('oi'\)/.test(page) && !/filterOptions\(/.test(page))
ok('page labels glossed to Chinese by exact text (resolutions etc. fall through)', /optionZh\(\)\.get\(label\)/.test(page) && /'Compact', 'ehp_dm_0'/.test(page))
