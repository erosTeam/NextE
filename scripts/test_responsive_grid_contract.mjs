 ...
 *   • the thumbnail/cover image radius is ThemeConstants.THUMB_RADIUS=8 so visible thumbnail/cover
 *     content does not regress to square-looking 4vp corners while ordinary RADIUS_SM can stay compact;
...
const THUMB_RADIUS = num(/THUMB_RADIUS:\s*number\s*=\s*(\d+)/)
eq(MIN, 90, 'preview thumbnail min width respects the current product token')
eq(THUMB_RADIUS, 8, 'thumbnail image radius respects the current product token')
