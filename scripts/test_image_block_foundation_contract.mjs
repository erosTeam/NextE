...
assert.match(repo, /MAX_FILE_HASH_CACHE_ROWS:\s*number\s*=\s*8192/, 'file pHash cache has a finite durable cap')
assert.match(repo, /SQL_PRUNE_FILE_HASH_CACHE/, 'file pHash cache is pruned after writes')
assert.match(repo, /pruneFileHashCacheIfNeeded\(store\)/, 'file pHash prune is batched on the write path')
...
assert.match(runtime, /ImageBlockThumbnailHashMemoryCache/, 'thumbnail hash memory cache stays bounded by a dedicated cache type')
assert.match(runtime, /MAX_THUMBNAIL_HASH_MEMORY_ENTRIES:\s*number\s*=\s*1024/, 'thumbnail hash cache has a finite process-memory cap')
