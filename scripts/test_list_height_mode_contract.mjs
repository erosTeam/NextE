 ...
 *   • FIXED → every list row is pinned to one portrait-cover height (min==max) with the tag block CLIPPED
 *     (layoutWeight + clip) so the meta stays foot-anchored — uniform rhythm, no runaway height;
 *   • ADAPTIVE → the row only floors at a cover height (minHeight, no maxHeight) and grows with content;
...
// 3. GalleryCard: reactive read + the two structural layouts + the no-crop invariant.
{
  ...
  // FIXED → pin the row to one cover height (min==max) AND clip the tag middle so meta is foot-anchored.
  ok(
    'FIXED pins the card to exactly cardMinHeight (min==max)',
    /fixedHeight\s*\?\s*\{\s*minHeight:\s*cardMinHeight\(\),\s*maxHeight:\s*cardMinHeight\(\)/.test(c),
  )
  ok(
  ...
  // ADAPTIVE → floor only (minHeight, NO maxHeight) so the row grows; tags float between two springs.
  ok(
    'ADAPTIVE floors at cardMinHeight with NO maxHeight (row grows)',
    /:\s*\{\s*minHeight:\s*cardMinHeight\(\)\s*\}/.test(c),
  )
  ok(
