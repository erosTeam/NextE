...
const moreBtn = block(preview, /moreButton\(initialImagePage: number\)\s*\{/)
ok(/this\.onMore\(initialImagePage\)/.test(moreBtn), 'moreButton navigates to all-thumbnails (grid + hidden entry)')
const header = block(preview, /sectionHeader\(entry: boolean\)\s*\{/)
ok(/entry/.test(header) && /this\.onMore\(0\)/.test(header), 'horizontal-mode sectionHeader(entry) navigates to all-thumbnails')
// hidden branch and grid branch both render moreButton; horizontal branch renders sectionHeader(true).
ok(/this\.hidden\)[\s\S]*?this\.moreButton\(0\)/.test(buildBody), 'hidden mode offers the more-previews entry')
ok(/this\.horizontal\)[\s\S]*?this\.sectionHeader\(true\)/.test(buildBody), 'horizontal mode offers the tappable header entry')
ok(/else \{[\s\S]*?this\.moreButton\(this\.lastImagePage\(\)\)/.test(buildBody), 'grid mode offers the more-previews entry (button below)')
...
ok(/if \(entry\)[\s\S]*?this\.onMore\(0\)/.test(header), 'tapping the entry header navigates to all-thumbnails (onMore)')
