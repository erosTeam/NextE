...
同一准入门适用于 `containerMask`。当前的首个端侧候选是只输出 speech-bubble instance mask 的轻量
YOLO11n：研究 NAPI 仅返回 page mask 与实例框，且明确禁止把实例框提升为容器边界。它尚未加入 model pack、
设置页或默认 Reader 路径；必须先在真实样本矩阵记录边界漏检/误检、mask 内外擦除、设备 P50/P95 与 PSS。
候选上游自报指标只能作为筛选线索，不能替代 NextE 的真实页验收。
