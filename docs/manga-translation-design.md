...
v19/v31：Core Vision 系统 OCR 与可选 YSGYolo/PP-OCRv5 输出先映射到原图 geometry，再合并相邻纵列或横行。
原文清理始终遵循源文字 geometry；译文排版方向由目标语种决定，中文和日文可保留纵排，拉丁字母目标必须
使用横排单词换行，不能把英文逐字竖排。渲染前可以抑制被同一大段落几何包含的重复 OCR 小块，并合并相邻
源纵列，但不得跨越明显分离的左右段落。文字采用白色描边、深色填充的两遍绘制；pen 宽度固定为字号的
20%，并限制在 2–10 px，填充覆盖描边内侧后留下约一层正常字笔画的可见白边，不能退回固定 1–2 px 细边。

v13 两页原创测量基线严格命中 9/11，成功检测的普通文本方向为 9/9，单页端侧视觉耗时为 867–1677 ms；
v31 在真实英文 Reader 页中已修复逐字竖排、重复小块和弱描边，但段落仍会压住人物、贴近页边，页脚也有
残留，因此当前结果仍未通过通用制图质量门。完整测量边界见
...
这两个模型仍不输出文字像素 mask 或背景修复。后续高质量档仍需完成长图 detector 分块、扩展漫画 OCR
评测和可选 inpaint。上游
