...
容器评审从候选布尔标签升级为目标框一对一匹配后，必须额外按上游 source layout 分层：
`eligible` 表示目标内存在独立 OCR/layout，`merged_source` 表示 OCR 块跨越目标，`missing_source`
表示没有可用文字块。2026-07-26 修正一处把整格画面边框误标为字幕框的真值后，公版四页为 17 个目标，
...
因为全部候选 precision 只有 46.1538%，且独立最高分边尚无同轮廓证据。剩余 2 个合并目标和 3 个缺失
目标必须进入 OCR 分割/召回工作项，禁止通过容器阈值扩张、接受画格边框或扩大擦除范围补数。
