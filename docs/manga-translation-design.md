...
`core-vision-ocr-bubble-group-v43`，语义分组和 Reader layout 分别继承 v24/v43：Core Vision 系统 OCR 与
...
拟声词不能只依赖 LLM 自行判断。清晰、可重复的假名拟声块标记为 `sfx`，提示模型结合整页和区域裁片
解释声音；若假名为主的短拟声转录同时混入孤立拉丁字母或数字，视为 OCR 污染，render plan 必须保留
原画而不采用译文。短混合脚本乱码、纯符号目标文本及模型原样复制原文同样失败关闭。该规则只针对有明确
污染证据的块，不得把所有拟声词或假名对白统一关闭。
