...
- **状态**：持续维护；Phase 0 已闭环，V1 Reader 当前页路径、重启恢复与跨页同源译法审计已实现
- **首次整理**：2026-07-20
- **最近复核**：2026-07-21
...
译法冲突、同页重复和长文本均排除。memory 最多 64 项，有独立 1,800 字符 prompt 预算，完整前页上下文仍只
发送最近两页。提示词明确要求仅在当前规范化原文完全相等时采用，并让 glossary（尤其 locked 项）保持更高
优先级。该 memory 每次从可再生成页面缓存重建，不取得 glossary、人工修订、备份或同步语义；只有实际进入
prompt 的条目参与上下文指纹。
...
生成文档缓存现已在“存储”的既有缓存卡片中单独统计和清理，并纳入“清除全部缓存”；该入口仍只删除可再生成
文档和进程内编排器，不取得未来人工译文、术语、备份或同步语义。清理会先失效更早编排器的缓存写入，并将
持久写入与清理串行化，避免飞行中旧请求在清理完成后重新填充缓存。生产编排器现在会把最近前页的累积摘要补入
下一页请求；该摘要契约最初由 prompt v2 引入，当前 v3 继续保留，显式调用方摘要仍优先。prompt v2
的一次真实非缓存调用返回六个标准化块并显示 `已完成`。提示词现已对摘要、风格、术语和最近两页上下文分别设定预算；
...
缓存中的 project/profile/block/speaker/signal/error-code 标识也必须保持无首尾空白的规范形式；原文、译文、
摘要、样式提示和诊断消息仍按自由文本原样恢复，不因身份校验被 trim 或重写。
公开的 cache/scope key 与内存、RDB repository 入口会重新断言完整请求身份，因此即使未来调用方绕过
`ComicTranslationRequestIdentity.from` 手工构造对象，无效身份也不能在编码、内存写入或数据库访问后才失败。
编排器现会额外读取最多最近 32 个同 scope 生成页做纯本地一致性审计，但只把最近两页装入模型 prompt。
同一规范化原文出现不同译文时，Reader 结果增加待复核 warning；原 provider 文档先原样写入生成缓存，缓存命中
时再重算 warning，因此更早页面变化不会污染请求身份或留下过期派生信号。当前审计不会自动纠正译文，也不把
相似写法、别名或人物指代误当成同一术语。Reader 会在现有原文/译文卡片内标出具体冲突块，固定提示不包含
历史正文。Reader 的通用复核策略只把 `WARNING`/`ERROR` 质量信号和页面 `REVIEW_REQUIRED` 状态视为待复核；
`INFO` 不会单独改变页面状态。带 `blockId` 的 warning/error 会在对应文本块内显示固定本地化提示，页级信号只
改变页面状态；`REVIEW_REQUIRED` 页面中缺少译文的块也会标记。跨页一致性提示优先于通用提示，界面不会显示
provider 给出的 signal code/message，复核标记也不进入持久化或缓存身份。专用跨页一致性 code 同样必须是
`WARNING`/`ERROR` 才能成为冲突标记；同名 `INFO` 只保留为元数据，也不能阻止本地审计追加真实 warning。
稳定的 exact source/translation pair 还会作为临时 translation memory 回流后续请求，从而在完整两页上下文
之外提供有界预防信号；它不自动成为 glossary。该 prompt 语义使用 `comic-page-responses-v3`，通过既有
prompt-version 缓存身份与 v2 生成文档隔离。
Reader 的成功/待复核结果现提供显式“重新翻译”：强制刷新期间继续显示上次成功文档并禁用重复触发，只有新
结果通过既有 route/page/image publication fence 后才替换；刷新失败则显示固定错误码和重试，同时保留旧文档。
该动作仍是单页用户触发，不引入自动预取、批量调用或新的设置层级。
