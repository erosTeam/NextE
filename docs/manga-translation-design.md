...
- **状态**：视觉 Reader V1 首条分阶段路线已通过真实设备验收；多页形态与质量评估继续扩展
- **首次整理**：2026-07-20
- **最近复核**：2026-07-21
...
- [ComicTranslationSettingsPage.ets](../feature/settings/src/main/ets/pages/ComicTranslationSettingsPage.ets)
  只选择共享 LLM 源和漫画使用的模型；API 与 Codex OAuth 源统一由 LLM 源管理维护，同一 Codex 源也可供
  评论翻译选择。制图 sidecar 仍是独立能力和凭据边界，不混入 LLM 源列表。
...
跨语言覆盖还必须同时记录“能出图”和“译文自然度”，不能用前者替代后者。长图尚无真实高纵横比样本，
继续保持独立资源与质量验收项。
