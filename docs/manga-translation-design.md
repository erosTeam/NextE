...
v20/v36：Core Vision 系统 OCR 与可选 YSGYolo/PP-OCRv5 输出先映射到原图 geometry，再合并相邻纵列或横行。
...
段落信息；v36 又修复宽源段落被 360 px 上限反向缩窄的问题，同时禁止新增宽度抬高字号。但当前仍没有
内容感知障碍避让，页脚也有残留，因此当前结果仍未通过通用制图质量门。完整
测量边界见
