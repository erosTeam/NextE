...
v24/v39：Core Vision 系统 OCR 与可选 YSGYolo/PP-OCRv5 输出先映射到原图 geometry，再合并相邻纵列或横行。
...
文档 block 一对一生成 plan，不再根据译文或矩形关系重新合并、抑制语义块。
...
局部边缘感知纵向锚点；v24 analyzer 继承页脚过滤并修复多行段落跨气泡传递合并，v39 render 禁止翻译后
再次改变语义分组，并对可确认闭合的普通气泡使用内部安全矩形。但当前没有语义障碍/人物分割，艺术字
...
译图重复超分，不关闭普通页/长图 P50/P95、低内存失败回退或视觉质量门。随后 v24/v39 在同一第 2 页把
...
Hypium 全部通过。该单页结果关闭已知传递合并和闭合普通气泡贴边回归，不等于开放式气泡、复杂背景或艺术字
质量门通过。
