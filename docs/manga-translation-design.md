...
同一画廊第 5 页的扩展验收进一步把质量边界分开：sidecar 只导出了七个区域，其中一个小气泡
`むぅ～…` 完全没有 block，因此现有渲染链无法替换；另一个已定位区域把
...
在裁图前后复核 hash，避免派生输入脱离页面身份。这个修正仍只能覆盖“已有区域、原文识别错”，不能覆盖
“整块漏检”；在残留源文字或检测覆盖检查落地前，不能仅凭所有已返回 block 都有译文就宣称整页文本完整。
