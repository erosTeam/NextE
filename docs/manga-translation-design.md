...
v24/v38：Core Vision 系统 OCR 与可选 YSGYolo/PP-OCRv5 输出先映射到原图 geometry，再合并相邻纵列或横行。
...
局部边缘感知纵向锚点；v24 analyzer 继承页脚过滤并修复多行段落跨气泡传递合并，v38 render 禁止翻译后
再次改变语义分组。但当前没有语义障碍/人物分割，艺术字
...
译图重复超分，不关闭普通页/长图 P50/P95、低内存失败回退或视觉质量门。随后 v24/v38 在同一第 2 页把
14 个 OCR 行稳定分为 5 个文档块和 5 个渲染 plan；中间两个长气泡分别为 `y=648–819`、`y=869–988`，
保留 50 px 间隔，真实译图不再跨气泡重叠。该单页结果关闭已知传递合并回归，不等于通用气泡轮廓、内边距、
复杂背景或艺术字质量门通过。
