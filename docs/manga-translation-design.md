...
- 未显式开启 Reader 自动翻译时，因普通图片预取或页面出现调用付费模型；
- 一次请求上传整本画廊图片；
...
- `ReaderCacheWarmers()` 和多个图片组件也会调用 `onImageFileReady`。文件就绪只表示图片可用；只有用户
  显式开启本次 Reader 会话的自动翻译后，调度器才可以把它作为当前页或单页前瞻的就绪条件，普通图片
  预取本身不得触发模型请求。
...
- [ComicTranslationRuntimeService.ets](../shared/src/main/ets/services/ComicTranslationRuntimeService.ets) 已把
  ...
  结果发布；成功产物是经过校验的本地衍生图片，重新进入现有 Reader 图片路径。旧文字半模态已退出主结果
  路径，失败时继续显示原图。
...
`core-vision-ocr-bubble-group-v50`，语义分组和 Reader layout 分别继承 v24/v44：Core Vision 系统 OCR 与
...
日文可保留纵排，拉丁字母目标必须使用横排单词换行，不能把英文逐字竖排。v40 不再按背景明暗推导或反转
译文字色：有 CTD 文字 mask 时，从原字形像素中提取主色，高饱和彩色字优先保留占主导的色簇，黑白字取
字形暗色主体；缺少可靠样本时固定回退深色。文字仍以白色描边、源字色填充两遍绘制，pen 宽度为字号的
...
译文按 reading order 合并后在父容器内统一拟合和绘制。若统一译文无法容纳，先保留整组原画；允许在任何
擦除前进行一次有明确长度目标的纯文本精炼重试，但不得用漏画某个子块、缩细描边或越界排版换取“成功”。
2026-07-26 的真实设备 A/B 已否决“拆成语义段后也按子段独立修复/排版”：虽然纹理塌缩下降，但出现字号
失控、同容器重叠，并有一个长译文组因 fit 失败保留原文。该候选不进入 Reader。
排版的不可断词检查只能测量实际连续拉丁/数字 token，不能因为中文句子中嵌有 `34,000`、型号或英文缩写，
就把整段无空格中文视为一个拉丁单词。跨 OCR block 的断词续接必须失败关闭：只有父区域内候选唯一、上一
语义段以字母连字符结尾且空间邻接一致时才可作为开发候选；歧义、无连字符或跨父区域不得自动拼接。
2026-07-26 的 recording A/B 已证明父区域统一渲染和唯一续接能消除已知重叠，但新增近白尚未通过视觉门，
两项分组能力仍不得进入 Reader 生产分析。

拟声词不能只依赖 LLM 自行判断。确定分类为 `sfx` 的块必须在翻译协议中复制原文，并在 render plan
保留原画，不把 provider 返回的中文拟声重新绘制到艺术字上。短、假名占主导且混入拉丁字母、数字或其他
脚本残片的转录可以作为受污染拟声失败关闭；常见敬称、礼貌句式和动词标记则阻止普通对白被误归为
`sfx`。对仍属于对白、但同时夹有重复喘息或呻吟的块，中文目标在 provider 返回后确定性删除非语义声音，
保留同块中的对白；过滤后为空时回退原画。短混合脚本乱码、纯符号目标文本及模型原样复制原文同样失败
关闭。该规则不把所有短假名或含重复字符的对白统一关闭，也不把拟声语义判断重新交给渲染阶段。
...
v42 则在文档仍保留多个 block 时，只用同一已确认闭合边界把同气泡译文统一排版一次；source treatment
v29 复用同一视觉组，把相邻多列的原文字形合成一次 AOT 修复调用。
但当前没有语义障碍/人物分割，艺术字
...
端侧 source treatment 当前为 v29。AOT 不再按任意大文字区域原尺寸推理：长边超过 256 时保持比例缩小，
...
v29 只合并已经由同一闭合气泡边界确认的相邻 block：AOT 输入使用各 block 外围 crop 的并集，但 mask、
膨胀和 fallback 填色仍限制在各自 treatment 区域的并集，不会把两列之间的整块画面当作待清理内容；每个
block 仍必须分别命中 CTD mask。若合并 crop 的最长边超过最大单块 crop 的 1.35 倍，则保守回退为逐块
修复，避免为减少调用次数而显著降低 256px AOT 的有效分辨率。独立气泡始终保持独立调用。
真实七页对照中，跨独立气泡的同等 1.35 倍合并只把 render 均值降低约 3.1%，不足以抵消输入域变化，
因此已作为否决候选记录；后续性能优化不得重新放宽该边界。
...
同一问题的 source treatment 在 v29 进一步消除重复 AOT：确定性用例证明同气泡两列由 2 次降为 1 次，
而两个相邻但独立的气泡仍为 2 次。设备 `237` 使用发布版 AOT 权重的真实 NAPI 用例得到
`blocks=2 region=239x454 inference=136x256 load=114ms inference=208ms`，且全部变化像素仍位于原闭合
气泡 `x=430–590, y=320–740` 内。该结果只关闭已确认同气泡多列的重复修复，不把跨气泡 AOT 批处理或
连续多页性能视为完成。

原生连续压力基准固定为同一测试进程先 AOT、后 CTD，最多 20 轮，并同时记录 P50/P95、逐轮 PSS 与系统
热档位。设备 `237` 的最终候选中，AOT 为 `261/428 ms`，CTD 为 `643/972 ms`；进入 CTD 后热档位保持 2。
CTD 的双全页 mask 必须由 VM 管理的 ArrayBuffer 持有：HarmonyOS 不导出
`napi_adjust_external_memory`，external ArrayBuffer 对照在 20 轮内从 514,482 kB 持续升到 582,502 kB，
而 VM 管理版本从 540,646 kB 到 561,721 kB 峰值后两次回收，最终 454,164 kB。该变化不改变任何 mask
字节、模型 profile 或缓存身份，只让 VM 能看到输出内存压力。原生压力通过不等于真实 Reader 连续页通过；
解码、系统 OCR、LLM、PNG 编码、衍生页缓存和长图仍需按完整流水线单独测量。
...
v33 继承 v30 引入的第三个可选原生组件 CTD text mask。运行时擦除 mask 继续使用 0.3 阈值；0.15 低阈值输出只提出
...
未被 Core Vision 覆盖的 YSGYolo detector 区域继续按通用 16 区域上限补充识别；CTD proposal 另设独立
上限，只尝试按面积排序最强的 2 个候选，且只有最强候选允许半块和系统 OCR 重试。该预算不得反向截断
detector 区域，也不得因一页 CTD 画面噪声把后续十余个候选逐一送进识别器。
低置信度结果、页边短标签、已被系统 OCR 覆盖的碎片，以及短小的拉丁/数字与单个假名混合转录全部拒绝；
只有获得可接受 transcript 的候选才能进入语义分组和文档。该边界优先避免误擦画面，不把未识别拟声词
伪装成已完成翻译。

v39 进一步把检测类型与最终分组后的文字证据绑定到渲染许可。YSGYolo 明确定义为倾斜文字的 `shuqing` /
`hengxie` 区域，在当前仅支持轴对齐排版时保留原稿；CTD 提出的区域若占页面至少 2%，而最终合并文本只有
不超过 4 个非空白字符，也视为“低文字密度的大型艺术字候选”并保留原稿。该判定发生在语义分组之后，
保留标记随纵横合并和包含块折叠传播，渲染计划会跳过相应 block。普通 `qipao`、完整对白以及小面积标点
不受影响。这里的目标是失败时不再产生大面积错误消字或白洞，不是宣称艺术字已经翻译；在引入可靠的旋转
几何、艺术字识别与相应排版前，这些区域应明确保留源文字。

2026-07-27 的冻结设备审计明确了当前模型栈的停止边界：5 张真实页产生 28 个最终 layout，但只有 6 个
（21.4%）获得容器候选；把“无容器候选”直接失败关闭会丢掉约四分之三、包括正常竖排对白的替换，而继续
让 OCR 矩形进入 AOT 又会在彩色开放背景留下白块/纹理破坏。因此在 `YSGYolo + PP-OCRv5 + CTD + AOT 256`
下，不存在一个已证实可上线的容器准入阈值。该链路只能作为轻量阅读辅助：遇到艺术字、复杂纹理或开放背景
必须优先保留原图；不得再通过 glyph 膨胀、矩形扩张、跨气泡合并或 LaMa 回归去“调出”通用品质。要提高该类
页面的成品率，必须先资格验证一个新的整图成图模型或新的移动端修复/分割模型，并重新通过真实集质量、性能、
内存和许可证门。
...
**资格复核（2026-07-27）**：设备 `237` 在用户指定的真实高密度彩页 P21 上先后实测了
`gemini-3.1-flash-lite` 与运行时目录中更高档的 `gemini-3.5-flash`。两次均返回中文，但均有明显日文残留
与中文叠字、竖排溢出；后者不能通过开发页，因而不进入两张 holdout，也不再继续穷举 Torii 模型、字体或
prompt。这个结果只否决“当前 Torii 配置已能覆盖所有复杂彩页”的结论，**不否决 Torii 相对端侧路线的优势**：
它是当前质量更高的整图路径，端侧仍只是离线/低成本的原图保留回退。是否作为默认路线属于独立产品决策；在此之前
继续由用户显式选择。选择确认文案必须准确说明只会向 Torii 发送当前页与上下文，且不会转交已移除的 BYOK/共享
LLM 凭据。
...
的一部分，切换字体必须使缓存失效。字体菜单和运行时请求必须按目标语言取 Torii 明确声明支持的交集：
简体中文只提供 NotoSans、MaShanZheng，英文提供 NotoSans、WildWords、MaShanZheng；繁体中文、日文及
其他未核验语言暂时只提供 NotoSans。历史设置若留下不兼容字体，运行时必须回退 NotoSans，不能继续把
不受支持的字体发给 Torii。该字体选项只属于 Torii 云端整图。端侧路线固定使用系统默认字体，不下载、
不内置，也不承诺复刻 Torii 字形；两条路线的字体能力不得混为同一设置。
...
进入云端请求与缓存身份”的疑问，但该英文 A/B 不构成 WildWords 支持中文的证据。Torii 当前公开字体
元数据没有把中文列入 WildWords 支持语言，而 MaShanZheng 明确包含简体中文；不得因字体名称或肉眼观感
绕过语言兼容目录，也不据此关闭 Torii 的排版、语义或拟声词质量验收。
...
区域端侧路线在调用文本 translator 前，必须由实际负责回填的后端判断页面是否至少存在一个可安全替换的
候选。零文字页，或所有 block 均为 `sfx` / 已标记保留原稿的页面，以 `no_translatable_text` 停止，
不得为了最终得到 0 个 render plan 仍消耗一次 LLM 请求。混合页只要存在一个可替换正文块就继续，并把
完整页面文档保留给上下文；该门禁不能删除保留块、伪造无文字结果或改变整图 provider 的独立产物路线。
...
“自动翻译”是 Reader 顶部更多菜单中的显式会话开关：默认关闭，只在当前 Reader/画廊会话生效，退出
Reader 或切换画廊后自动关闭，不写入全局设置。开启前提示当前路线可能产生的额度和图片上传影响；运行时
只允许一个翻译任务，当前页优先；当前页完成或明确失败后，可以串行准备紧邻的下一页。前瞻任务完成后
不得继续滚动到下下页，只有用户实际翻页后才再向前补一页，因此不能把整章或 Reader 图片预取队列隐式
转成翻译队列。快速跳页时优先追随最新当前页，缓存命中继续优先；单页失败只阻止该页在本次会话内自动
重试，手动“翻译当前页”可重试，后续页面不受影响。
...
端侧容器候选必须以跨画廊人工标签校准 precision、recall、边界泄漏和置信度可分性。候选相对绘制
layout 的比例只能说明观测覆盖，不能代替 recall；覆盖率、连通区紧致度、面积占比和亮度波动等诊断
特征在 Core/holdout 关闭前只能进入开发 recording，不得直接成为生产接受阈值。低置信度、开放边界、
文字贴图或无法容纳粗描边译文时，Reader 继续保留原画或由用户主动选择云端整图路线。

容器评测必须把“候选是否正确”和“目标容器是否漏检”分开标注。高置信度分层即使在已复核样本上达到
100% precision，也不能在 recall 尚有浅色/灰白闭合框漏检或没有独立画廊 holdout 时接管生产修复。
轴对齐矩形候选还必须同时通过独立正向和硬负向留出；四边最低覆盖率只能用于 recording 采样。2026-07-26
的独立老扫描正向留出表明，将四边门从 0.9 降到 0.5 只新增 1 真 1 假，整体 recall 仍为 55.5556%，
而增加四角硬门又会把正向候选全部拒绝，因此 Reader 不得执行该分支。后续只能评估同一连通轮廓上的
短断边或成对平行线段，并继续使用同一双留出门。

容器评审从候选布尔标签升级为目标框一对一匹配后，必须同时读取 render source layout 与分析文档块。
`eligible` 表示目标内存在独立可渲染 source；其余状态必须区分渲染源跨目标合并、安全策略保留、仅文档
有源和文档级真正缺失。2026-07-26 修正一处把整格画面边框误标为字幕框的真值后，公版四页为 17 个目标，
其中只有 12 个属于容器阶段可处理范围。recording-only 两阶段边框搜索把 eligible recall 从
10/12 提高到 12/12；七页、43 个 layout 的彩页硬负向中矩形分支新增接受为 0。它仍不得进入 Reader，
因为全部候选 precision 只有 46.1538%，且独立最高分边尚无同轮廓证据。剩余五个目标实际为 2 个渲染源
跨区合并、1 个安全保留的合并文档块和 2 个在系统 OCR、YSGYolo 与 CTD proposal 中都不存在的真正
...
横排分隔探针必须同时记录视觉证据和适用范围。首个真实公版样本中，正间距、最终合并宽度覆盖和连续
深线/边缘联合门把 189 条边界收敛到 4 条 P12 容器分隔；既有七页彩页硬负向因没有多 detector 横排
合并而得到 0 条探针，不能被宣称为横排 false-positive 门。进入 Reader 前必须新增含正常英文/横排多行
对白的负向页，并证明拆分后文档边界、译文阅读顺序和渲染均无回退。
...
2026-07-23 的完整视觉基准进一步覆盖 AOT、排版、PNG 编码和衍生页写盘：两张 1024×1536 页各 3 轮时，
分析 P50/P95 为 4.253/6.673 秒，渲染为 1.947/2.347 秒。受控 1024×3072 拼接长图在轻负载下为
17.793/4.643 秒，在前置持续压力后的热状态下为 40.380/43.752 秒；它只证明长图完整链路可运行，不代表
真实 webtoon 已验收。2 线程对照显著拖慢普通页，生产保持 4 线程；Reader 当前只允许单个串行翻译任务，
任务目标只能是当前页或紧邻的下一页，不增加自动多页并发。阶段诊断只记录耗时、数量和字节，不记录图片、
文本或凭据。
...
同一准入门适用于 `containerMask`。首个只输出 speech-bubble instance mask 的轻量 YOLO11n 已在真实彩页
P20 被否决为独立容器方案：它只命中两个传统封闭气泡，漏掉页面主体的细长矩形对白框。研究 NAPI 只返回
page mask 与实例框，且明确禁止把实例框提升为容器边界；它不得加入 model pack、设置页或默认 Reader 路径。
只有后续多形状真实样本矩阵证明边界漏检/误检、mask 内外擦除、设备 P50/P95 与 PSS 后，才可作为多信号
候选重新比较。候选上游自报指标只能作为筛选线索，不能替代 NextE 的真实页验收。

同日对 joint text/bubble detector 的离线复核进一步固定了边界：彩页中的细长矩形文字区可能在语义上是
`text_free`，而不是 speech bubble。只输出该类 box 的模型可以补充未来的 source-text recall 比较，却不能
替代 `containerMask`，更不能把 rectangle box 直接变成 AOT 擦除范围或译文排版空间。模型卡许可证与下载
资产元数据冲突时，按资产侧更严格的许可处理并淘汰；不得因模型卡的宽松标签绕过资源准入。
