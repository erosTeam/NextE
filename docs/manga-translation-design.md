...
v21/v37：Core Vision 系统 OCR 与可选 YSGYolo/PP-OCRv5 输出先映射到原图 geometry，再合并相邻纵列或横行。
同一 OCR tile 内，若窄纵列被明显更大的纵排段落包含，分析阶段会把子列中尚未出现的原文并入段落并停止发布
子块，让 LLM 只翻译一次完整段落；渲染阶段的译文 owner 判重只保留为旧文档兼容保护，不再承担主合并职责。
...
局部边缘感知纵向锚点；v21 analyzer 排除窄边界内的页脚署名误识别。但当前没有语义障碍/人物分割，艺术字
...
译图重复超分，不关闭普通页/长图 P50/P95、低内存失败回退或视觉质量门；第 2 页多气泡被过度合并并重叠的
现象仍须在分析/布局边界修正。
