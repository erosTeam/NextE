...
`core-vision-ocr-bubble-group-v29`，语义分组和 Reader layout 分别继承 v24/v43：Core Vision 系统 OCR 与
...
v29 增加第三个可选原生组件 CTD text mask。运行时擦除 mask 继续使用 0.3 阈值；独立的 0.15 低阈值实例
只提出 Core Vision 与 YSGYolo 未覆盖的候选区域，不能直接触发擦除。候选按 4 px 网格、有限膨胀和连通域
生成，coverage 按实际网格并集计算；每个候选先走 PP-OCRv5 整块识别，近方形区域可做上下半块重试，仍未
识别且尺寸允许时再放大后调用系统 OCR。低置信度结果、页边短标签和已被系统 OCR 覆盖的碎片全部拒绝；只有
获得可接受 transcript 的候选才能进入语义分组和文档。
