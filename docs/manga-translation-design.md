...
只允许一个翻译任务，快速翻页追随最新当前页，不把整章或 Reader 图片预取队列隐式转成翻译队列。缓存命中
继续优先；单页失败只阻止该页在本次会话内自动重试，手动“翻译当前页”可重试，后续页面不受影响。
...
真实 webtoon 已验收。2 线程对照显著拖慢普通页，生产保持 4 线程；Reader 当前只允许单个当前页翻译任务，
不增加自动多页并发。阶段诊断只记录耗时、数量和字节，不记录图片、文本或凭据。
