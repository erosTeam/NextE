...
- **禁止为 HDS 标题栏自制或替换任何菜单。** `HdsNavigation` / `HdsNavDestination` 标题栏的菜单、
  溢出菜单和“更多”入口必须继续由 HDS 原生标题栏持有；不得用隐藏锚点、代理节点、`bindMenu`、
  自绘弹层、自定义“更多”按钮或应用侧菜单复制其菜单项，也不得为了规避平台缺陷改变直接项与
  溢出项的分配。遇到 HDS 菜单透明、材质异常或 API 行为变化时，只能先调查官方 API、SDK、系统
  版本和设备证据；在用户对替代交互给出明确授权之前，保持原生 HDS 菜单实现不变。
...
- 滚动末尾为系统安全区、悬浮控件或视觉间距保留的空间属于滚动内容。更换滚动所有者时，不得把
  末尾 `Blank`、spacer 或 content inset 机械改成父容器固定 padding，从而永久缩小可视区域。
  系统半模态已经处理的键盘或安全区也不得在内层重复占用；局部 inset 必须对应当前容器仍然存在的
  实际遮挡，而不是直接复用全窗口 avoid-area 数值。
