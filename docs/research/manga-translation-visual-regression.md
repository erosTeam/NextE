...
当前执行顺序和停止条件以[端侧漫画翻译质量收口](../plans/completed/manga-translation-local-quality-closure.md)
...
来源追踪确认 P6 的大多数长横线都来自源 OCR 中的 `---`，是正常对白停顿，不能全局删除。P9 则有一个
可证明的反例：`---AND G000' MMMMMM--- SZZZIP!` 的四个 Latin token 中三个是数字混淆或连续重复的
拟声/污染 token；模型只是将这个坏输入扩写成新的破折号行。端侧新增极窄的源 SFX 门：仅当短 Latin
transcript 至少有三个污染/拟声 token、它们覆盖至少三分之二字符，且至少一个是连续重复字母时，保留
原图文字。单个错字、单个拉长语气、正常破折号和普通数字对白都不触发。

新文档改变了 block kind，旧冻结响应的 context fingerprint 因此被安全拒绝；P9 仅重新执行一次 provider
请求（9.919 秒），之后导入该响应回放。设备 `237` 的最终页从 10 个 layout 变为 9 个，坏块不再绘制
中文破折号，保留原始 `AND GOOD! MMMMMM SZZZIP!` artwork；其余九个对白保持翻译。相对上一轮文字
样式结果，changed/new-white/flatness 为 3.0189%/0.9234%/0.4298%，其中新增近白再降 0.2258 个百分点。
...
同一真实彩页家族进一步抽取首/中/末三页进行 live provider 回放。三页各自通过 export、主应用翻译和
import render；provider 耗时为 11.518/11.746/12.650 秒，local analysis 为 2.448/2.778/2.644 秒，
render 为 4.053/3.937/3.463 秒。三页共 27 个 OCR 翻译块，最终合并成 19 个可绘制组；全部 19 组均各
调用一次 AOT 修复，因而修复仍是本地渲染主耗时。

更关键的是，19 个真实组中只有 2 个取得了可用的闭合容器候选（首页 1、中页 0、末页 1）。视觉对照也
确认，当前结果仍会把 OCR 矩形当作源处理和竖排可用区，导致复杂气泡的留白、边界和颜色层次无法可靠
保留。这个比例否决了直接把 recording 容器候选接管生产的想法：下一步必须在跨页人工标签上提高闭合
容器召回，并按独立的每组文字颜色记录/回放验证；不是继续压缩字号，也不是用矩形扩张遮盖问题。
