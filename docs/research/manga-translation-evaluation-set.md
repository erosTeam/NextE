...
existing franchises, third-party pages, logos and watermarks. The source PNGs and machine-readable references live
only under `entry/src/ohosTest/resources/rawfile/`; production code does not bundle or automatically upload them.
...
Both PNGs are 1024 × 1536. The manifest is the reference truth; generated artwork must not be silently edited without
updating its hash and re-reviewing every visible text block.
...
The next trigger must therefore run in the production module after an explicit user confirmation, or use a separate
test-module login. The production option should process page 1 first, pass its saved source/translation/summary plus the
locked `優 -> 优` term into page 2, delete temporary cache files, and log only provider/model, image identity, bytes,
latency and aggregate structural/accuracy counts. Adding that action changes the settings information hierarchy and
fixture packaging, so it requires a deliberate product choice rather than a hidden test hook.
