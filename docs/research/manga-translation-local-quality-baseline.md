...
- **current production render profile**: `reader-local-bubble-layout-v32` /
  `local-ctd-aot-inpaint-v24` / `local-bubble-typography-v27`
- **measured**: 2026-07-21
...
视觉结论仍是 **未达到生产可用**：左右长段落虽已分开且不再逐字竖排，但恢复的短句在原拟声词附近仍然
拥挤，页脚 OCR 也有残留。v30 完成时还触发过 `THREAD_BLOCK_6S`，当时进程 RSS 约 1.95 GiB，主线程
