...
[端侧漫画翻译质量收口](../plans/completed/manga-translation-local-quality-closure.md)为准。文中按日期记录的
“下一步”不自动成为当前任务。
...
## 真实画廊艺术字失败隔离与普通页回归（2026-07-24）

设备 `237` 使用同一 35 页日文黑白画廊验证，不把页面、原文、译文、URL、哈希或裁片提交到仓库。问题页
同时包含普通纵排对白、斜置手写字和大面积艺术字。旧候选会把一个占页面 10.4% 的 CTD 区域当作普通横排
块：局部系统 OCR 只有 2 个字符，语义合并后为 4 个字符，最终仍进入 AOT，造成大面积错误消字和小号译文。
这证明只看识别器局部返回值或 detector 类别不足以决定是否允许擦除，必须使用最终分组后的证据。

生产 backend v39 现在对两类当前无法正确回填的区域失败关闭：

- YSGYolo `shuqing` / `hengxie` 倾斜区域保留原稿；
- CTD 区域占页面至少 2%，且最终非空白字符不超过 4 个时保留原稿。

新鲜 cache miss 日志确认问题页 11 个最终 block 中，4 个区域保留原稿，7 个普通区域继续翻译和渲染；
占 10.4% 的 4 字符区域从 `keep0` 变为 `keep1`。最终截图不再出现原先的大白块，斜置/手写字保持源样，
普通对白仍为中文。该页阶段耗时为：分析 3.907 秒、LLM 10.792 秒、渲染 7.225 秒、持久化 0.103 秒，
总计 22.029 秒。渲染的主要本地成本仍是 7 个 AOT 区域串行 inpaint（6.586 秒）。

同一画廊的常规对白页随后重新分析。9 个最终 block 全部 `keep0`、全部进入 render plan，没有被新规则
误保留；最终截图的 9 个气泡均完成中文回填。该页分析 2.517 秒、LLM 9.748 秒、渲染 8.556 秒、
持久化 0.044 秒，总计 20.866 秒。两页共同证明本次修复是“不能可靠制图的区域保留原稿”，不是整页关闭
翻译，也不是以单张问题页阈值替代通用质量评测。

该阶段关闭当前真实反例中的破坏性大块擦除，并建立问题页/普通页成对回归。它没有解决艺术字翻译、旋转
排版、普通页翻译语义质量或 AOT 串行性能；这些仍属于 B2/F 开放项。

## 彩色字与受污染拟声转录（2026-07-24）

用户授权的新真实彩色画廊用于验证端侧译文字色、描边与拟声处理，不提交页面、文字、URL、画廊标识或哈希。
旧 v39 只有背景颜色采样，译文字体固定深灰填充与 20% 字号的白色 pen。后续 profile 改为从 CTD 文字
mask 覆盖的源像素选择字色：高饱和彩色簇达到最小占比时保留主导色簇，否则取字形暗色主体；背景亮度
不参与文字明暗决策，采样失败固定回退深色。白色 pen 提高到字号 50%、4–24 px。
...
URL、画廊标识、哈希、数据库副本和截图均只作为临时证据，不进入仓库。本证据关闭这一类“受污染短拟声
转录被强制回填”的已知反例，不代表所有拟声词语义、艺术字 OCR 或彩色样式已经跨画廊达标。

## 彩色真实画廊 P11–P20 扩展回归（2026-07-24）

同一用户授权彩色画廊继续扩展到 P11–P20；持久材料不记录页面、文字、URL、画廊标识、哈希、数据库或
截图。v46 在 P11 与 P13 两张旧拟声反例上均把 CTD 艺术字保留为原画，旧版不合逻辑的中文拟声覆盖从
两页中消失。P16–P20 的首次生产运行阶段如下：

| 页面 | 分析 | LLM 翻译 | 修复/排版/编码 | 总计 |
|---|---:|---:|---:|---:|
| P16 | 2.185 s | 15.301 s | 4.326 s | 21.848 s |
| P17 | 2.023 s | 9.932 s | 4.468 s | 16.489 s |
| P18 | 2.056 s | 11.448 s | 2.256 s | 15.815 s |
| P19 | 2.201 s | 12.464 s | 1.661 s | 16.385 s |
| P20 | 2.060 s | 14.149 s | 1.797 s | 18.058 s |

五页中位数为：分析 2.060 秒、LLM 12.464 秒、渲染 2.256 秒、总计 16.489 秒。该样本的首要耗时仍是
LLM，AOT/编码随实际 render plan 数波动；它不是跨设备 P50/P95。

扩展页同时推翻了 v46 的一条过宽保护：简体中文目标只要含任意假名就拒绝整个译文。真实 provider 会在
有效中文里保留人名或少量 OCR 残片，P16–P20 合计有 13 个块因此回退原文。v47 改为按脚本占比失败关闭：
仅当假名数量多于汉字时保留原稿；中文占主导、夹少量假名的块继续渲染。设备 P19/P20 新鲜回归中，旧版
分别 5/3 个译文拒绝均降为 0；主要为日文的输出仍被拒绝。目标设备用例 37/37，signed app、signed
`entry@ohosTest`、V2 inventory 与 `git diff --check` 通过。

视觉结果仍未过门。v47 的 P19/P20 分别有 7/6 个可绘制块，却仍得到 7/6 个独立 render group、合并数
都是 0；P19 出现多列拥挤，P20 右下多个译文互相覆盖，白色修复区域也没有形成气泡级整体。新增有效译文
...
## 彩色真实画廊 P29–P34 无文字页与提供方长尾（2026-07-24）

同一用户授权彩色画廊继续覆盖六张连续真实页；持久材料仍不记录页面文字、URL、画廊标识、哈希、数据库或
截图。P29/P30 是高文字量页，最新文档分别有 9/8 个 block，其中 P29 有 2 个 `sfx`；两页各有 1 个
包含假名残片的译文块和 1 个包含拉丁字母或数字残片的译文块，视觉质量仍未过门。

| 页面 | 分析 | LLM 翻译 | 修复/排版/编码 | 总计 |
|---|---:|---:|---:|---:|
| P29 | 2.768 s | 80.251 s | 3.477 s | 86.664 s |
| P30 | 2.677 s | 11.606 s | 4.370 s | 18.714 s |

P29 的 provider 请求本身耗时 79.820 秒，而 P30 为 11.177 秒；这次 80 秒级延迟是远端翻译提供方长尾，
不是 detector、OCR 或渲染阶段持续退化。真实阅读体验的 P95 不能再仅用十余秒的常见样本估算。

P31/P33/P34 均没有形成可翻译 block，在 1.897/1.492/1.788 秒端侧分析后停止，未调用 LLM，原图保持
不变。P32 由 CTD 留下 1 个假阳性候选并调用一次 provider，但本地渲染门最终得到 0 个 render plan，
在 6.543 秒安全失败且没有覆盖原画。该结果证明无文字页大多能低成本停止，但分析后、调用 provider 前
仍需更严格的假阳性质量门。

当前错误语义不正确：零文字页被日志归为 `provider_request_failed`，Reader 又统一显示“端侧漫画图像
处理暂不可用”，会把正常的“当前页没有可翻译文字”误报为服务故障。下一实现项必须提供独立的无可翻译
文字结果，并保持真正的 provider、端侧分析和渲染错误互不混淆。

本轮还确认译文文档缓存身份此前遗漏提示词版本：提示词变化后可能读取旧译文文档，再把旧内容写成新提示词
版本的渲染产物。视觉编排器现已把 `promptVersion()` 纳入文档 provider 身份；设备回归 9/9 证明提示词
变化会重新分析、翻译和渲染，单纯渲染配置变化仍可复用译文文档。另一次只靠提示词提高目标脚本纯度的
P22/P23/P25 对照结果不稳定，已拒绝并恢复文本协议 v8，不能以提示词试验替代 OCR 残片和长段精炼门。

零文字错误语义随后完成修复。P31 的真实 Reader 手动路径在 2.466 秒后得到
`no_translatable_text`，日志确认 `blocks=0` 且没有进入 provider；原图保持不变，页面显示“当前页未
检测到可翻译文字”。对应编排器设备回归为 10/10，四语资源、signed app 和 signed
`entry@ohosTest` 均通过。真正的 provider、端侧分析和渲染失败分类保持不变；P32 一类“假阳性候选已
进入 provider、渲染前被拒绝”的成本问题仍开放。

P32 的 provider 前质量门随后关闭。修复前，端侧分析已把唯一候选标记为
`local_source_geometry_not_supported`，但编排器仍调用 provider：分析 2.390 秒、provider 5.326 秒，
总计 7.866 秒，最终 0 个 render plan。修复后，区域后端在不删除文档 block 的前提下声明是否至少存在
一个可安全替换的候选；全页候选均需保留原稿时，编排器以 `no_translatable_text` 在文本翻译前停止。
同一真实 P32 的复测为分析 2.444 秒、总计 2.530 秒，日志没有
`text_request_payload`、`text_request_timing` 或 provider 调用。

为防止门禁被做成只适配单一反例，本轮又扩展三类真实页。纯画面 P41 得到 0 个 block，在分析
1.778 秒、总计 1.790 秒后停止；带道具小字但无对白的 P45 得到 2 个 block，两块均需保留原稿，在分析
1.662 秒、总计 1.673 秒后停止；两页均未调用 LLM。高密度正文 P20 的新鲜 cache miss 则正常进入完整
链路：9 个 block 中 6 个可回填、3 个保留原稿，分析/LLM/渲染/总计为
2.119/15.562/4.404/22.165 秒。设备编排器回归为 11/11，证明门禁同时覆盖零文字、全保留和正常混合页，
不会因部分 block 保留原稿而跳过整页翻译。

P20 的视觉结果仍未过门。彩色源字和部分艺术字原稿得以保留，但译文中夹带的拉丁单词被按字符纵排，
