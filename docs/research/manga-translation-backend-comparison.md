...
- **current Reader profile**: `core-vision-ocr-bubble-group-v33` /
  `reader-local-bubble-layout-v45` / `local-ctd-aot-inpaint-v30`
...
设备 `237` 的同进程 20 轮原生压力基准进一步给出当前资源边界：AOT `P50/P95=261/428 ms`，CTD
`P50/P95=643/972 ms`；AOT 全程处于系统热档位 1，进入 CTD 后为热档位 2。CTD 的双 1024 × 1536 mask
改用 VM 管理的 ArrayBuffer 后，PSS 从 540,646 kB 到 561,721 kB 峰值，随后回收到 432,968 kB，最终
454,164 kB。相同作用域、旧 external ArrayBuffer 对照从 514,482 kB 持续升到 582,502 kB，没有在 20 轮内
回收。输出位完全相同，性能中位数基本不变，因此该修改只修复 VM 无法感知外部 mask 压力的问题，不改变
CTD profile、视觉结果或缓存身份。

该压力基准关闭“原生 AOT/CTD 热调用与 mask 输出是否明显逐轮滞留”的子门，不替代真实 Reader 页面的
解码、OCR、LLM、PNG 编码和页面缓存测量。后续优先扩大合法艺术字/负样本集，并在真实普通页和长图中统计
完整流水线 P50/P95、峰值 PSS 与失败回退，再评估真正面向移动端的轻量修复模型。整图模型可并行接入，
但不能代替这条可编辑、可缓存的分阶段主路径。
