...
去重区域，加载 39 ms、首次 207 ms、热推理 160 ms，完整 Hypium 253/253。模型仓库自身标记为
[MIT](https://huggingface.co/YSGforMTL/YSGYoloDetector?not-for-all-audiences=true)，正式分发前仍需完成独立
model-pack 来源、转换和许可证记录。

这一步证明“模型可端侧运行”，不表示 Reader 已使用它。production 接线还需要模型下载/校验/清理、设备
能力回退，以及把 Core Vision OCR 行可靠分配给 OBB；后续才轮到专用 OCR 和 inpaint 模型。
