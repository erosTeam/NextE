...
- **local profile**: `core-vision-ocr-directional-render-v13`
...
API/Codex 传输和本地衍生页缓存。它没有端侧神经网络 detector，也没有 LaMa 级内容生成式 inpaint；
`ComicLocalVisualBackend` 中的判断、mask、背景恢复和布局仍是确定性算法。
...
下一阶段不应继续把所有质量问题压进 `ComicLocalVisualBackend` 的启发式。优先边界是：先把 region
detector/OCR 作为可下载视觉 profile 接入，再建立残留原文、mask 越界、排版 overflow 和连续多页资源门；
整图模型可并行接入，但不能代替这条可编辑、可缓存的分阶段主路径。
