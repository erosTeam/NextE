...
- **current Reader profile**: `core-vision-ocr-bubble-group-v30` /
  `reader-local-bubble-layout-v43` / `local-ctd-aot-inpaint-v28`
...
AOT 已形成现有端侧组合；LaMa large 已因真机资源和延迟门被否决。后续优先测 AOT 重叠区域合并/批处理、
连续多页 P50/P95 与更大的合法艺术字/负样本集，再评估真正面向移动端的轻量修复模型。整图模型可并行
