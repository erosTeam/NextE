...
`2+ credits`，Gemini 3.5 Flash 为 `3+ credits`。但公开契约没有机器可读的模型目录端点，NextE 不能把
网页当前选项硬编码成长期目录；必须在模型失效时允许刷新/回退。其更新记录还说明跨页 context 可能按
...
“分阶段 Torii”不是把整图翻译接口暂停在中间，而是不用 `/api/v2/upload` 的一键译图路径，改为编排三个
独立端点：

1. 原图调用 `POST /api/v2/ocr`；Torii 返回 paragraph/line/word/symbol 层级的原文、polygon、置信度、
   字号、角度、横排/竖排方向、对齐和颜色。NextE 为 paragraph 建立稳定 blockId，并保留原始像素坐标。
2. NextE 把 blockId、原文、必要的区域裁图、术语表、前页摘要和画廊上下文交给已选 `ComicTextTranslator`；
   这里可以使用普通 API Key，也可以继续使用 Codex OAuth。返回结果必须逐 blockId 对齐。
3. NextE 从 OCR 的 word/symbol polygon 生成文字 mask，连同原图调用 `POST /api/inpaint`，取得消字图。
4. NextE 把译文和 OCR 的 polygon、direction、angle、alignment、颜色、fontsize 组合成 `text_boxes`，将
   消字图调用 `POST /api/typeset`。日/中文竖排使用 `top_to_bottom` 并保留对应角度，得到最终译图。
5. 最终图片仍经 NextE artifact 校验、页面 identity、缓存和 Reader 发布 fence；任一阶段失败都回退原图，
   不能把 OCR 文本面板当作完成结果。

这条路线的 Torii 费用是 OCR 1 + inpaint 0.02 + typeset 0.02 = 1.04 credits/页。其价值不是省下 0.04
credit，而是 LLM 凭据不经过 Torii、支持 Codex OAuth，并由 NextE 统一人物名、术语和跨页上下文。代价也
很明确：NextE 重新承担 block 映射、mask 膨胀、竖排参数和分段重试；OCR 漏框无法被后续翻译补回，mask
过窄会残字、过宽会伤画，独立 typeset 的效果也未被证明等同于 Torii 一键路径的内部编排。公开文档没有
提供“提交 NextE 译文，同时复用整图接口内部检测/消字/排版”的端点。

因此首轮云端质量验证应先使用 Torii 一键整图，由用户选择 Torii 模型并可选 BYOK；只有当一键质量已达标，
而 Codex OAuth、统一 LLM 源或逐块可控性确实值得额外复杂度时，再对同一 fixture 验证分阶段路线。两条路线
最终都必须发布同一种 `ComicRenderedPage`，Reader 不感知内部差异。
