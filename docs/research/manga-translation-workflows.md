...
因此不再做无凭据的 API 穷举、也不新建 provider 或设置页。重新评估只能由用户明确授权某个服务的
数据转交和费用边界，或由厂商公开修正上述条款/计量后发起新的独立计划。
...
|---|---|---|---|
| [Torii Image Translator API](https://toriitranslate.com/api) | `POST /api/v2/upload` 一次返回最终译图、消字图、逐块原文/译文/坐标/方向/颜色/字体和更新后的跨页 `context`；服务本身支持自带供应商或自托管兼容密钥。 | 作为独立 `ComicWholePageRenderBackend`：NextE 只上传整页和上下文、接收最终图片，不把 Torii 的 OCR/inpaint/typeset 子端点拆入端侧管线。NextE 只开放 Torii credits 代付，不开放 BYOK。 | **首个已验证云端整图 provider**：两页真实出图、上下文续接和本地缓存不重复扣费已通过；严格称呼一致性、站牌和拟声词质量仍需更广样本。其[隐私政策](https://toriitranslate.com/privacy)说明部分图片可能临时进入私有存储做诊断/质量控制，提取文本会转发给所选模型。 |
...
第二行只记录 Torii 对外提供的能力和费用，不是 NextE 当前产品接法。NextE 的 BYOK/自托管移除决策见
[漫画翻译设计](../manga-translation-design.md)。
...
Torii 的 `translator` 模型选择与 BYOK 在服务契约里是两个正交参数；这仍是费用研究事实，但 NextE 不再
把共享 LLM 源、URL、模型或 Key 转交给 Torii。设备 `237` 的真实矩阵中，同一可列出 39 个模型的
OpenAI-compatible 源只有 `gemini-2.5-flash-lite` 成功，DeepSeek 与 GPT 路由分别拒绝 Torii 所需的
`response_format`、`usage` 或 `reasoning` 参数。NextE 无法预检或修改 Torii 的上游负载，因此产品只保留
Torii credits 代付；端侧路线继续独立使用共享 LLM 源、Codex OAuth、术语上下文和自有视觉处理。
...
协议。因此 Torii 永远作为独立一键整图 provider，NextE 只接入托管代付，不接入 BYOK/自托管模式。
Torii 与端侧路线最终都发布同一种 `ComicRenderedPage`，Reader 不感知内部差异。
