...
| 路线 | 优势 | 主要缺口 | 适合场景 |
|---|---|---|---|
| 整页多模态 | 接入快，能结合画面理解语气、顺序和指代，可同时给出原文与译文 | 精确坐标、蒙版和逐字可核对性不稳定；模型可能漏字或补字 | Reader 译文面板、快速阅读辅助 |
...
1. 第一版不要求 OCR，也不要求精确坐标；整页多模态可以承担原文转录和草译。
2. 第一版必须保存原文，不能只保存译文，否则无法检查识别错误、建立术语表或做后续一致性修订。
3. 坐标、蒙版、说话人和样式必须是可选能力；缺失时显示文本面板，存在时才能启用覆盖或重绘。
4. 术语和跨页摘要属于画廊项目，不能只放在一次模型请求中。
5. 视觉分析结果、文本翻译结果和渲染结果必须分别缓存，避免改一个人名就重新上传图片。
6. 低质量结果应进入可见的待复核状态；人工原文、译文和锁定术语不得被后续模型静默覆盖。
7. 当前没有必要把完整桌面翻译栈移植到应用内。先建立 provider-neutral 文档和接口，未来可接直接
   多模态 API、自建网关或用户自托管后端。
