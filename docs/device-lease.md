...
HDC target/IP 只表示本轮如何连接设备，可能随网络和调试端口变化；它只能写入 manifest 的
`target` / `authorizedTarget` 和当次执行记录，不得单独用作长期设备名、证据目录名或跨轮比较依据。

每份设备视觉证据必须另外记录并按以下状态分组：

- 稳定设备别名和设备型号；
- 折叠状态：`folded`、`unfolded`、`half-folded` 或 `not-applicable`；无法确认时只能写
  `unknown`，该样本不得用于折叠状态间的几何结论；
- 窗口方向；
- 该状态下实际生效的窗口/显示像素宽高。

证据目录使用稳定身份和状态命名，例如
`<device-alias>__<model>/<fold-state>/<orientation>-<width>x<height>/<scenario>/<run-id>`；
不得使用 `237` 或完整 IP 代替设备身份。一次连续录屏包含旋转、折叠/展开或窗口尺寸变化时，
manifest 必须按动作顺序记录源状态和目标状态；目录中的状态段使用
`<source-state>_to_<target-state>`。HDC target 变化而稳定设备身份与状态未变时可以比较；稳定设备、
折叠状态、方向或实际尺寸不明或不一致时，不得进行跨样本像素坐标、缩放比例或落点结论。
