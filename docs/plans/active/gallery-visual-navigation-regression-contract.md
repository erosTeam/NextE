...
- 2026-06-19 06:59 +0800 — Gate V2 current-main reachable-subtab refresh added on Mate X7 emulator target `127.0.0.1:5555`; hdc commands ran through the approved DevEco hdc path, outside the Codex sandbox policy boundary. A stale contract expectation was corrected in `scripts/test_retained_tab_contract.mjs`: retained Home/Favorites/Toplist bottomBuilders now gate the current helper form where `bottomBuilder(..., height = SELECTOR_BAR_HEIGHT)` is the single source of truth, while Download is the explicit `DOWNLOAD_SELECTOR_BAR_HEIGHT` exception. Gates passed: `test_selector_reload_preserves_content_contract.mjs`, `test_retained_tab_contract.mjs`, `test_gallery_paging_contract.mjs` (47 assertions), V1 inventory 0, and official `scripts/build_hvigor_signed.sh` PackageHap + SignHap. Current signed HAP was installed. Home default baseline, Home default -> popular immediate/settled, Toplist all baseline, and Toplist all -> year immediate/settled were captured. Layout inspection found real gallery rows and no terminal `没有数据/没有更多了` copy in both immediate and settled switch states. Evidence: `/private/tmp/nexte_gate_v2_current_main_evidence/` (`home.png/json`, `home_hot_immediate.png/json`, `home_hot_settled.png/json`, `toplist_all.png/json`, `toplist_year_immediate.png/json`, `toplist_year_settled.png/json`). Favorites favcat/order and Site 表/里 remain outside this refresh because they require safe logged-in/auth state. SearchFilter/Search chrome are current baseline and were not reopened.
