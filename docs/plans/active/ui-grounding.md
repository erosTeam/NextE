...
Primary information: when large gallery pages load slowly, the Reader should show a native busy indicator until image byte totals are known, then switch to native linear progress for cache-backed image downloads; users can reduce or increase forward preloading from 0 to 5 pages.
...
Reuse or deviation: reuse `CachedImageFileService.load()`/`cached()`, visible Reader image components, native `LoadingProgress()` for unknown-total waits, native `ProgressType.Linear` for determinate downloads, and native settings row dropdown menus; deviate by replacing the off-screen `Image(remoteUrl)` warmer with a cache-backed warmer, giving streamed image downloads a bounded reader-specific connect timeout while keeping the long binary read timeout, clearing automatic retry count only after success or explicit source reset, and replacing hardcoded preload windows with the persisted 0..5 setting.
...
Verification: UI grounding contract, gallery paging contract, V1 decorator inventory, signed HarmonyOS build, and emulator smoke covering date seek, pull-to-refresh previous-page prepend, then back-to-top returning to page 1 instead of prepending again.
