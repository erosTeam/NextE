...
- **consumers**: comment translation and staged manga translation
...
Torii 云端整图不是 LLM 源 consumer。2026-07-22 的设备实测证明，通用源可查询模型并不代表能够接受
Torii 的结构化输出与附加参数；NextE 已移除 Torii BYOK/自托管入口和凭据转发。此边界由
[漫画翻译设计](../../manga-translation-design.md)的架构决策约束，不得借共享源管理恢复。
...
- Torii 整图只保存 Torii API Key、官方模型和字体，不显示共享 LLM 源或自托管模型；
...
- [x] 把 API model catalog、Codex OAuth/token refresh/model catalog/usage 从漫画设置类中抽到 source service；
- [x] 为 chat-completions、Responses、image input/output 声明能力，不凭 source type 猜模型能力；
- [x] 评论与漫画通过 source resolution 获取一次性运行配置，服务层不读取彼此 settings。

### C. 幂等迁移

- [x] 用旧设置 fixture 覆盖仅评论、仅漫画 API、两者相同、两者不同、Codex、部分损坏和重入；
- [x] 先持久新源/secret，再发布 binding；失败保留旧路径，成功后停止旧键双写；
- [x] 不在诊断、fixture 或测试输出中泄露真实 Key/token。

### D. 设置页面

- [x] 新增同级 LLM 源列表/详情；源详情承载添加、编辑、认证、模型目录、用量和删除；
- [x] 评论翻译页收敛为策略 + source/model binding；
- [x] 漫画翻译页收敛为 source/model binding + 漫画业务/制图服务；
- [x] 保持三个翻译入口层级，不新增“翻译总入口”嵌套。

### E. 验证与清理

- [ ] 评论普通 API、漫画 API、漫画 Codex 和同源双消费者均通过 fake transport；
- [x] 真机迁移保留设备 `237` 的现有 Codex 登录与模型选择，评论现有设置不丢失；
- [x] 构建、Hypium、V2、i18n、persistence、secret、backup/sync 门禁通过；
- [ ] 旧专用设置类不再被运行服务读取后，单独提交删除/兼容清理。

## 2026-07-21 验收记录

- 设备 `237` 迁移得到 3 条独立可复用源：评论 API、漫画 API 与 Codex；引用计数分别为 1、0、1，
  没有把不同凭据错误合并；
- 漫画消费绑定保留 `Codex（迁移） + gpt-5.6-luna`，评论消费绑定保留
  `评论翻译 API（迁移） + deepseek-chat`；Codex 登录账号仍有效；
- Codex 用量详情先显示缓存再后台刷新，只呈现后端实际返回的 `7D` 剩余百分比；未返回的 `5H` 不显示，
  页面没有手动刷新按钮；
- 首次真机进入 LLM 源管理暴露 `LlmSourceManager` 未登记到 `IndexRouteCoordinator` 的崩溃；补齐
  manager/detail route family 并增加回归测试后，列表与两类详情均可稳定进入；
- 设备 Hypium：`Tests run: 199, Failure: 0, Error: 0, Pass: 199`；签名 production 与 ohosTest
  构建通过，V2 inventory 为 `0 file(s)`，i18n、persistence、secret、backup/sync 门禁通过；
- 同视口前后对比：`.hvigor/outputs/llm-source-ui/compare-manga-before-after.jpeg`。改造后漫画页只保留
  source/model binding 与提供方评测，账号、配额、凭据和模型目录均回到源详情。
