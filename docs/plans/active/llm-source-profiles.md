...
- **consumers**: comment translation, staged manga translation, and Torii BYOK
...
Torii 云端整图在 BYOK 模式下也选择现有 OpenAI-compatible 源和独立模型，不复制 URL/Key；请求时才把
源 URL、模型和可选 Key 明确披露给 Torii。Codex OAuth 不进入该第三方凭据转交路径。
...
- Torii BYOK 在漫画页复用 OpenAI-compatible 源选择；Torii API Key、计费和字体仍属于整图 provider 设置；
