...
| 关口 | 当前工作 | 通过出口 | 失败出口 |
| --- | --- | --- | --- |
| G1：冻结真实样本与真值 | 固定 Core-12：4 类真实页面（常规闭合黑白气泡、彩色闭合气泡、开放/贴字文字、密集多列或 SFX），每类 3 页；一次性批量补齐容器/原文保留真值与独立 holdout 标记。现有真实 provider 三页只算候选样本，不能替代真值。 | 可复跑地报告每个候选的容器召回、泄漏、残留原文、文字重叠与越界；训练/开发/holdout 不混用。 | 若真实样本或真值不能支持稳定比较，停止调参和宣称质量提升，先将缺口记录为数据问题。 |
