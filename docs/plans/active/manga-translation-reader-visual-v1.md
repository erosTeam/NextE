...
2026-07-26 离线容器评审升级为目标框 schema v2：候选与目标做一对一空间匹配，并按 recording 的
source layout 把目标分成 `eligible`、`merged_source`、`missing_source`。旧 schema v1 已用 final-v5
记录复跑，10 真/13 假的既有统计保持兼容。视觉复核同时删除一处错误真值：`p13-top-left` 是包含人物和
场景的整格画面边框，不是可擦除字幕容器。修正后公版四页共有 17 个目标，其中 12 个有独立 source
layout、2 个 OCR 合并、3 个 OCR 缺失；旧候选只命中 10/12 个 eligible 目标。
...
OCR 合并或缺失。七页、43 个 layout 的真实彩页硬负向中矩形分支新增接受为 0，既有普通容器候选和最终
译图指标不变。

该候选继续禁止接管 Reader：全部候选 precision 仍只有 46.1538%，而四条独立最高分边不保证来自同一
轮廓。下一质量阶段不再继续调统一阈值，拆为两个工作项：容器侧增加同轮廓/成对平行线证据以降低误检；
OCR 侧单独处理 2 个合并目标与 3 个缺失目标。任何整格画面边框都不得为了提高 recall 进入擦除范围。
