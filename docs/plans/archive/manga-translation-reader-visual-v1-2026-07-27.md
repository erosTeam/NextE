# 漫画翻译端侧视觉 Reader V1

- **status**: active; 2026-07-22 local-first + validated independent whole-page cloud route
- **created**: 2026-07-21
- **product authority**: [漫画翻译设计与演进指南](../../manga-translation-design.md)
- **reset audit**: [漫画翻译产品重置](../completed/manga-translation-product-reset.md)
- **research**: [漫画翻译工作流调研](../../research/manga-translation-workflows.md)
- **backend A/B**: [视觉后端与可替换技术栈](../../research/manga-translation-backend-comparison.md)

## 结果与边界

当前目标是低操作成本的 Reader 阅读翻译。用户显式翻译当前页后，Reader 显示一张原文不再干扰、译文
位于对应区域的视觉译制页；缩放、平移、单双页、切页和返回缓存继续工作。文字列表、结构化文档、模型
响应、编译成功和测试数量都不是产品结果。

正式默认路径必须在端侧完成检测、OCR、原文处理、排版和图片编码，只调用用户已选择的 LLM 源进行翻译
及必要的视觉语境判断。普通用户不部署外部程序、不配置制图服务地址和账号。现有 sidecar 链路只作为
端侧实现的协议原型、质量基准和显式高级后端；它通过真实设备验收只证明工作流能够闭环，不满足本计划
新的本地交付退出条件。

V1 不建设专业漫画制作台，不承诺印刷级修图、所有拟声词/艺术字完美处理、PSD/CBZ 导出或整章人工
工作流。后续制作分支复用本计划的数据和渲染边界，但有独立入口与验收。

## 正式默认路线

```text
Reader 原图
  -> 端侧视觉后端：检测/OCR，返回区域原文与本地渲染文档
  -> NextE：规范化 ComicPageDocument，装配前页/摘要/术语
  -> 已选 API 或 Codex provider：按 blockId 返回上下文译文
  -> 端侧视觉后端：消字/修复/排版并生成结果图片
  -> NextE：校验并缓存非破坏性衍生页
  -> Reader：显示视觉译制页
```

端侧后端复用现有 `ComicRegionRenderBackend`、`ComicPageDocument`、`ComicTextTranslator` 和
`ComicRenderedPage` 边界；替换视觉执行位置不改变 LLM 源、上下文、页面身份、缓存或 Reader 发布协议。
本地模型按需下载并校验，至少覆盖 detector、OCR 和可选 inpaint；长图必须分块且所有 geometry 回到原图
坐标。基础档可以先使用确定性的 mask/背景填充与文字布局，高质量档再引入本地修复模型，但任何档位都
必须输出真实视觉页面，不能降级成文字面板。

## 外部原型与可选后端

现有 sidecar profile 以 `manga-translator-ui v1.9.9` / commit
`696dc63bd0b4803f96cc3d4f844322cef4910f8e` 为上游基线，并要求 NextE 兼容补丁提供
`/translate/import/json/nexte-load-text-v2`。vanilla v1.9.9 的 import 虽返回 200，却没有真正应用
`load_text` 工作流参数；未暴露专用路由的服务不属于该 profile。不能把未固定的 `main` 或 v2.x 当作
同一 profile：截至 2026-07-21 的静态复核中，其导出响应模型与 `TextBlock.to_dict()` 字段也已失配。
NextE 只做窄协议互操作，不复制其 GPL 实现或模型权重。该后端用于研发对照和显式高级模式，不得成为
普通用户路径的配置要求、默认失败原因或正式 V1 交付依据。

整图出图是第二条完整入口，不被本计划废弃。首个实现固定为 Torii 一键整图：原页和画廊上下文只发送给
`POST /api/v2/upload`，直接接收最终译制图片。Torii 不复用、不拆入端侧/sidecar 分阶段管线，其公开的
OCR/inpaint/typeset 子端点不属于本计划。该 provider 实现 `ComicWholePageRenderBackend`，跳过本地
检测/OCR/render，但仍经过统一视觉产物校验、render identity、缓存、原图回退和 Reader 发布。

整页多模态仍可提供图像语境、转录对照或上下文译文，但当前 `geometry=false` 的 analyzer 不能独立发布
视觉页。sidecar 不可达或输出不合格时，Reader 保留原图并显示可重试失败，不显示文字面板代替结果。

## 稳定接口与身份

实现前后都保持三个独立边界：

- `ComicRegionRenderBackend`：检测/OCR 与后续 source treatment/render；拥有 backend profile 和短期适配器模板；
- `ComicWholePageRenderBackend`：可选的整图出图能力；直接返回衍生图片和 provider identity，不伪造逐框
  geometry；
- `ComicTextTranslator`：接收规范 block、整页图像和画廊上下文，按 blockId 返回译文；复用 API/Codex
  传输与认证，但不复用整页 analysis-only 响应结构；
- `ComicRenderedPageRepository`：保存可再生成的视觉衍生页及其完整身份，不把文件路径当内容身份。

render identity 至少包含：稳定画廊/页身份、原图 SHA-256、region/backend profile 与 revision、OCR/document
revision、translation source profile/revision/model/prompt/context/glossary revision、source-treatment profile、layout profile、
render backend/profile/revision、目标语言。任一可见输入变化都必须使下游衍生页过期；只改译文不得重跑
检测/OCR，只改布局不得重新调用翻译模型。

`ComicRenderedPage` 至少记录 project/page、source image hash、artifact hash、MIME、宽高、render identity、
创建/访问时间和状态。原图、sidecar 模板、生成文档和衍生图片都属于可再生成数据，默认不进入备份/同步；
未来人工译文、locked 术语和用户备注必须另行定义用户数据所有权。

## 设置与安全

漫画翻译仍是设置首页三个独立翻译入口之一。页面先选择视觉路线：默认“端侧处理”，或用户主动选择
“Torii 云端整图”。端侧路线继续从共享 [LLM 源管理计划](llm-source-profiles.md)选择
`sourceProfileId + modelId`；Torii 是整图视觉 provider，不加入 LLM 源列表。Torii 路线选择其官方模型
快照并使用 Torii credits 代付，不显示 BYOK、自托管端点或共享 LLM 源。

普通设置移除“制图服务”入口，不把 sidecar 伪装成第三种 LLM provider。若保留远端 sidecar，只能放在
明确标注的开发/实验入口，并默认关闭。Torii 只保存其自身 API Key；默认端侧路线不得读取 Torii 凭据或
上传图片，共享 LLM 源密钥也不得转交给 Torii。

sidecar 凭据与 API Key/Codex token 分开保存、分开脱敏、分开备份审计。远端图片上传只能由用户显式
翻译动作，或显式开启的本次 Reader 自动翻译会话触发；普通 Reader 预取和页面出现不得调用。协议适配器
必须限制请求图片、ZIP/JSON、条目数量、文本、模板和结果图片大小，拒绝路径穿越、压缩炸弹、异常
MIME/尺寸及不匹配页面身份。HTTP 仅用于用户明确配置的本地/私网 sidecar，并持续提示明文传输；公网
地址要求 HTTPS。

## 当前 V1 收敛路线（2026-07-26，唯一执行队列）

下面 A–F 保留为实现与证据历史，不能再把其中散落的“下一阶段”当成新的工作授权。端侧视觉
Reader V1 现在只剩 **3 个连续的收敛关口**；提交只是保存已经验证的增量，提交后必须自动进入下一项，
不是向用户停工或重新索要方向的理由。

| 关口 | 当前工作 | 通过出口 | 失败出口 |
| --- | --- | --- | --- |
| G1：冻结真实样本与真值 | 固定 Core-12 开发集：4 类真实页面（常规闭合气泡、彩色闭合气泡、开放/贴字文字、密集多列或 SFX），每类 3 页；另从不同画廊冻结不参与方案选择的 holdout。一次性批量补齐容器/原文保留真值。现有真实 provider 三页只算候选样本，不能替代真值。 | 可复跑地报告每个候选的容器召回、泄漏、残留原文、文字重叠与越界；开发集与 holdout 不混用。 | 若真实样本或真值不能支持稳定比较，停止调参和宣称质量提升，先将缺口记录为数据问题。 |
| G2：容器方案一次决策 | 仅比较现有像素轮廓、矩形分支和语义 detector 候选；先用冻结数据筛除，再用未参与选择的 holdout 视觉复核。不得为了通过单页结果持续叠加启发式。 | 一个方案在四类页面均满足本计划既有的无越界擦画、无残留/重叠、可读排版和保留不安全 SFX 的视觉门；黑白基线不回归。 | 没有方案通过时，明确拒绝“端侧自动容器约束”进入 V1；复杂/开放/艺术字页保留原图，整图云端路径独立可用，不继续无限调参。 |
| G3：Reader 交付关闭 | 只把 G2 已通过的方案接入生产；用真实 provider 的独立页面验证完整链路，并完成长图、连续翻页、缓存、失败回退、P50/P95 与全部本地门禁。 | Reader 可发布的端侧范围、性能和已知不支持页型都有实测证据；通过后关闭本 V1。 | 若真实 Reader 验收不通过，回退到最后通过的安全行为并只修复该明确缺陷；不得重开 G1/G2 的泛化调参。 |

当前正在执行 G1。其人工部分是一次性的批量真值标注，用来结束“凭观感反复调参”；不会在每次实现改动后
重新人工审核整套数据。G2 不通过会直接走明确的降级边界，G3 之后没有隐藏的“第四阶段”。

## 实施阶段

共享 LLM 源迁移在 B/C 之前完成。视觉领域契约 A 不依赖现有漫画专用设置，可以先独立提交；之后不再
给 `ComicTranslationProviderSettings` 增加字段或继续扩张专用凭据 UI。

### A. 协议与领域契约

- [x] 固定 upstream revision/profile，并保存不含漫画隐私的最小 export/import fixture；
- [x] 定义 region、adapter template、translation batch、render profile 和 rendered artifact 模型；
- [x] 同时定义 `ComicWholePageRenderBackend` 能力与统一产物校验接口，首个提交不要求真实 provider 实现；
- [x] 明确各 revision、cache key、stale 传播、失败保留和清理语义；
- [x] 用 fake backend 证明无 geometry、缺 block、重复 block、越界区域和错误图片不能进入 render-ready。

### B. 外部原型适配器（研发证据，不是本地 V1 退出条件）

- [x] 实现 bounded multipart export 请求与 ZIP/JSON 解析，规范化区域、顺序、原文和 backend template；
- [x] 实现 bounded import/render 请求，校验返回图片签名、尺寸、内容 hash 和页身份；
- [x] sidecar 版本/能力不兼容时本地失败，不让部分结果覆盖最后成功检查点；
- [x] 设置页仅增加同级“制图服务”配置与连接检查，不改变三个翻译入口层级。

该设置项是原型阶段遗留，端侧 reset 后必须从普通用户路径移除；以上完成项不再代表正式部署完成。

### B2. 端侧视觉后端

- [x] 固定首个可分发 detector/OCR 模型及许可证、模型大小、hash 和设备能力边界；YSGYolo detector 与
  PP-OCRv5 Mobile Recognition 已按组件许可证发布到 `model-pack-v1.1.4`，并接入可选下载、校验与回退；
- [ ] 在 HarmonyOS NDK/ncnn 上完成有界分块推理，输出原图坐标 geometry 与 OCR 文本；普通页 detector
  已输出原图 OBB，补充 recognizer 已输出 OCR 文本并接入 production，长图分块和扩展样本门仍未完成；
- [x] 接入 HarmonyOS Core Vision 系统 OCR 基线，按 2048px 有界分片并映射回原图 geometry；
- [x] 实现端侧方向保持、相邻纵列合并、局部背景遮盖、基础排版与 PNG 衍生页编码；
- [x] 建立两页原创 fixture 的端侧准确率、方向、耗时、画面覆盖和 PSS 可复现基线；
- [ ] 扩展合法质量集并完成 region recall、残留原文、遮盖越界、overflow 与连续多页 P50/P95 门禁；
- [ ] 增加漫画专用 detector/OCR、内容感知修复、字体回退和布局 overflow 质量门；
- [ ] 资源缺失、内存不足、模型不支持语言或处理失败时保留原图并给出可恢复状态；
- [x] 本地后端不读取 sidecar 地址或凭据，默认运行时身份只依赖本地能力与渲染 profile。

### C. 上下文文本翻译

- [x] 从现有 Responses/Codex 传输抽出 `ComicTextTranslator`，输出必须按 blockId 完整对齐；
- [x] 提供当前页图像、区域原文、最近前页、滚动摘要、术语与风格约束，并保持既有预算/脱敏边界；
- [x] 缺块、重复块、未知块、空译文或身份不匹配时拒绝候选；
- [x] 缓存 identity 使用模型实际可见的受限上下文，不因未发送内容制造错误 miss。

### D. 视觉编排与缓存

- [x] 将现有 orchestrator 收窄为分析/翻译检查点，再组合 region -> translation -> render stages；
- [x] 建立有界衍生页缓存；进程重启可命中，清理不会被旧飞行请求回填；
- [x] 失败保留原图和最后成功衍生页，过期结果只写回创建它的 page identity；
- [x] 任何 analysis-only 文档都不能返回 Reader-ready 状态。
- [x] 分阶段/端侧与整图 backend 仅在 `ComicRenderedPage` 发布边界汇合；整图路线不进入或复用逐阶段管线；

### D2. 可选托管整图后端

- [x] 静态复核具有公开 API 契约的托管候选；Torii 为首个漫画专用验证候选，PixLab 为通用图片翻译备选；
- [x] 记录公开费用与 BYOK 拆分，但 NextE 产品只保留 Torii credits 代付；
- [x] 保留 Torii `translator` 官方模型选择，移除 BYOK/自托管模型目录；
- [x] 明确否决 Torii 分阶段接入：只调用一键整图端点，端侧/sidecar 管线继续独立演进；
- [x] 在用户明确选择服务、提供测试密钥并接受费用/图片上传边界后，用两页原创 fixture 做同译文 A/B；
- [x] 固定 provider profile、认证、带复核日期的内置官方模型快照、请求/响应上限、超时、重复扣费、缓存
  命中和数据删除语义；不抓取 HTML 或共享 LLM 源充当 Torii 模型目录；
- [x] 通过 `ComicWholePageRenderBackend` 发布并复用现有 artifact 校验、缓存、原图回退和 Reader fence；
- [x] Torii 路线只保存独立 Torii API Key；不得读取或转发共享 API/Codex 凭据。

### E. Reader 替换

- [x] 保留现有显式动作、本地文件输入和 route/page/file/UI epoch fences；
- [x] 删除 `ComicTranslationSheet` 作为主结果，生成中/失败/待复核改为页面状态；
- [x] Reader 在同一页原图与衍生页间切换，衍生页走现有图片加载、缩放、平移和切页路径；
- [ ] 原文/译文检查仅作为翻译页的次级入口，未实现时可以暂不提供，不能阻塞视觉结果；
- [x] 单页失败重试、普通切页返回和持久缓存不串页，缓存命中不重复调用 provider；
- [x] 快速切页与单双页完成独立质量和发布 fence 验收；
- [ ] 长图继续做独立质量与资源边界验收。

### F. 端侧验收与关闭

- [x] fake backend、fixture、parser、identity、cache、failure 和 security tests 通过；
- [x] signed app、`entry@ohosTest`、持久化/secret/backup/i18n/V2 门禁通过；
- [x] 在用户指定设备上用合法样页完成一次端侧视觉后端 + 已选 provider 真实链路；
- [x] 提供同页原图与视觉译制页截图，证明原文不与译文竞争；
- [x] 证明缩放/平移/切页正常，返回同页命中衍生页缓存且不调用视觉后端/provider；
- [x] 关闭 sidecar 后默认路径仍可完整工作，普通用户流程不要求地址和账号；
- [x] 失败时原图保持可读，不出现文字列表替代品。

只有 B2 与 F 全部满足才允许称为“漫画翻译端侧视觉 Reader V1 完成”。外部 sidecar 的 A–F 历史验收
只能称为原型证据，不能继续沿用“V1 已完成”的表述。

## 第一阶段提交边界

第一个代码提交只做 A：领域模型、身份/失效规则、fake backend 和 fixture 测试。它不改 Reader UI、不增加
设置项、不调用模型、不上传图片；设备回归也只运行离线 fixture。协议契约通过后再进入 B，避免再次让 UI
领先于真实产物。

> 下面的 2026-07-21 记录保留外部 sidecar 原型的真实证据与已知质量问题。它们用于端侧实现对照，不表示
> 当前端侧 V1 已交付，也不重新授权普通用户配置外部服务。

2026-07-21：端侧 reset 的首个可运行基线已接入生产 Reader 运行时。`ComicLocalVisualBackend` 使用 Core
Vision 系统 OCR、2048px 分片、原图坐标 geometry、本地不透明原文处理、基础居中排版和 PNG 编码；运行时
不再读取 sidecar 地址/凭据，普通漫画翻译设置只保留共享 LLM 源、模型与提供方评测。设备 `237` 的合法
原创日文样页识别出标题、站牌与对白并生成 `1024x1536` 本地译制页；第一轮系统 OCR 把单字 `小` 误判成
大区域，基线现保守拒绝所有单字候选，避免错误遮盖人物，但也会漏掉单字对白、拟声词和注音。最终设备
Hypium 为 233/233，signed app、`entry@ohosTest`、资源 JSON、V2 门禁和 `git diff --check` 通过。

该结果只证明“无外部制图地址也能在设备上完成 OCR -> geometry -> 本地渲染”的端点无关闭环。它没有
证明断网资源可用，也没有完成已选 Codex provider 的生产 Reader 全链路、漫画专用检测、内容感知修复或
复杂排版质量；因此 B2 与 F 的相应项目继续开放，不能称为端侧视觉 Reader V1 完成。

2026-07-21：端侧质量基线补齐并升到 v13，详见
[漫画翻译端侧质量基线](../../research/manga-translation-local-quality-baseline.md)。设备 `237` 上最终
`core-vision-ocr-directional-render-v13` 连续三次目标质量用例均为 1/1；signed app 与显式 signed `entry@ohosTest`
构建通过。两页原创样例严格原文块命中 9/11（81.8%），已检测普通文本方向 9/9、reading-order error 0；
单页 OCR 为 228–380 ms，本地回填与 PNG 编码为 487–1449 ms，总计 867–1677 ms，不含 LLM 网络时间。
竖长气泡保持纵排，跨 Core Vision block 的相邻列按右到左合并，注音进入清理范围，竖排标点孤行已修正；
原文处理改为字形 mask 与局部邻域恢复，页级字号约束消除了第二页对白约 42/30 px 的明显断层。

同一次复核仍明确判定 V1 未完成：两页拟声词均漏检，复杂纹理和艺术字没有专用 detector/inpaint，系统
字体也没有匹配源页面风格，质量集不足以代表真实漫画。系统 OCR 文档因此提升为待复核警告；不能用本次
81.8% 或目标用例通过宣称通用可用。固定 Docker sidecar 的同页 A/B 为 10/11，并抓到第二页拟声词，但
仍出现注音残影、标点冲突、过重拟声词和书页文字倾斜/下划线瑕疵；它是可替换重后端与质量参照，不是
端侧成品质量捷径。

2026-07-21：领域契约已在设备 `237` 完成 184/184 Hypium 回归，其中新增视觉契约 6/6 通过；signed app
与 `entry@ohosTest` 构建、V2 门禁和 `git diff --check` 通过。随后固定首个 sidecar profile 为
`manga-translator-ui v1.9.9`，保存协议派生的原创合成 export/import fixture，并加入字段漂移、页身份和
模板 hash 拒绝测试；阶段 A 至此关闭。最终 signed app 与 `entry@ohosTest` 构建通过，设备 `237` 完整
Hypium 为 205/205，其中新增 sidecar 协议 4/4 通过。真实 sidecar 网络调用、ZIP 解包和返回 PNG 校验
仍属于阶段 B。

2026-07-21：阶段 B 的无 UI 传输子阶段完成。NextE 按固定提交的真实 FastAPI 字段发送
`image + config` 到 export、发送 `image + json_file + config` 到 import；请求图片、multipart、响应 ZIP、
JSON/TXT 和 PNG 均有独立上限。ZIP central/local header 会在系统 zlib 解包前检查，且只允许固定两个根
条目；公共 HTTP、认证 header 注入、源图 hash 漂移、profile 字段漂移、非 PNG、不可解码 PNG 和尺寸不符
均在写入视觉产物前失败。设备 `237` 的 fake transport 完整链路、ZIP 解包、blockId 回填、PNG 解码与原子
落盘测试 3/3 通过；完整 Hypium 为 208/208。制图服务设置与真实 sidecar 连接检查仍是 B 的剩余项，
当前代码不会因 Reader 出现或预取而上传图片。

2026-07-21：阶段 B 已补齐同级“制图服务”设置、凭据隔离与只读 capability 检查；检查只请求
`openapi.json`，不会上传漫画页。阶段 C 随后完成：`ComicTextTranslator` 复用现有 API/Codex Responses
传输、Codex token 刷新和统一 LLM 源选择，但使用独立的逐块响应协议。请求同时携带经最终 SHA-256/MIME
复核的当前页图像、全部 sidecar block 原文，以及受限的前页、滚动摘要、术语、翻译记忆和风格上下文；
context fingerprint 只覆盖协议实际发送的受限内容。模型响应会按原 document 顺序重建 batch，少块、重复
blockId、未知 blockId、空译文和 project/page/image/language 身份漂移均在 render 前拒绝。signed app 与
`entry@ohosTest` 构建、V2 门禁和 `git diff --check` 通过；设备 `237` 完整 Hypium 为 214/214，其中新增
多模态逐块翻译器协议 3/3 通过。阶段 D 的视觉编排与持久衍生页缓存仍未完成，因此当前 Reader 仍不能
把本阶段单独当作完整漫画翻译结果。

2026-07-21：阶段 D 的分阶段路线基础设施已闭环。`ComicVisualTranslationOrchestrator` 只发布经过
完整 identity 校验的 `ComicRenderedPage`，按 region -> context-aware translation -> render 顺序执行；
analysis document 与 translation batch 不再具有 Reader-ready 出口。Translator 返回的 source/model/prompt/
context/glossary identity 必须与当前请求逐项一致，错误批次在 render 前失败。衍生页仓库以原图、上下文、
provider、sidecar 和 render profile 的严格 lookup key 持久化，启动后复核文件大小和 SHA-256；最多保留
64 页/512 MiB，cache clear epoch 阻止旧飞行请求重新写入可命中索引。失败重渲染不会覆盖最后成功页。
signed app、`entry@ohosTest`、V2 门禁和 `git diff --check` 通过；设备 `237` 完整 Hypium 为 220/220，
其中本阶段新增编排测试 6/6 通过。Reader 原图回退/发布 fence 与整图 backend 路由仍属于 D/E 的剩余项，
因此此时仍只是视觉基础设施，不是可验收的 Reader 翻译结果。

2026-07-21：Reader 分阶段路线已接入生产运行时。显式“翻译当前页”会使用已选共享 LLM 源与制图服务，
只读取当前页已验证的本地原图；成功结果以 `file://` 衍生图片重新进入现有单双页/连续阅读图片路径，菜单可在
原图与译图之间切换。旧文字结果 sheet 已从主结果路径移除；route/file/UI request fence 会拒绝换画廊、换源图
或被新请求取代的迟到结果，单纯切页不会再丢弃已经消耗后端和模型的结果，完成页始终按发起 page identity
进入缓存。制图服务未配置的设备验收中，Reader 保持原图且只在顶栏下方显示非交互失败状态，没有文字列表
替代品；提示会明确指向制图服务而不是误报模型。最终 signed app 与 `entry@ohosTest` 构建、资源 JSON、V2
门禁和 `git diff --check` 通过，设备 `237` 完整 Hypium 为 221/221。该设备当前未配置可达的
`manga-translator-ui` sidecar，因此真实出图、原/译图对照、缩放切页与重启缓存证据仍未满足 F，不能称为 V1
完成。

2026-07-21：真实 sidecar 验收以 `manga-translator-ui v1.9.9` 对应提交
`696dc63bd0b4803f96cc3d4f844322cef4910f8e` 的 ARM64 CPU 镜像启动，并从实际路由确认 export/import
均通过 `X-Session-Token` 认证，而不是通用 `Authorization`。NextE 的传输、连接检查、secret 设置语义和
fake transport 断言已统一到该固定协议，避免能力检查成功但首次上传必然 401。修正后资源 JSON、signed app、
`entry@ohosTest`、V2 门禁和 `git diff --check` 通过，设备 `237` 完整 Hypium 为 221/221。真实样页出图与
Reader 缓存交互证据仍按 F 继续执行。

2026-07-21：真实 export 首次通过认证后暴露上游空配置会默认调用自身 OpenAI，空 key 导致 500；这与
“sidecar 负责视觉、NextE 选中的共享 LLM 源负责翻译”的边界冲突。使用同一真实镜像直接验证
`translator=original` 后，`/translate/export/original` 返回 200、OCR 区域和原文 ZIP，且不触发远端翻译。
协议 revision 因此升为 2，并固定 visual-only config；fake transport 同时断言 multipart 包含 `original`
且不包含 `openai`。后续真实链路不得向 sidecar 配置第二套 LLM。

2026-07-21：真实链路继续暴露 vanilla v1.9.9 的 `/translate/import/json` 会返回 200，却忽略
`prepare_translator_params()` 产生的 `load_text` 参数，随后再次检测/OCR并运行 sidecar translator；这正是
“接口成功但译图仍是原文”的根因。协议 revision 升为 3，固定上游 commit 的独立 GPL sidecar 补丁增加
`/translate/import/json/nexte-load-text-v1`，构建器会校验 commit、应用补丁并生成独立镜像；NextE 的
OpenAPI 检查要求专用路由，vanilla 服务会在图片上传前被拒绝。

同日设备 `237` 用合法日文画廊第 4 页完成真实验收。Reader 回调尺寸为 `2560x3634`，实际缓存源文件为
`1280x1817`；运行时现以编码文件的真实尺寸建立视觉身份，避免 export 身份误判。sidecar export 得到五个
区域，已选 Codex 源返回五个中文译文，专用 import 日志为 200 且没有第二次检测/OCR或 sidecar translator。
同页原图与视觉译制页、双击缩放、拖动平移、切到下一页再返回均已实机检查。切页返回以及应用重启后的
显式“翻译当前页”恢复得到同一 SHA-256
`8ecebff860bb4852d45d94bbafc786af49420b523495479235501ce966cdbb66`，期间 sidecar 无 POST，持久缓存路径
在 provider 调用前返回。F 的首条分阶段视觉 Reader 验收已满足；长图、双页和更广语料仍保留在 E 的后续
覆盖项，不能从这一个样本推断字体排版质量已经完成。最终 signed app 与 `entry@ohosTest` 构建、资源
JSON、sidecar builder 语法、V2 门禁和 `git diff --check` 通过；设备 `237` 完整 Hypium 为 221/221。

2026-07-21：sidecar 认证从“手工粘贴会话 token”收敛为账号自动登录。固定上游 v1.9.9 的源码事实是
`POST /auth/login` 接受用户名和密码、`GET /auth/check` 验证 `X-Session-Token`，服务端会话空闲 60 分钟
失效。NextE 将账号凭据保存在应用私有存储，备份仅进入加密 secret 分区；登录所得 token 只保存在内存，
并发请求共用一次登录，401 时失效被拒绝的 token、重新登录并最多重放一次。旧版本保存的原始 token 继续
兼容，直到用户主动保存账号凭据，避免升级时静默丢失认证。凭据不进入视觉 revision 或衍生页缓存身份，
但其指纹进入运行时 backend 身份，兼顾译图缓存稳定与凭据轮换即时生效。连接检查会验证 OpenAPI、登录和
`/auth/check`，不上传漫画页。fake transport 覆盖登录合流、并发失效、旧 token、失败边界和 401 重试，
设备 `237` 完整 Hypium 为 226/226；真实账号密码仍由用户在设置页输入，验收过程没有读取或复制现存秘密。

2026-07-21：同一合法画廊第 5 页用于连续页和更广语料验收。前一页持久文档已进入本页 prompt，页面完成
七个 block 的翻译并生成视觉译图，证明滚动摘要与前页上下文路径真实运行；但视觉对照同时发现两个不同
问题：一个小气泡 `むぅ～…` 没有被 sidecar 导出，属于区域检测覆盖缺失；一个已有区域的日文被 OCR 错认，
旧 prompt 又把错误原文当成权威，导致中文出现错误的“去山里”语义。v2 虽明确要求模型结合整页图纠正
OCR，设备真实重译仍得到“我要去山里了”，因此没有作为修复提交。v3 保留整页上下文图，并按 blockId
追加有界放大的区域裁图，polygon 同时进入 prompt 与 context fingerprint；同页设备重译把该块修正为
“别说傻话了——我先走了！”，RDB 中相邻两次持久结果也分别保留了错误与修正译文，证明不是视觉错觉或
旧缓存。区域裁图设备协议测试 3/3 通过。该页仍不是完整质量通过：整块漏检需要后续残留原文/补充检测阶段
解决，不能靠翻译 prompt 或裁图伪造无 geometry 的 block。

2026-07-21：整块漏检的下一步没有继续堆翻译 prompt，而是回到 pinned sidecar 的真实配置边界。原先只传
`translator=original` 的非空局部配置会继承更严格的检测默认值；`recall-v1` visual-only profile 现明确固定
`box_threshold=0.5`、YOLO OBB、零最小区域比例，并开启 `48px` 到 MangaOCR 的 hybrid retry。profile 身份
变化使旧七块缓存失效，设备 `237` 同一第 5 页真实重跑得到八个 block；新增 `p4-r1` 为
`むぅ〜` → `呣～……`，四点 polygon 经专用 import 路由进入视觉译图。RDB 最近两代结果分别为 7 和 8 块，
实机对照未观察到新增对白误检；另一个错误 OCR 区域仍由 v3 整页图加区域裁图正确译成“别说傻话了，我出门
了！”。最终 signed app 构建通过，设备完整 Hypium 为 226/226，V2 门禁为 0。该证据只关闭本页已知的整块
对白漏检；艺术字/SFX、残留原文检测以及长图、单双页和更广语料仍保持开放，不据此宣称通用完整率。

2026-07-21：同一合法画廊第 6 页暴露了高召回 profile 的假阳性：sidecar 将人物脸部一条极小线段以
`0.316` 置信度识别为单字母 `I`，文本模型进一步编造“（脸红）”并实际回填。`recall-v2` 只在单个 ASCII
字母、置信度低于 `0.5` 且区域面积占比不超过 `0.0005` 三项同时成立时删除 region；日文单字、短对白和
普通拉丁文本不进入该规则。prompt v4 另行禁止对表情、动作或景物编造括号旁白。过滤后旧 mask 会被清空，
sidecar 按五个保留 polygon 重建 mask，保证被拒区域不会只消字不回填。真实重试最初暴露上游 JSON 坐标为
`float64`、OpenCV `fillPoly` 只接受整型坐标的 500；版本化补丁和镜像升级为
`nexte/manga-translator-ui:v1.9.9-nexte2`，增加 `/translate/import/json/nexte-load-text-v2`、`int32`
转换和 load-text 原始错误透传，NextE 协议 revision 升为 4 并在 OpenAPI 检查中拒绝旧路由。

设备 `237` 最终真实链路的 v2 import 返回 200，渲染日志显示五个 region；RDB 最新文档依次为
`30分後`、学校潜入、`早く帰って‼`、`まぁまぁ`、老师发现五块，均为四点 polygon，不再含 `I` 或
“（脸红）”。原图与最终译图在旧误检位置的 `100x100` screenshot crop SHA-256 完全相同：
`edb1c2b81da36e14b0276cf53e0305b50ac6a791977bf8d938f9fbc52cffb346`。最终 signed app 与
`entry@ohosTest` 构建通过，sidecar builder 从固定上游提交干净应用补丁并生成 nexte2 镜像，设备完整
Hypium 为 226/226，V2 门禁为 0。该阶段关闭已观察到的低信号单字母误检，不把启发式外推为通用准确率；
长图、单双页、快速切页和更广假阳性语料仍保持开放。

2026-07-21：设备 `237` 用合法英语画廊 `4066633` 的 P5 做快速切页和双页验收。旧实现会在
`vm.currentIndex` 变化时递增 UI epoch，sidecar 与 provider 仍完整执行，但 Reader 丢弃完成的衍生页；现改为
只要 route、原图文件和请求 epoch 仍一致，就把结果写入发起页，当前页不同则不弹“已显示”提示，也不替换
当前页。P5 export 将 20 个 OCR 片段整理为 10 个可渲染 block，专用 v2 import 返回 200；请求发出后立即
切到 P6，回填前后的 P6 截图 SHA-256 均为
`ec03c98f0af58b3779c80196c29bec58d80697cdb6d4c4c7b43c5b5051e1d800`。返回 P5 时无需第二次点击，中文
衍生页已经自动显示，随后“显示原图/显示翻译”双向切换通过。双页模式进一步同时显示左侧 P6 英文原图和
右侧 P5 中文衍生页，证明页面替换按 page identity 独立生效，不会把整个 spread 一并替换。十组持久译文中
九组语义和排版可读；`Funny face lol` 仍被直译成“搞笑的脸哈哈”，因此英语口语自然度继续作为质量项，
不为单个样本再次扰动 prompt/cache revision。当前 `entry@ohosTest` 构建成功，设备完整 Hypium 为
226/226。长图候选尚未形成真实高纵横比样本，继续保持开放。

2026-07-21：设备 `237` 已用合法画廊 `4066324` 的 P3 建立首个真实长条漫画样本，源文件与 Reader
身份均为 `720x14804`。第一次请求在 sidecar 完成长图分片检测（11 个 patch、77 个候选）并返回 export
200 后，被 NextE 以 `provider_request_failed` 拒绝；脱敏诊断证明实际故障不是 provider，而是 region 25
的旋转坐标被还原到 `x=-3.917`。固定上游 v1.9.9 的 `TextBlock.to_dict()` 会围绕所有顶点的平均中心反旋转
`lines`；旧适配器先错误使用整体 AABB 中心，修正中心后又旋转整个 AABB，仍会制造不属于任何原始文字行的
外扩角点。协议 revision 5 / `geometry-v2` 现按上游均值中心逐顶点逆变换，再只保存源图空间的紧致 AABB；
sidecar 原始 lines/angle 仍完整保留在 render template，不用内部裁图矩形替代排版几何。合成回归专门覆盖
不均匀多行、旋转和贴边区域；同类错误同时归类为 `output_geometry_invalid`，不再误报模型请求失败。

同一 P3 重跑后，NextE 跨过 export 规范化，已选 `gpt-5.6-luna` 完成 31 个 block，专用
`/translate/import/json/nexte-load-text-v2` 返回 200，并生成 SHA-256
`ccf1320f2760b8ca33d1a8a2062242ebbe5e0d16fc8a612d8222f68abea50bc6`、`720x14804`、7,196,537 bytes
的 PNG。Reader 发布仍停留在 3/8，菜单可在“显示原图/显示译图”间即时切换；持久缓存 metadata 记录
`geometry-v2` profile、源图 hash、Codex source/model 与完整 render identity。该结果关闭“长图无法经过
协议并形成完整视觉产物”的故障，但**没有关闭长图质量验收**：从源身份解析到 ready 约 6 分 35 秒；
乌克兰语被 48px OCR 大量转写成混合拉丁/西里尔近似字，面向日文的 MangaOCR fallback 又产生伪日文；
实图虽有中文回填，但多处字号过大、文字越出气泡或相互重叠，渲染器还报告 polygon union
`TopologyException`。因此 E 的长图项继续保持未完成，下一阶段顺序固定为“语言感知 OCR/profile ->
布局 fit/overflow 质量门 -> 残留原文与语义抽检 -> 长图延迟/资源预算”，不能以本次 200 或完整 PNG
宣称可直接交付。

2026-07-21：语言感知 OCR/profile 已完成第一轮实现和真实验收，但只关闭错误 fallback。样本详情信息行写
`Japanese`，显式 `language:ukrainian` tag 才是正文语言；Gallery 现在优先把显式语言 tag 传至 Reader，
下载/归档本地源切换也保留该值。auto/日文继续使用 48px + MangaOCR hybrid，显式非日语改用独立
`non-japanese-v1-geometry-v2` profile 并禁用 MangaOCR；profile、revision 和缓存身份随配置分离。

设备 `237` 重跑 P3 时 sidecar 仍检测 77 个候选，日志全程只有 `Model48pxOCR`，没有 MangaOCR；已选
`gpt-5.6-luna` 回传后渲染 34 个译文块，产出 SHA-256
`24d19263c14693aa89f9e02ac8ba692ea180c8fec3c137aaadd709533aa15466`、`720x14804`、7,172,846 bytes
的 PNG。缓存 metadata 确认 source language 对应的 non-Japanese profile 和完整 render identity。实际成品比
旧 profile 少了伪日文与超大字号，但仍有漏译、残留原文、错译和局部相压；panel detection sorting 回退、
polygon union `TopologyException` 与位置修正警告仍存在，端到端约 6 分 41 秒。E 的长图项继续未完成，
下一阶段转入 layout fit/overflow 与消字质量门，不能把本阶段解释为可交付成品。

2026-07-22：Torii 首个独立整图 provider 已接入，但没有把 Torii 的 OCR、消字或排版能力拆给现有管线。
设置页先选择“端侧处理”或“Torii 云端整图”：前者继续使用共享 LLM 源和现有逐阶段视觉实现；后者只向
固定的 `POST /api/v2/upload` 发起一次请求，并直接校验、缓存和发布最终 PNG。两条路线只共享
`ComicRenderedPage` 产物边界、Reader 发布 fence 与原图回退，route/provider/context/model/credential
fingerprint 均进入独立 identity，不能跨路线误命中。该边界是后续实现约束：不得为了复用 Torii 的某个
中间能力再把它接回 `ComicRegionRenderBackend`、`ComicTextTranslator` 或 sidecar import/export。

Torii 设置固定 2026-07-22 复核的八个官方 translator ID，并只使用 Torii credits 代付。Torii API Key
使用独立 encrypted-only credential，不读取共享 API/Codex secret；默认端侧路线也不读取 Torii
credential。请求限制源图 20 MiB、multipart 24 MiB、响应
96 MiB、最终 PNG 80 MiB，并校验 MIME、PNG 签名、可解码尺寸、源图同尺寸和 artifact hash。上一页返回的
Torii `context` 只在同 project、目标语言、provider profile、model 和 prompt identity 下传给下一页，属于
可清理的再生缓存，不把未知页面上下文串入请求。

fake transport 与设备测试覆盖官方单端点、无 BYOK 字段、上下文续接、错误尺寸拒绝、设置回退、
identity、持久缓存和失败不写缓存；不会访问 `/ocr`、`/inpaint` 或 `/typeset`。设备 `237` 最终完整 Hypium
为 242/242；测试过程中定位并修复 multipart 合法字段名下划线被本地校验误拒的问题。另一个既有 sidecar
fixture 测试曾因在大图片 multipart 上逐字节反复扫描触发 `THREAD_BLOCK_6S`，改为每请求只解码一次后，
该组由约 17 秒降到 0.694 秒，完整套件恢复稳定。最终 i18n、持久化、secret/backup、V2、`git diff --check`
和 signed app 构建全部通过。该提交当时尚未使用真实 Torii key 产生付费译图，因此只完成了可运行协议、
设置、安全、缓存与 Reader 路由，没有把 fake transport 结果称作真实云端质量验收。

2026-07-22：Torii 两页原创 fixture 已在用户指定设备 `237`、managed billing、
`gemini-3.1-flash-lite` 上完成真实付费验收。固定端点返回的 JSON `image` 实际为
`data:image/jpeg;base64,...`，与官方示例所写 PNG 不一致；NextE 现按声明 MIME 与真实文件签名共同校验
PNG/JPEG/WebP，并在设备上把 JPEG/WebP 有界归一化为同尺寸 PNG 后才进入统一 `ComicRenderedPage`
契约。两页最终产物合计 3.8 MiB，一次新鲜完整运行 13.0 秒；第 1 页与第 2 页均保持竖排中文，上一页
`context` 已实际发送到下一页。首轮成功输出两页都使用“优”，但后续新鲜复跑的第二页变成“小优”；核心
人名没有换字，严格称呼形式却仍不稳定。站牌和部分拟声词也有语义/视觉瑕疵。这次通过只关闭 Torii
接口、出图、上下文传递与缓存边界，不外推为跨页术语或通用画质达标。

同一配置的一次新鲜两页运行从 6021.55 降到 6019.13，实际消耗 2.42 credits；整轮响应契约诊断和验收从
6030.00 降到 6016.71，共消耗 13.29 credits，其中包含修复前被客户端误拒的成功 JPEG 响应及一次用于定位
强制刷新语义的重复运行。评测身份随后改为稳定 fixture project，并取消无条件 force refresh；杀进程后
复跑分别为 28 ms、25 ms，余额在复跑前后都为 6016.71，证明命中本地衍生页缓存且没有再次调用 provider。
结果页安全区也已实机复核，标题、运行摘要和第一页内容不再互相遮挡。最终 signed app、显式 signed
`entry@ohosTest`、设备完整 Hypium 250/250、i18n、持久化、secret/backup、AppStorageV2、V2 inventory
与 `git diff --check` 全部通过。D2 的首个真实 provider 验收关闭；端侧 B2/F 与长图质量门仍保持开放，
不能因此宣称整个端侧视觉 Reader V1 完成。

2026-07-22：默认端侧路线已在用户指定设备 `237` 的真实 Reader 与实际日文画廊完成生产链路验收。
设置只选择共享 Codex 源 `gpt-5.6-luna`，没有配置或启动 sidecar；第 1 页由系统 OCR 提取 10 个区域，
第 2 页提取 7 个区域，随后均经过逐块上下文翻译、本地字形处理、纵横排版、PNG 编码和 Reader 发布。
第 2 页首次完整运行约 16.6 秒，其中包含远端 LLM；已存在翻译文档时，第 1 页只重做端侧渲染约 1.8 秒。
同页原图与译图、切到第 2 页再返回、双击缩放和拖动平移均在 Reader 内实机检查，译文随衍生图片缩放，
不是独立浮层。单双页模式下的原图/译图切换也已检查；Reader 现在会在衍生路径或可见状态变化时通知对应
spread 数据源，避免状态已切换但双页仍保留旧图片。

首次真实端侧出图暴露本地后端把 PNG 写入临时自定义目录，因不符合 repository artifact 契约而被正确拒绝。
本地后端现与整图路线统一写入受控 `comic-translated-pages/<identity>-<artifact>.png`，落盘前计算内容 hash，
非法 artifact 路径独立归类为 `render_artifact_invalid`。最终 v18 增大纵排右侧原文处理范围，清除了第 2 页
中央叙述框残留；纵排译文只把误置于开头的句末标点移回末尾，避免第 1 页气泡顶部孤立 `？！`。进程重启
后再次显式翻译第 1 页，从 source identity 解析到 `reader_page_ready` 为 8 ms，日志为 `cache=1`，期间没有
OCR、视觉后端或 provider 调用。

F 至此关闭，但 B2 和整体 V1 仍未关闭：第 2 页小气泡 `え?` 仍因系统 OCR 漏检保留原文，漫画专用
detector/OCR、内容感知修复、扩展质量集与长图资源/质量门仍是正式完成条件。当前结果证明默认端侧工作流
已经真实衔接到 Reader，并不等于端侧制图对通用漫画已经可用。最终 signed app 与 signed
`entry@ohosTest` 构建通过；设备完整 Hypium 为 251/251，持久化、secret/backup、AppStorageV2、V2 inventory
与 `git diff --check` 全部通过。

2026-07-22：生产端侧 analyzer/render 升至 v14/v19。真实日文 Reader 中，同一气泡被 Core Vision 拆开的
相邻纵列和横行会先合并，再按整个气泡空间统一拟合字号；第 1 页 `な、なんで` 与后续正文、横排叙述不再
分块相压，第 2 页 `はい。` 与 `洗うから後ろ向いて。` 不再出现同气泡字号跳变。原文处理仍限制在原文字形
附近，扩大的排版矩形不会退化成整块圆角遮盖。设备截图已进入端侧质量基线。`え?` 漏检和一处注音残留
仍存在，不能以 v19 关闭 B2。

同阶段完成首个 detector 移植 spike：从 Docker 对照链路提取 10,838,944-byte 的
`ysgyolo_1.2_OS1.0.onnx`，转为 ncnn param/bin，并在现有 HarmonyOS NDK 运行时增加异步 NAPI 与类型化
ArkTS 边界。CPU FP32 相对 ONNX Runtime 最大绝对误差 0.001312；ncnn 默认 FP16 storage 会产生不可接受
偏差，因此首个 profile 明确禁用 FP16/packing。设备 `237` 在 1024 × 1536 原图上返回 5 个去重区域，模型
加载 39 ms、首次推理 207 ms、热推理 160 ms，带临时模型 fixture 的完整 Hypium 为 253/253。

模型二进制没有进入本提交，production Reader 也没有切到该 detector。B2 下一边界是：在独立 model-pack
补齐来源、hash、license、下载/校验/清理后，将系统 OCR 行归属到 detector OBB，并验证缺包、推理失败和
不支持设备时无损回退 v14；之后再选择 OCR/inpaint 模型，而不是把 YOLO 的区域输出误写成完整制图能力。
移除临时模型资源后的最终 signed app、signed `entry@ohosTest` 构建通过，设备 `237` 完整 Hypium 为
252/252，V2 inventory 为 0，证明待提交代码与模型试验 fixture 已解耦。

2026-07-22 后续交付：YSGYolo 已以 `model-pack-v1.1.2` 独立发布，模型卡的 MIT 声明与 checkpoint 内嵌
`AGPL-3.0` 冲突按更保守的 `AGPL-3.0-only` 处理；NextE MIT 代码与可选模型资产分别标许可。设置页只增加
一条模型状态入口，按需下载并校验 10,747,791 bytes。production profile 在模型可用时只用 OBB 归并系统
OCR 行，保留原 OCR polygon 作为遮盖/排版依据；缺包、长图越界或推理失败均回退 v14。此前“尚未发布、
尚未接入”的记录由本段取代，下一边界转为漫画 OCR、长图 detector 分块和内容感知 inpaint。

发布闭环已实际完成：NextE-Models `model-pack-v1.1.2` 的 tag 解引用到
`9f57c3995aa83518f43db48a64cdd560b4fc83c4`，Actions run `29893521705` 完成验证、构建、资产收集与不可变
Release 创建，共发布 29 个资产。设备 `237` 用最终 signed app 从该 Release 下载 ncnn param/bin；日志从
`download_start` 到通过大小与 SHA-256 校验后的 `download_success` 为约 5.4 秒，合计 10,747,791 bytes，
设置页状态随后显示 `YSGYolo`。最终 signed app 构建、设备完整 Hypium 254/254、V2 inventory 0 与
`git diff --check` 通过。该证据只关闭 detector 的发布、安装与 production 接入，不关闭 OCR、inpaint 或
通用视觉质量。

2026-07-22 OCR 组件交付：官方 `PP-OCRv5_mobile_rec` 固定到 source commit
`682f20538d8c086cb2128e5cfac775e6c4904e85`，转为 ncnn param/bin/dictionary 并以 Apache-2.0 记录；它与
`AGPL-3.0-only` 的 YSGYolo 可以同处一个按需模型包，但资产来源、hash 和许可证保持逐组件对应。首次
`model-pack-v1.1.3` CI 因 `paddle2onnx` 未声明 `packaging` 依赖失败，保留失败 tag；修复后发布新的
`model-pack-v1.1.4`，Actions run `29898205012` 成功，tag 解引用到
`312bbe09df402b9615b2ffbc150115d2e5a7284a`。Release 三份 OCR 资产已重新下载并逐个通过大小/SHA-256 验收。

设备 `237` 的直接 NAPI 测试中，模型加载 53 ms，三个区域推理 162/46/47 ms。production 只对 detector 已
发现且 Core Vision 完全未覆盖的区域调用 recognizer，接受阈值为 0.85，不覆盖已有 OCR；正确但仅 0.736 的
纵排候选也会被拒绝。该边界关闭模型选择、转换、发布和补充接线，不关闭拟声词、艺术字、长图、文字 mask
或 inpaint。设置页仍只保留一条“端侧漫画模型”，许可证放在卡片末尾，不增加 provider 层级或多余副标题。

最终生产验收中，设备 `237` 已从设置页安装 `model-pack-v1.1.4`，五份资产逐文件通过大小和 SHA-256 后，
同一行状态显示 `YSGYolo + PP-OCRv5`；已有 YSGYolo 文件可复用，只增量下载缺少的 recognizer 文件。
最终 signed app 与 signed `entry@ohosTest` 构建通过，`ComicLocalVisualBackend` 6/6，设备完整 Hypium
256/256，V2 inventory 为 0。测试模块因 HarmonyOS 数据沙箱不能读取正式应用模型目录，未为此增加跨沙箱
权限或降低 production 0.85 阈值；正式资产证据来自生产下载校验与同设备直接 NAPI 基准。

2026-07-22 长图 detector 交付：旧 16 MP 总像素门禁已替换为 2048 边长、256 px 重叠分块，最多 64 块；
每块 OBB 映射回原图后跨块去重，任一块失败则整页 detector 回退，避免发布部分检测结果。生产 profile 更新为
`ysgyolo-1.2-os1-ncnn-fp32-tiled-v2` / revision 2。设备 `237` 的 `720 × 4608` 离线 fixture 验证三块边界、
重叠去重和最大 Y=3834 的坐标回映；`720 × 14804` 合成长图真实运行已发布 YSGYolo，共 9 块，模型加载
48 ms、总推理 1731 ms。合成页没有文字，这一阶段只关闭长图 detector 的资源/坐标边界，不关闭真实长图
recall、PP-OCR 补充覆盖、排版 overflow 或内容感知 inpaint。

2026-07-22 Torii 自托管决策：设备 `237` 使用同一 OpenAI-compatible 源完成真实整页矩阵。该源可查询
39 个模型，但 `deepseek-chat` 拒绝 `response_format`，`gpt-5-mini` 拒绝 `usage`，
`gpt-5-chat-latest` 拒绝 `reasoning, usage`；`gemini-2.5-flash-lite` 才成功返回 200 和同尺寸译图。
“模型可列出”因此不能作为 Torii 自托管兼容性证明，且 NextE 无法修改 Torii 发给上游的请求体。本计划
移除 BYOK/自托管设置、运行时凭据转发与 `x-byok-*` multipart 字段，旧配置加载时删除并回到 Torii
credits 代付。该决定取代本文此前所有 BYOK 产品接入描述；外部 API 具备 BYOK 能力不构成恢复入口的
理由，重新评估必须另立产品决策并拥有稳定预检契约和设备兼容矩阵。

2026-07-23 端侧真实英文页复核：此前输出问题不是“整条本地路线不可接入”，而是 source geometry 与 target
layout 混用，并在渲染时同时保留段落大块和内部 OCR 小块。生产 analyzer/render 升至 v19/v31：detector
geometry 在分组前生效，相邻源纵列可合并；拉丁字母目标统一横排，中文/日文目标仍可纵排；被同一段落包含
的重复小块在渲染时抑制，明显分开的左右段落不得合并。白色描边按字号 20%、2–10 px 两遍绘制，避免复杂
灰阶背景上的固定细边失效。对应确定性用例覆盖目标语种方向、相邻 detector 列合并和描边比例。

设备 `237` 的同一真实页从 v27 逐字竖排、v28 重叠、v29 过度合并推进到 v31。v31 直接产物
`a1ceffbc38d3…` 已能稳定显示横排段落和同量级白描边，完整本地处理约 13.0 秒；但段落仍压人物、贴页边，
页脚和原拟声词有残留，所以 B2 与整体 V1 保持打开。v30 完成时曾在系统 Toast 路径产生
`THREAD_BLOCK_6S`，Reader 已删除成功 Toast；v31 ready 后仍可响应控件且进程存活，后续仍需连续页峰值
PSS/appfreeze 验证。下一质量边界是区域内容归属和可用排版空间，不再通过任意扩大矩形或缩小描边掩盖。

2026-07-23 Torii 云端字体真机验收：设备 `237`、同一画廊同一第 1 页、
`gemini-3.1-flash-lite` 与英文目标不变，仅切换 WildWords / NotoSans。两次均走
`whole_page_render`，分别产生独立 provider identity `487f79d3687d62c3` / `ca34269618752cbd` 和独立
artifact hash `9cf45dcd9a60…` / `2ba2560de3c8…`；直接产物可见 WildWords 为漫画手写全大写，NotoSans
为常规无衬线混合大小写。余额依次从 5954.69 降至 5953.23、再降至 5951.79，对应 1.46 / 1.44 credits。
该证据关闭 Torii 字体参数传递、缓存失效和真实渲染差异验收。端侧路线按产品决策固定使用系统默认字体，
不实现 Torii 字体选择，也不以端侧字体与云端字形一致作为 B2/F 的完成条件。

2026-07-23 端侧 v32 区域归属修复：v31 的包含块抑制只按 geometry 判重，会静默删除位于大段落内部但
语义独立的 OCR 小块。v32 保持“一个区域只绘制一次”，但先比较规范化译文：大块已经表达的短语继续抑制，
独有译文合入 owner 后再停止子块绘制；拉丁短语使用 token 边界，避免把 `Very` 误判成 `every` 的子串。
横排目标加入安全页边距，左右区域只向物理页边扩展，不越过各自 source geometry 的内边界。对应设备用例
覆盖重复/独有嵌套块和左右页边约束，`ComicLocalVisualBackend` 为 12/12。

设备 `237` 上先在设置中确认 Image processing 为 On-device，再复用同一真实日文画廊第 1 页的英文文档，
没有触发 Torii。Reader 真实输出恢复 `Very`、`As a rule, condoms...`、`Who could “that guy” be...?` 和
右侧压力段落，左下段落不再贴住物理页边；signed app、signed `entry@ohosTest` 和 V2 inventory 均通过。
该结果只关闭“包含块丢译文”和“横排贴页边”回归，B2/V1 仍打开：短句在原拟声词附近拥挤，页脚与艺术字
仍有残留。下一边界是把“译文绘制 owner”与“全部成员 source polygon 的原文清理”分离，而不是继续放大
矩形、换端侧字体或重新调用 LLM。

2026-07-23 端侧 v33 分析边界修正：继续追踪后确认父段落的原文 mask 已经扫描其内部字形，另做“所有子块
polygon 清理”不会解决短句堆叠，所以上一段的暂定下一边界被证据否定。真正问题是独立翻译已经发生后，
渲染层才尝试把子句拼回父段落。生产 analyzer/render 升至 v20/v33：同一 OCR tile 内，被明显更大纵排
段落包含的窄纵列会在分析阶段把唯一原文并入父段落并停止发布子块，LLM 因而只翻译一次完整段落；旧文档
的译文 owner 判重仅作为兼容保护。规则限定为纵排、同 tile、父块文本至少 2 倍、面积至少 3 倍、宽度至少
2.4 倍且包含率至少 90%，避免重新引入 v29 的左右段落误合并。

设备 `237` 对同一真实第 1 页重新执行端侧识别、翻译和制图。v32 左侧 `Very` / `As a rule` / `Who...`
独立短句在 v33 中不再堆叠，右侧仍包含此前子列补回的“每天用于缓解压力”语义；目标设备回归为 12/12，
signed app、signed `entry@ohosTest` 和 V2 inventory 均通过。该阶段关闭嵌套 OCR 的重复翻译/排版问题，
但 B2/V1 继续打开：下一质量边界是长段落的可用空间与拟声词/人物避让、页脚和艺术字检测，以及连续页
峰值内存，不再回到渲染后拼接子句或扩大遮盖矩形。

2026-07-23 端侧 v36 宽段落空间修正：真实页复核发现 360 px 原本既是扩展上限，也会错误缩窄已经约
435 px 宽的源段落；直接放宽后又会让单块布局选择更大字号，v34/v35 因此均作为视觉退化候选拒绝。最终
规则保留真实 source paragraph 宽度，但页面统一字号上限和宽段落自身字号都以 360 px 参考宽度拟合，新增
空间只能减少换行，不能放大文字。生产 render/layout 升至 v36/v31，analyzer 仍为 v20。

设备 `237` 复用同一真实英文文档重排：左上段落恢复到接近源宽，行数和垂直占用低于 v33，右侧与左下
段落未回退；新增宽源段落用例后 `ComicLocalVisualBackend` 为 13/13，signed app、signed
`entry@ohosTest` 和 V2 inventory 均通过。本阶段关闭“宽源段落被扩展上限反向压窄”和“放宽后字号变大”
回归，B2/V1 仍打开：下一步必须针对拟声词/人物等画面障碍做内容感知布局，并继续处理页脚/艺术字和连续
页资源门，不能再以无约束扩矩形代替障碍检测。

2026-07-23 端侧 v37 有限障碍感知布局：审计确认 YSGYolo 五类仅表示文本/气泡 region，不提供人物或画面
分割，不能把 detector 类别包装成语义避让。渲染器改在全部原文清理完成后，对“源纵排、目标拉丁横排、
至少四行且有足够垂直余量”的块读取局部 PixelMap，按实际行宽和描边比较上/中/下三个候选文字包围盒的
高对比边缘密度；只有相对居中改善至少 16% 且超过绝对门槛才改变锚点，读取/评分失败回退居中。

设备 `237` 的合成密集障碍用例将译文像素从居中 `y=262–506` 移至顶部 `y=195–440`，留出障碍间距；同一
真实英文页中只有左上长段落上移并与下方拟声词拉开，右侧和左下保持 v36。生产 render/layout 升至
v37/v32，analyzer 保持 v20；目标回归为 14/14，signed app、signed `entry@ohosTest` 和 V2 inventory
通过。该阶段只关闭“有明确垂直余量时仍固定居中压住紧邻高边缘内容”，B2/V1 仍打开：人物/气泡语义
分割、二维候选区域、页脚/艺术字和连续页资源门仍需独立完成。

2026-07-23 端侧 v21 非对白页脚过滤：同一真实页的 v20 文档有 7 个块，其中 4 个是正文，另外 3 个为
`儿/=门…`、Patreon/Enty/Pixiv 署名和 Twitter/Fantia 署名。v21 只在页面底部 6% 内，对横排明确网址/平台
署名或不超过 12 字符的符号主导碎片做分析时过滤；中部网址和底部自然对白均有反例回归，不能扩大为“删除
底部文字”。过滤发生在 LLM 和渲染前，原图署名像素保持不变。

设备 `237` 重新执行分析、翻译和制图后，持久文档为 4 个正文块，并记录
`local_non_dialogue_footer_filtered`、数量 3；干净 Reader 截图确认正文仍按 v37 排版，页脚没有被清理或
覆盖。目标回归为 15/15，signed app、signed `entry@ohosTest` 和 V2 inventory 通过。该阶段关闭当前真实
样本的页脚署名误译，不关闭 B2/V1：其他位置的艺术字/拟声词、人物与气泡语义分割、二维候选区域和连续页
资源门仍需继续完成。

2026-07-23 端侧 v28 资源边界：真实页追踪确认 v21 的主要风险不是 PixelMap/file handle 未关闭，而是大区域
AOT、固定 1024 CTD text mask 的 ncnn scratch 留存，以及译图 ready 后又进入 Reader 2× 超分。AOT 现按
上游 256 输入语义保持比例缩放并恢复到源区域；detector、CTD 和 AOT Extractor 使用每次推理独立的 mmap
pool，结束后统一解除映射；`comic-translated:` 衍生页跳过二次超分，普通原图超分行为不变。

设备 `237` 同一画廊第 1 页从 v21 ready 后约 1.84 GiB、v25 仅缩小 AOT 后约 990 MiB，降到 v28 峰值
917,833 kB、完成后约 776,144 kB。连续第 2 页从翻译前约 815,509 kB 经 992,035 kB 峰值回落到
716,860 kB，未逐页累加；两页日志均无译图 `reader-super-resolution.process_start`。第 1 页三个 AOT 合计
约 1.15 秒，第 2 页一个 AOT 为 351 ms。signed app、V2 inventory 与设备 Hypium 267/267 通过。

这一阶段只关闭当前普通页样本的明显 scratch 留存、AOT 原尺寸推理和译图重复超分，不关闭 B2/F。第 2 页
真实截图仍显示多个聊天气泡被合成一个长译文块并跨气泡重叠；下一工作项明确转为多气泡 region 分组与布局，
之后再扩展长图、低内存失败回退和连续多页 P50/P95，不能把资源结果表述为端侧制图完成。

2026-07-23 端侧 v24/v38 多气泡边界：第 2 页后续日志证明旧分析器先形成两个多行段落，再拿已增长的段落
高度计算下一次允许行距，50 px 的独立气泡间隔因此被传递合并；旧渲染器还会在翻译完成后重新抑制或合并
plan。分析器现为每个块保存原始 OCR 行高/列宽尺度，任何后续合并都不能放大该尺度；同段行框允许有限轻微
重叠。渲染器则严格按持久文档一 block 一 plan，不再承担语义分组。

设备 `237` 的同一 `1280 × 1780` P2 上，14 个 detector/OCR 行最终稳定为 5 个 block；第二、第三长气泡
geometry 分别为 `y=648–819` 与 `y=869–988`，保留 50 px 间隔，渲染日志为
`documentBlocks=5 plans=5`。完成图 808,123 bytes、artifact `c8c663f311c5…`，五份英文译文均未跨入相邻
气泡。新增“两个已合并段落不得再次跨气泡合并”和“轻微重叠同行框仍应合并”回归后，signed app、signed
`entry@ohosTest`、V2 inventory 和设备完整 Hypium 270/270 通过。

该阶段关闭当前真实 P2 的跨气泡传递合并与渲染后重分组问题，不关闭 B2/F：短气泡内边距仍接近轮廓，
下一质量项转为气泡形状感知的安全排版边界，同时继续保留艺术字/拟声词、复杂背景修复、更多真实样本和
连续页性能门。

2026-07-23 端侧 v39 闭合气泡安全区：原文处理完成后，横排布局从 OCR 原始区域在有界邻域内搜索与气泡
底色相近的闭合连通域；只有连通域不触碰候选边缘且覆盖尺度充分时，才裁掉稀疏尾部并内缩为安全排版矩形。
开放式、复杂纹理、深色或读取异常均保留原 geometry fallback。同一解析矩形同时用于字号拟合、有限障碍
偏移和最终绘制，禁止三个阶段各自重算。

设备 `237` 的同一 P2 五个 block 全部命中安全区。右侧短气泡从 fallback `602,1039–890,1126` 收敛到
`642,1043–855,1126`，最小气泡收敛到 `465,1144–599,1188`；五份英文译文均位于轮廓内且无重叠。
CTD 为 648 ms，五个 AOT 为 238/343/347/289/282 ms，最终图片 806,706 bytes、artifact
`c659d88ca744…`。新增闭合气泡回归后，signed app、signed `entry@ohosTest`、V2 inventory 与设备完整
Hypium 271/271 通过。

该阶段关闭当前 P2 的普通闭合气泡贴边问题，不关闭 B2/F。下一轮真实页验证必须覆盖开放尾部、不规则轮廓、
深色/纹理气泡和艺术字；不确定轮廓继续 fail-open，不能为了扩大空间裁掉译文或把气泡外画面误当内部。

2026-07-23 端侧 v40 窄竖框拉丁布局：真实 P3 为 1280 × 905 黑底页，三个八边形色块均为窄高竖排源块。
旧 v39 普通横排会把 `Whenever` 等英文拆成单字母，灰色块出现词内断行，粉色块则因未确认安全区而使用
过宽 fallback 并跨出色块。v40 对源 `vertical-rl`、目标横排且高宽比至少 1.8 的矩形先交换逻辑宽高完成
整段横排，再围绕物理中心顺时针旋转 90°；闭合安全区继续沿用，未确认块则保持原 OCR 矩形大小并平移到
既有页边 fallback 的 8 px 内边界。旋转段落跳过横排障碍上下偏移，避免两个坐标空间重复移动。

设备 `237` 的 v40 实际译图为 136,656 bytes、artifact `62b1460cce966300…`；旧 v39 对照为
151,147 bytes、artifact `4ac43aa038300414…`。三段英文均沿各自竖向长轴保留完整单词，无跨块重叠、色块
外溢或页边接触。新增 `uncharacteristically` 窄竖框确定性回归，并保留既有物理页边回归；signed app、
signed `entry@ohosTest` 与设备完整 Hypium 272/272 通过。

该阶段关闭当前 P3 的窄竖拉丁框拆词和越界回归，不关闭 B2/F。整段 90° 侧转是轻量阅读折衷，不宣称
专业英文排字；下一轮仍需覆盖开放尾部、曲线轮廓、人物/纹理干扰和艺术字，不能把这三个规则色块外推为
通用漫画制图完成。

2026-07-23 端侧 v42 同气泡多列布局：真实 P4 证明 analyzer 仍可能把一个闭合不规则气泡中的多个纵列保留
为多个合法 block；v40 对每个 block 单独拟合后，会在同一气泡内产生不同字号、译文相压和明显空间浪费。
这不授权渲染器按矩形邻接重做语义分组。v42 继续让 document、blockId、translation batch 和源文字清理
保持一对一，只在像素级 `shapeConstrained` 闭合安全区能独立覆盖组内全部源块中心时，按既有 reading order
把译文作为一个视觉组统一拟合、绘制；没有单一确认边界的传递组保持原样。

设备 `237` 的真实 P4 日志为 `documentBlocks=24 plans=24`、`groups=17 merged=7`、`cache=0`，最终 PNG
691,220 bytes、artifact `1204916145c8…`。中央大粉气泡、顶部灰气泡、右侧窄气泡和底部粉气泡均只排版
一次，独立气泡未观察到误合并；新增 fixture 明确保留两个 analysis block，再验证渲染收敛为一个组且像素
不越出闭合气泡。目标设备用例 21/21，完整 Hypium 273/273，signed app、signed `entry@ohosTest`、V2
inventory 和 `git diff --check` 均通过。

该阶段关闭当前真实样本的同气泡重复排版，不关闭 B2/F：无法确认闭合边界时仍会保守回退，艺术字/拟声词、
复杂背景修复、更广样本 overflow 与连续多页 P50/P95 继续保持开放。后续不得把视觉组写回语义文档，也不得
用普通矩形距离替代闭合边界证据。

2026-07-23 端侧 v43 窄竖拉丁框直立优先：v40 的整段侧转保留为防拆词保底，但不再无条件应用。渲染器会
同时拟合物理框内的正常横排；仅当每个拉丁单词完整、字号至少 10 px，且达到侧转候选字号 75% 时采用直立
结果。设备 `237` 上新增短句直立与长单词侧转两条相反回归，`ComicLocalVisualBackend` 为 22/22。真实 P3
中两个可读色块恢复正常方向，最窄长句仍侧转；P4 多个中宽气泡恢复正常方向，未新增拆词、重叠或越界。
最终 signed app、signed `entry@ohosTest`、V2 inventory、`git diff --check` 与完整 Hypium 274/274 通过。

该阶段不改变 analyzer v24、语义文档、LLM 译文或源文字清理，生产身份仅升级为
`reader-local-bubble-layout-v43` / `local-bubble-typography-v37`、render revision 29。后续若要减少剩余侧转，
必须从可靠边界空间或翻译长度入手，不得降低可读字号门槛或恢复词内断行。

2026-07-23 端侧 backend v29 艺术字候选与局部失败隔离：真实 P5 证明 YSGYolo 的 27 个区域没有覆盖两处
画面手写字。CTD 现提供 0.3 擦字 mask 与 0.15 proposal-only 两个输出；低阈值候选必须再通过 PP-OCRv5
整块/半块或内部放大 Core Vision，不能直接擦字或送入 LLM。右侧 `732,488–876,652` 最终由放大系统 OCR
补回 3 字，左侧沿用已有 OCR，页边 `LIVE` 与中央噪声被拒绝。相邻纵列允许 detector box 约三分之一字宽
交叠后，P5 从 22 个旧 document block 收敛为 16 个完整气泡块。

渲染改为先在原图上解析气泡安全区并预排版，确认可绘制后才擦除；不可排版的单个视觉组保留原文，不能再
中止或预先擦空整页。英文布局先缩字号保证完整单词，再换行。设备 `237` 最终 P5 为 16/16 组、P4 为
18/18 视觉组、P3 为 3/3 组，三页 `skipped=0` 且均 ready；真实截图保存在
`.hvigor/outputs/aot-reader-production/nexte-v59-p3-final.jpeg`、`nexte-v59-p4-final.jpeg`、
`nexte-v59-p5-final.jpeg`。目标套件 23 个确定性用例通过；端点无关 Core Vision 测量仍因固定 5 秒上限
偶发 timeout；完整 Hypium 为 276 项、275 项通过、同一测量超时，不能写成全绿。

2026-07-23 端侧 CTD 双阈值复用：`ctd-seg-1024-ncnn-fp16-tiled-v3` 只做一次模型 forward，再从同一恢复输出
生成 0.15 proposal mask 和 0.3 treatment mask。高阈值结果按源图 hash/尺寸进入最多 2 项、合计 16 MiB 的
一次性内存缓存；render 命中即删除，缓存缺失仍执行原 0.3 路径。设备 `237` 的新鲜 P5 只记录一次 CTD：
71521/64137 像素、846 ms，随后存入并命中 1,158,400-byte mask，没有旧版第二次 692 ms 推理；16/16 视觉组
仍完成，手写字补漏与失败隔离没有回退。证据为 `nexte-v60-p5-final.jpeg` 和
`nexte-log-20260723-093255.txt`。目标套件 24 个确定性用例通过；Core Vision 实测仍因固定 5 秒上限偶发
timeout；完整 Hypium 为 277 项、276 项通过、同一测量超时。

下一项转为扩大艺术字/拟声词合法样本和误检率统计，并评估 AOT 批处理与连续页资源；不得降低 OCR 安全
阈值，也不得让低阈值 mask 直接决定擦除。

2026-07-23 端侧 backend v30 生产模型基准：新增显式 opt-in 设备用例，直接运行已安装的 YSGYolo、
PP-OCRv5、CTD 和 Core Vision，而不是继续用单页截图主观调参。两张原创 1024 × 1536 fixture 稳定命中
9/11，两个缺口均为艺术化拟声词；一处画面曾被系统裁片误识别为 `O ラ 00O`。v30 对短小混合脚本转录
fail-closed，并让占用不足 64 个 CTD 网格的稀疏碎片完全跳过补充 OCR，误识别由 1 降为 0，未识别内容保留
原图。两页端侧分析从 18.486 s + 8.958 s 降为 4.992 s + 7.824 s，总计约 27.4 s 降到 12.8 s；补充
PP-OCR 候选由 5/2 收敛为 1/1。一次整页解码缓存和 RGBA 直连均实测无收益并撤回，没有带入生产复杂度。

最终 `ComicLocalVisualBackend` 在 60 秒测量上限下为 27/27，生产组合基准为 1/1、9/11 且
unexpected=0；signed app、signed `entry@ohosTest` 和 V2 inventory 通过。默认 5 秒上限的完整 Hypium
为 280 项、279 项通过，唯一 Error 是同一个端点无关 Core Vision 实测在 5.76 秒返回，不是断言失败。
下一项不再调整 CTD/OCR 启发式，转为漫画专用 recognizer 候选与扩大样本集；任何新模型必须同时提高
拟声词 recall、保持误擦为 0，并单独报告设备延迟和内存。

2026-07-23 recognizer 候选阶段已完成首轮否决性验证。48px CTC 在精确 CTD 裁片上正确识别第 2 页
`パラ…`，但第 1 页仍错为 `ザ7？`，真实 P5 候选为空；固定宽 ncnn FP16 约 82 MiB、桌面约 45–50 ms，
动态宽转换数值不等价，且 release 没有独立模型许可证。Apache-2.0 的 `manga-ocr-base` 对普通正文较好，
但 5/5 画面负样本都生成非空文字，真实 P5 也误读。Core Vision 精确裁片分别返回 `N 000` 与
`O ラ 00O`，证明放宽页边重试不能解决缺口。三条候选均不接入 production；v30 与 9/11、unexpected=0
基线保持冻结。下一项是扩大合法艺术字/负样本集并寻找或训练可明确分发、可校准拒绝的 recognizer，
不是继续调阈值或为当前两个模型增加 UI/下载入口。

2026-07-23 LaMa large 端侧阶段完成否决性验证，不进入 production。Docker 的
`lamalarge.onnx` 为 207,482,655 bytes，SHA-256
`107c8306ac1d27c83638d6535846986542dfe2707f1498b1ac9be25b4a963864`；固定 256px ncnn 图与原 ONNX
数值一致，MAE `1.2777e-7`、最大误差 `4.2617e-6`。三张带真值的自生成 A/B 中，LaMa 在半调线稿和雨纹
背景上分别比 AOT 高 7.96 dB 与 4.59 dB，平坦纸张则低 2.78 dB，证明合理方向若资源允许也应是有条件混合，
不是全面替换。

设备 `237` 给出决定性资源门：LaMa ncnn FP32 冷/热单区域为 23.968/23.759 秒，复测
23.780/24.212 秒；测试进程总 PSS 样点最高 1,142,558 kB，模型 bin 约 195 MiB。现有 AOT 真实区域为
238–468 ms，因此当前导出约慢 50–100 倍。动态 shape 转换也在 `pnnx pass_ncnn` 崩溃。实验 NAPI/profile
分支、测试下载模型和设备临时文件均已撤回或清理；该候选阶段保持 AOT v28，不得增加 LaMa 设置项、隐藏开关或
模型发布。下一项回到 AOT 重叠区域合并/批处理与连续页 P50/P95；只有更小移动模型或更合适运行时同时通过
画质、峰值 PSS、热稳定性、失败回退和许可门，才允许重开生成式端侧修复候选。

2026-07-23 AOT 同气泡分组阶段完成。source treatment 升至 v29，但没有扩大语义分组：只有 v42 已用同一
闭合气泡边界确认的多个 drawable block 才共享一次 AOT。实现对各 block 的 treatment 区域取 crop 并集，
对 mask、膨胀和 fallback 填色取区域并集；每个 block 仍须单独命中 CTD mask。合并 crop 最长边若超过最大
单块的 1.35 倍，自动回退逐块修复，避免降低 256px AOT 的有效分辨率。确定性设备用例证明同气泡两列从
2 次 AOT 降为 1 次，邻接但独立的两个气泡仍为 2 次。

设备 `237` 使用 `model-pack-v1.1.6` 发布权重完成真实 NAPI 回归：
`blocks=2 region=239x454 inference=136x256 load=114ms inference=208ms`；变化像素没有越出闭合气泡
`x=430–590, y=320–740`。含临时模型下载的 opt-in `ComicLocalVisualBackend` 在 60 秒测量上限下
28/28 通过，模型文件随后从 test cache 清理；最终完整设备 Hypium 为 281/281。该阶段只消除已确认同气泡
多列的一次重复 AOT，不合并独立气泡，也不关闭连续多页 P50/P95、峰值 PSS、复杂纹理或艺术字质量门。
下一项转为连续页 AOT/CTD 性能与热稳定性测量。

2026-07-23 原生连续压力阶段完成。新增最多 20 轮的 opt-in 基准，在同一 ohosTest 进程依次保持 AOT 与
CTD 模型驻留，并记录推理 P50/P95、逐轮 PSS 和系统热档位。设备 `237` 最终候选为：AOT
`261/428 ms`、PSS `437,720 -> peak 449,986 -> 410,757 kB`、热档位 1；CTD
`643/972 ms`、PSS `540,646 -> peak 561,721 -> 454,164 kB`、热档位 2。

对照确认旧 CTD external ArrayBuffer 在 20 轮从 514,482 kB 持续升到 582,502 kB 且没有回收。HarmonyOS
不导出 `napi_adjust_external_memory`，因此 CTD 两张全页 mask 改为 VM 管理 ArrayBuffer；同字节输出只多
一次有界复制，不改变视觉 profile 或缓存身份。新实现于第 12/18 轮发生回收，两个原生集成用例 2/2、
最终完整 Hypium 281/281 通过。本阶段关闭原生 mask 输出未计入 VM 压力的资源子门；真实 Reader 连续页
仍需覆盖解码、Core Vision、LLM、PNG 编码、缓存和长图，B2/F 对应检查项保持未完成。

2026-07-23 完整端侧视觉基准已覆盖 YSGYolo、Core Vision、PP-OCRv5、CTD proposal/treatment、AOT、
排版、PNG 编码和衍生页写盘，不包含网络与 LLM。设备 `237` 在 4 线程、两张 1024×1536 固定页各 3 轮下，
普通页分析 P50/P95 为 4.253/6.673 秒，渲染为 1.947/2.347 秒；PSS 只记录阶段终点，热档位由 0 升至 1。
同进程最后的受控 1024×3072 拼接长图在持续热负载下为 40.380/43.752 秒，较轻单轮重跑为
17.793/4.643 秒，瓶颈主要落在 CTD proposal、补充 OCR 和 AOT。视觉拉回没有发现长图接缝、粗暴矩形
覆盖、方向或同气泡字号回退。2 线程对照把普通页分析 P50/P95 拉到 8.202/9.148 秒、渲染拉到
3.221/3.822 秒，故不改生产 4 线程默认值；当前 Reader 只有一个受 `comicTranslationRunning` 保护的
当前页任务，也没有以人工多页压力为由新增调度。普通页完整视觉阶段与受控长图子门已关闭；真实 webtoon、
Reader 连续翻页、运行中峰值 PSS、LLM/缓存和长期热稳定性仍属于 B2/F 未完成项。

2026-07-23 Reader 自动翻译入口采用会话级同级菜单项，不新增设置页层级：保留“翻译当前页”，其后以
勾选态显示“自动翻译”。开关默认关闭，退出 Reader 或切换画廊即复位；开启前提示额度与图片上传影响。
调度保持单任务，当前页完成或明确失败后只预处理紧邻下一页；完成该前瞻后不继续滚动，用户实际翻页后
才再补一页。快速跳页优先追随最新当前页，完整 render identity 缓存继续优先，绝不预先提交整章。某页
失败后本次会话不自动循环重试，手动动作仍可重试，后续页继续运行。精确选择规则由
`ReaderComicTranslationAutoPolicy` 的设备测试约束：当前页优先、已收敛后仅选择 `current + 1`、不能
越过未收敛当前页或扩展到下下页。真实连续翻页的费用、P50/P95、峰值 PSS 和热稳定性仍需设备路径验收。

设备 `237` 已关闭首个真实 Reader 行为子门：开启后第 39 页先复用完整译图缓存并进入 ready，第 40 页
才启动端侧视觉管线，证明没有并行提交相邻页；第 40 页完成后菜单保持勾选且提供“显示原图”。退出再进入
Reader 后开关恢复关闭，也没有沿用上一会话的译图显示状态。对应新鲜日志分别记录
`reader_auto_start pageIndex=38`、`reader_page_ready page=38`、随后
`reader_auto_start pageIndex=39`，退出时记录 `reader_auto_reset`；完整设备 Hypium 为 281/281。
该证据只关闭菜单、串行调度、结果可见和会话复位，不替代连续多页费用与资源基准。

2026-07-23 单页前瞻补齐：此前设备证据中的第 40 页只有翻页后才出现 `reader_auto_start`，说明原实现只是
“自动触发当前页”，没有预处理。Reader 现在于当前页收敛后串行准备下一页；设备 `237` 安装新 signed HAP
后，第 1 页命中译图并保持可交互，随后翻到第 2 页的首次截图已经直接显示译图。这个 smoke 只证明 Reader
发布路径没有因后台页任务回退；是否严格只前瞻一页由上述确定性策略测试与源码边界证明。后续仍需在未命中
缓存的连续多页样本上记录完整 `prefetch=1`、阶段耗时、峰值 PSS 和热状态，不能用本次缓存路径替代性能门。
最终 signed app、signed `entry@ohosTest`、CI preflight、V2 inventory 和 `git diff --check` 通过；设备完整
Hypium 为 286/286，其中新增单页前瞻策略 3/3。

2026-07-23 未命中缓存的连续页路径已补测。设备 `237` 在 9 页真实画廊第 8 页开启自动翻译：
`reader_page_ready page=7 prefetch=0 duration=32103ms` 之后才出现
`reader_auto_start pageIndex=8 page=9 prefetch=1`；第 9 页后台完成后翻入即显示译图，没有新的 OCR/LLM
等待。第 8 页分析/翻译/渲染为 2.886/25.959/3.078 秒，第 9 页为 5.531/6.473/0.949 秒。第 9 页 13 个
CTD 候选只尝试最强 2 个，分析较旧无界路径的 19.158 秒降至 5.531 秒；YSGYolo detector 候选不受 CTD
两候选预算截断。翻页后 PSS 为 641,297 kB。该证据关闭“自动翻译不预处理下一页”和当前样本的 CTD
误报重试爆炸，不关闭更长会话、真实 webtoon、跨设备 P50/P95 或通用艺术字召回门。最终 signed app、
四条 CI preflight、V2 inventory、secret safety 与 `git diff --check` 通过；设备完整 Hypium 为 287/287。

2026-07-23 零预载前瞻回归已关闭。根因是隐藏图片 warmer 完全服从普通预载页数且只回报文件路径，预载
为 0 时下一页没有隐藏解码，自动任务也拿不到宽高。自动翻译开启期间现在至少保留一个紧邻下一页 warmer，
并从隐藏 `Image.onComplete` 回报尺寸；关闭自动翻译后仍严格服从用户原预载值。设备 `237` 临时把预载
5 改为 0 后，P4/P5 均在切页前以 `prefetch=1 visible=0` 完成，总耗时分别为 12.676/10.365 秒；阶段为
分析 3.417/3.173 秒、LLM 5.971/5.191 秒、修复渲染 3.266/1.917 秒。图片请求准备 0.155 秒，不是主要
瓶颈。验证后已恢复预载 5 并退出 Reader；完整设备 Hypium 在 10 秒 runner 宿主上限下为 288/288，
内部性能断言未变。运行中的后台页若被翻入，状态文案改为“正在完成预翻译…”，不再误报为刚开始翻译当前页。

2026-07-23 Torii 字体兼容边界已纠正。公开字体元数据显示 WildWords 不支持中文，MaShanZheng 明确支持
简体中文；设置菜单改为按当前翻译目标过滤，运行时再做一次独立归一化，避免历史 WildWords 设置在切换
语言后继续进入请求和缓存身份。设备 `237` 临时切换简体中文后，Torii 字体菜单只显示 NotoSans 与
MaShanZheng，没有 WildWords；验证后已恢复原有“端侧处理 + English”状态。新增 adapter 用例同时确认
简体中文请求发送 `font=mashanzheng` 与 `target_language=zh-cn`；完整设备 Hypium 为 283/283。

2026-07-23 CTD 普通页单瓦片热路径完成无损减负。旧实现已在原生阈值循环生成两张 mask，却仍把整页 mask
交给 ArkTS 各扫描一次计数；生产 wrapper 随后又分配两张等大合并 mask、逐像素复制并再次扫描。新实现让
原生结果携带同一阈值循环内得到的主/次像素数，ArkTS 对范围和整数性校验后直接使用；当页面完整落入唯一
2048×2048 tile 时直接返回原生 mask，多瓦片页面仍走原有重叠合并与最终去重计数，视觉 profile、阈值、
坐标和缓存身份均不变。

设备 `237` 使用同一 1024×1536 固定页、同一发布权重和三次生产 wrapper 热调用对比：旧实现总耗时
`2733/1945/1549 ms`，推理外开销 `2098/936/922 ms`；新实现总耗时 `887/1325/922 ms`，推理外开销
`259/435/286 ms`。按中位数，wrapper 开销从 936 ms 降到 286 ms（-69.4%），总耗时从 1945 ms 降到
922 ms（-52.6%）。新基准逐轮重算 mask 像素并比较生产路径三次输出，主 mask 均为 32,380 像素、次 mask
均为 29,366 像素且字节完全一致；带真实 CTD 权重的完整设备 Hypium 为 288/288。该阶段只消除普通页的
重复 CPU/内存工作，不把推理波动宣称为模型加速，也不改变长图多瓦片质量边界。

同日后续两个“看似可并行/少复制”的候选均经 `237` 实测否决并撤回。其一是在 detector 后重叠 Core
Vision OCR 与 CTD：串行基线两张普通页分析为 3,499/4,070 ms、受控长页 17,452 ms；重叠候选为
3,500/4,005/17,645 ms，且热档位更早从 1 升到 2。CTD 资源竞争吞掉了 OCR 重叠，没有净收益。其二是让
PP-OCR 直接消费首次区域读取的 RGBA、跳过临时 PixelMap 往返：唯一完整候选轮次的普通页为
6,288/3,987 ms、长页 17,633 ms；虽然个别普通页 supplemental OCR 减少 0.2–0.5 秒，但长页从
7,734 ms 变为 8,015 ms，第二轮又在进入生产基准前遇到测试宿主 `GetCurrentTopAbility` 状态错误，无法
形成可重复收益证据。两条生产源码与接口改动均未保留。后续不得凭阶段可并行或表面少一次复制重新引入；
应先对 PP-OCR 区域裁剪、预处理、系统 fallback 分别建立独立计时，再优化被证明的主耗时。

2026-07-24 PP-OCR 分段计时证明此前的主耗时不在模型、区域裁剪或临时 PixelMap，而在错误的 native
任务优先级。`ComicNativeTextRecognizer` 现在分别记录区域源 PixelMap、裁剪读取、临时 PixelMap、
recognizer 读取、NAPI 调用，以及原生模型加载、预处理、推理和 CTC 解码；生产日志用两条短事件输出，
避免 hilog 截断。设备 `237` 的后台 QoS 基线中，PP-OCR 的普通页 NAPI 调用为 2,602/1,396 ms，其中
桥接与队列等待为 2,538/1,290 ms；受控长页三次调用合计 7,909 ms，其中等待 7,558 ms，而原生
预处理、推理和解码合计只有 351 ms。输入侧所有读取与 PixelMap 操作在长页也只合计 20 ms，因此不再
沿用“少一次像素复制就是主优化”的错误假设。

补充 PP-OCR 每页最多只执行少量有界识别并直接阻塞用户请求的当前页结果，故仅把这一 native work 从
`background` 提升为 `user_initiated`；YSGYolo、CTD、AOT 和超分等重任务的 QoS 均未改变。两轮独立
目标复测中，普通页 native 桥接与等待稳定为 1–15 ms，长页为 13–14 ms；普通页 supplemental OCR
分别稳定在 125–173 ms 与 288–301 ms，长页稳定在 318–336 ms。对比同轮基线，长页 supplemental OCR
由 8,084 ms 降至 336 ms（-95.8%），长页完整分析由 17,332 ms 降至 8,804 ms（-49.2%）；完整目标用例
耗时由 38,193 ms 降至 27,743 ms（-27.4%）。识别质量保持 11 个预期块命中 9、误检 0，输出块文本和
渲染字节数未变。最终 signed app、signed `entry@ohosTest` 构建通过；重装 test host 后设备完整 Hypium
为 288/288。一次未重装宿主的重复完整运行在后半段出现 `GetCurrentTopAbility failed`，该次 263/288
不作为门禁证据，也没有被误判为 native 或质量回归。

2026-07-24 YSGYolo 分段计时随后确认同类调度空洞也存在于当前页 region detector。生产 detector
现分别记录 source PixelMap 创建/释放、RGBA 读取、NAPI 总调用、模型加载和推理，并用独立短日志计算
`nativeCall - load - inference`，不改变模型输入、阈值、瓦片、去重或视觉 profile。设备 `237` 的
`background` 基线中，三页 native 调用为 1,822/2,962/3,411 ms，模型加载加推理仅为
619/213/428 ms，剩余桥接、前后处理与排队达到 1,203/2,749/2,983 ms；RGBA 读取只有 1/1/6 ms。

YSGYolo 是当前页分析的必经阻塞任务，且单 tile 推理约 0.13–0.43 秒，因此只把
`DetectComicRegions` 的 native work 提升为 `user_initiated`，超分仍保持 `background`，CTD 与 AOT
也未在本阶段调整。两轮候选中，三页 native 调用分别为 355/217/523 ms 与 249/195/551 ms，对应桥接、
前后处理与排队为 65/71/191 ms 与 73/61/210 ms；完整目标用例由基线 28,147 ms 降到 20,518 ms，
第二轮为 18,280 ms。detector 输出始终为 5/5/11 个区域，质量保持 11 个预期块命中 9、误检 0，
三页渲染字节数未变。第二轮阶段末热档位为 1/1/2，PSS 样点为
513,853/502,010/428,612 kB，没有用一次冷机快跑替代热状态记录。

最终 signed app、signed `entry@ohosTest`、CI preflight、V2 inventory 和 `git diff --check` 通过；
重装 test host 后设备完整 Hypium 为 288/288。下一段应继续按相同方法拆分 CTD proposal 与 AOT
inpaint 的 native 调用、推理和 wrapper 开销；不得直接把更重的任务提升优先级，也不得用并行化重新引入
已经实测无收益的 CTD/Core Vision 资源竞争。

2026-07-24 CTD/AOT 分段测量没有继续套用 QoS 结论。CTD 与 AOT 的 native 结果现分别暴露预处理、推理、
后处理和 NAPI 总调用；CTD wrapper 另记录 tile 像素读取与总耗时，AOT render 另记录区域读取、mask
构造、native 调用和 PixelMap 回写。设备 `237` 基线证明 AOT 每个 region 的 native bridge 只有
16–41 ms，主体是 179–467 ms 的真实模型推理；区域读取为 1–14 ms、mask 构造为 29–77 ms。故本阶段
不调整 AOT 的 `utility` QoS、线程、分辨率或同气泡合并边界。

CTD 普通页没有同类 wrapper 问题，但 1024×3072 两瓦片长页为 2,972 ms，其中像素读取 69 ms、native
调用 1,851 ms，余下约 1,052 ms 来自 ArkTS 对每个 tile 全量逐像素 OR、再全页扫描计数。多瓦片合并现
按行维护已覆盖右边界：新覆盖区域使用 typed-array 行复制，只在重叠段逐像素 OR 并扣除重复命中，主/次
mask 像素数在同一遍直接维护，不再二次全页计数。两个候选轮次的长页 wrapper 余量稳定为 223/221 ms，
较基线减少约 829/831 ms（约 -79%）。同期 native 推理由 1,332 ms 波动到 1,728/1,925 ms，故完整
proposal stage 分别为 3,140/3,492 ms，对比基线 3,351 ms 只说明模型热波动可吞掉 CPU 减负，不能宣称
固定端到端加速。

两轮候选仍为 11 个预期块命中 9、误检 0，长页主/次 mask 计数为 54,937/48,555，三页 PNG 字节数保持
3,271,874/3,054,637/6,317,177。该阶段保留的是确定性的多瓦片 CPU/内存遍历减负和可持续计时，不把
CTD 提升为 `user_initiated`，也不恢复已否决的 CTD/Core Vision 并行。后续若继续拆 CTD，应先区分
VM-owned mask 结果复制与实际队列等待；不得用 external ArrayBuffer 重新引入连续页 PSS 不回收问题。
最终 signed app、signed `entry@ohosTest`、CI preflight、V2 inventory 和 `git diff --check` 通过；
重装 test host 后设备完整 Hypium 为 288/288。

2026-07-24 CTD 队列与结果回传继续拆分。native 结果新增 VM-owned mask 分配、主/次 mask 复制和 JS
对象组装的独立计时；设备 `237` 的 `utility` 基线中，三页 result marshalling 仅为 1/1/21 ms，
而扣除加载、预处理、推理、后处理和回传后的 native queue/bridge 为 111/221/455 ms。结果复制不是
当前瓶颈，因此明确保留 VM 管理 ArrayBuffer，不用 external ArrayBuffer 换取至多几十毫秒并重新引入
此前连续页 PSS 不回收问题。

CTD proposal/treatment 串行阻塞当前页，模型输入、线程、阈值、瓦片和推理本身均未改变；只将
`InferComicTextMask` 的 native work 从 `utility` 调整为 `user_initiated`，AOT 仍为 `utility`。
两轮候选的三页 queue/bridge 分别为 130/116/317 ms 和 105/108/323 ms；普通第 2 页和长页的等待缩短
可重复，长页 proposal stage 由 2,993 ms 降至 2,474/2,493 ms，完整目标用例由 18,928 ms 降至
18,213/18,266 ms。质量保持 11 个预期块命中 9、误检 0，三页渲染字节数未变。utility 基线热档位为
1/1/2，连续热跑候选为 2/2/2，因此这里只认定热态下仍稳定和队列等待下降，不宣称热负载改善。

同日 AOT FP32 packing 候选未进入 production。候选只启用 ncnn packing，不改 256px 上限、精度类型、
线程、mask 或区域边界；设备 `237` 的单区域推理仍为 186–443 ms，与原路径 179–467 ms 同档。三页 render
由原路径约 1,513/2,761/4,885 ms 变为 2,003/2,530/4,692 ms，第一页明显回退，另两页的小幅下降不足以
证明稳定收益；PNG 字节数也由 3,271,874/3,054,637/6,317,177 变为
3,271,880/3,054,628/6,317,112。虽然质量仍为 11 个预期块命中 9、误检 0，完整 Hypium 288/288，
但候选同时缺少稳定加速并改变像素输出，故恢复 `use_packing_layout=false`。后续不要把 packing 当作无损
开关重复启用；若评估 FP16、其他导出或运行时，必须使用固定画质集建立新的 profile 和独立缓存 revision。

AOT mask 四轮膨胀随后改为逐轮 frontier。旧路径每轮复制整张带 64px 上下文边界的 region mask，再扫描
全区域；新路径仍只让上一轮已存在的像素参与本轮扩张，但只记录新增长的 index，轮末再合入主 mask，并把
下一轮扫描限制在保守包围盒内。相对同设备同算法的整图复制对照，第一页四个 region 的 maskBuild 合计由
296 ms 降至 178 ms，第二页五个 region 由 231 ms 降至 137 ms，受控长页十个 region 由 662 ms 降至
365 ms，减少约 40–45%。三页 PNG 字节数精确保持
3,271,874/3,054,637/6,317,177，质量仍为 11 个预期块命中 9、误检 0；热档位 2 的长页 render 为
4,450 ms，完整 Hypium 288/288。该阶段只减少 ArkTS mask 遍历与复制，不改变 CTD mask、膨胀轮数、AOT
输入、模型输出、fallback 或缓存 identity。

2026-07-24 真实画廊失败隔离：停止使用生成图片代替 Reader 质量验证，改用用户已授权的 35 页日文黑白
画廊成对复核问题页与普通对白页。问题页日志确认旧路径漏判的大型区域在局部 system OCR 中为 2 字符，
但最终分组 block 为 4 字符、面积占页 10.4%；它此前进入 AOT 后形成大面积错误消字。生产 analyzer 升至
`core-vision-ocr-bubble-group-v39`：YSGYolo `shuqing` / `hengxie` 以及“面积至少 2%、最终文字不超过
4 字符”的 CTD 区域保留原稿，标记在合并/折叠中传播，render plan 明确跳过。

设备 `237` 的新鲜 cache miss 结果中，问题页 11 个 block 保留 4、渲染 7，错误白块消失，普通对白仍翻译；
阶段耗时为 3.907/10.792/7.225/0.103 秒，总计 22.029 秒。普通对白页 9 个 block 全部渲染，没有被新规则
误保留；阶段耗时为 2.517/9.748/8.556/0.044 秒，总计 20.866 秒。真实页面、裁片、文字、画廊标识和哈希
只作为临时设备证据，不提交仓库；持久文档只保留匿名特征、计数、耗时和结论。本阶段关闭该真实反例中的
破坏性擦除，不关闭艺术字翻译、旋转回填、跨画廊准确率或 AOT 串行性能门。

后续视觉复核改用用户授权的真实彩色画廊，不以生成图片代替。渲染器从 CTD 文字 mask 覆盖的源字形像素
提取主色，不再根据背景亮暗把深色背景上的译文自动改成浅色；高饱和主色与黑白暗色主体分别走确定性选择，
采样失败固定回退深色。白色描边 pen 从字号 20% 提高到 50%（4–24 px），使填充覆盖内侧后的可见外沿
达到普通字面笔画量级。真实 Reader 页面确认普通对白继续翻译、彩色艺术字保持源色，短混合脚本乱码不再
覆盖原稿；闭合气泡统一布局另由 obstacle 回归保证粗描边不越界。

同页随后暴露一条语义错误：一组重复假名喘息被译成笑声。持久化文档证明这不是纯翻译问题；OCR 正确读到
首组喘息，后组发生假名错位，并把装饰符读成孤立拉丁字母，模型再依据受污染转录生成了语义连贯但错误的
笑声。v43 因此把低多样度、重复型假名块标记为 `sfx`，同时对“假名拟声为主且夹有孤立字母/数字残片”
的转录直接保留原画。它不屏蔽普通假名对白，也不屏蔽干净的重复拟声词。

设备 `237` 的新鲜 cache miss 复测中，该喘息保持原有彩色日文字样，错误中文消失；同页普通纵排对白仍
完成中文回填，先前的纯符号和短混合脚本伪译也未重新出现。真实页面、文字、URL、画廊标识、哈希、数据库
副本和截图只保留在临时设备证据中，不提交仓库。signed app、signed `entry@ohosTest` 构建通过，完整
Hypium 为 296/296；CI preflight 与远端 Actions 作为本阶段提交门禁继续执行。

2026-07-24 彩色真实画廊扩展到连续 P11–P20 后，v46 在两张已知拟声问题页上均能保留 CTD 艺术字，旧版
错误中文覆盖不再出现；但 P16–P20 又暴露了目标脚本门禁的系统性误判。旧规则对简体中文目标只要检测到
任意假名就拒绝整块，而真实 provider 经常在有效中文中保留日文人名或少量 OCR 残片，五页因此合计误拒
13 个块。v47 改为只有“假名数量多于汉字”的近乎未翻译结果才失败关闭；设备 P19/P20 新鲜回归从旧版
5/3 个拒绝降为 0，主要为日文的结果仍会保留原稿。目标 `ComicLocalVisualBackend` 为 37/37。

这次修复只关闭整块误回退，没有把视觉质量误判为完成。v47 的 P19/P20 分别产生 7/6 个 render plan，
仍然是 7/6 个独立组、合并数 0；更多有效译文进入渲染后，窄列拥挤、跨区域重叠和白色修复块反而更清楚。
当前真实阻塞已经从“是否有译文”收敛为“detector/OCR 文字条不是可靠气泡容器”：继续放宽相邻阈值不能
建立气泡级排版边界。后续必须优先补充容器语义或碰撞安全的容器拟合，再谈字体和局部间距微调。

2026-07-24 同一授权彩色画廊继续扩展到 P21–P28。v50 将所有已分类 `sfx` 固定保留为原画，并为受损
短对白加入敬称、礼貌句式和动词标记保护；文本协议 v8 对普通对白中的重复喘声做确定性后过滤，不能再依赖
provider 是否遵守提示。八页共 71 个 block、9 个 `sfx`，高拟声压力页不再出现旧版重复中文声音覆盖。
八页 cache miss 的分析/LLM/渲染/总计中位数为 2.374/11.905/3.748/18.334 秒，首要耗时仍是 LLM。

Reader 重新安装后会话开关按设计为关闭；显式开启后，P27/P28 均在前一页可见时以
`visible=0 prefetch=1` 串行完成，翻页首帧直接显示已有译图，并只继续补紧邻下一页。该结果关闭本轮
“是否真实预处理”的验证，不改变默认关闭和退出 Reader 后重置的产品约束。

视觉门继续开放：规整短对白页相对可读，但长多列段落仍含 OCR 残片和不够精炼的目标文本。最新 geometry
证明这些大块多为同一连续多列段落，不能因结果拥挤就盲目拆列。下一工作单应建立 OCR 残片率、目标脚本
纯度和长段译文精炼的可测门槛，再在本批失败页上验证；页面文字、URL、画廊标识、哈希、数据库和截图仍
只保留为临时本地证据。最终目标回归为文本协议 5/5、端侧视觉 37/37，signed app 与 signed
`entry@ohosTest` 均构建通过。

2026-07-24 同一授权彩色画廊继续扩展到 P29–P34。P29/P30 的分析/LLM/渲染/总计分别为
2.768/80.251/3.477/86.664 秒和 2.677/11.606/4.370/18.714 秒；P29 的 provider 单次请求达到
79.820 秒，新增了真实远端长尾证据。两页仍各有目标脚本残片，视觉门不关闭。P31/P33/P34 没有形成
可翻译 block，并在 1.5–1.9 秒端侧分析后停止；P32 的单个假阳性候选触发了不必要的 provider 调用，
但 0 个 render plan 使原画安全保留。

这批页同时暴露零文字结果被误记为 `provider_request_failed`，并被 Reader 显示成端侧服务暂不可用。
下一项先拆分“当前页无可翻译文字”、provider 失败和本地视觉失败，再给调用 provider 前的假阳性候选
增加可测质量门。译文文档缓存身份已纳入 `promptVersion()`，提示词变化必须重新分析和翻译；对应设备
回归为 9/9。只靠提示词提高目标脚本纯度的 P22/P23/P25 对照结果不稳定，已拒绝并保持文本协议 v8。

零文字页的错误语义已在同日关闭：分类器新增 `no_translatable_text`，Reader 四语资源提供独立提示；
P31 真实手动路径在 2.466 秒分析后显示“当前页未检测到可翻译文字”，原图保持不变，日志确认未进入
provider。目标编排器设备回归为 10/10，signed app 与 signed `entry@ohosTest` 构建通过。下一项只处理
P32 型假阳性候选在 provider 前的质量门，不把无文字页重新归为服务故障。

P32 型 provider 前质量门随后完成。区域后端新增“是否存在可安全替换候选”的能力边界；若所有 block
都是 `sfx` 或已经标记为保留原稿，编排器在文本 translator 前返回 `no_translatable_text`，但不删除
文档 block，也不改变仍含可替换正文的混合页上下文。P32 从分析 2.390 秒 + provider 5.326 秒、总计
7.866 秒，降为分析 2.444 秒、总计 2.530 秒且无 provider 请求。纯画面 P41 同样在 1.790 秒停止；
带道具小字的 P45 虽形成 2 个 block，但两块均不可安全替换，也在 1.673 秒停止。

高密度正文 P20 的新鲜 cache miss 证明正常路径未被误伤：9 个 block 中 6 个进入回填、3 个保留原稿，
分析/LLM/渲染/总计为 2.119/15.562/4.404/22.165 秒。目标编排器设备回归为 11/11。视觉门仍开放：
该页译文中的连续拉丁词被逐字母纵排，3 个几何风险块仍保留原文；下一阶段应先建立混合脚本排版策略和
容器碰撞门，不得为消除原文而重新放开已判定不安全的区域。P41–P47 的视觉巡检还确认这一段多为无对白
画面叙事，不能把连续空样本当作 OCR 失败率。

## 视觉质量收敛门

2026-07-24 对既有真实结果做跨类型复核后，当前质量问题不再按单页参数缺陷处理。规整黑白气泡页已经能在
原有白底容器内得到相对可读的结果；彩色气泡、文字直接贴图、多列竖排和不规则开放边界则重复出现白色修复
块、译文越界、列间粘连和拉丁词逐字拆分。该差异证明当前首要缺口是**容器语义**：detector/OCR 的文字
geometry 只能描述源字区域，不能同时作为原文修复边界和译文排版容器。

后续必须显式区分三套几何：

1. `glyphMask`：只描述需要移除的源字像素；
2. `containerMask`：描述允许修复和排版占用的气泡、标题框或安全色块；
3. `layoutBox`：在容器内部、已为粗描边和阅读方向留出空间的实际译文区域。

AOT、LaMa 或其他修复模型只能在 `glyphMask` 与 `containerMask` 可靠后比较；更换模型不能修复错误容器。
白色描边继续保留已确认的粗度要求，排版器必须把描边宽度计入列间距、边界和碰撞判断，不能以再次缩细描边
掩盖空间不足。OCR 残片、目标脚本纯度和译文精炼属于独立文本质量门，不能替代几何门。

质量验证固定使用 12 页真实样本矩阵，每类 3 页：规整黑白闭合气泡、彩色闭合气泡、文字贴图或开放边界、
高密度多列或拟声压力页。样本只在本地保存，持久文档只记录匿名类别和统计。新增页面只用于发现新的失败
类别，不得因为单页失败立刻增加专用阈值。一个候选只有同时满足以下条件才进入生产：

- 12 页没有新增艺术内容擦除或气泡外平涂；
- 所有已替换组没有可见源字残留、译文重叠或越出已确认容器；
- 粗描边后的文字包围盒仍位于容器内，独立列之间不相交；
- 已分类拟声词和不安全区域按策略保留原画，不能为了提高覆盖率重新开放；
- 文本质量没有孤立 OCR 残片或错误目标脚本，长译文必须在不改变语义的前提下适配容器；
- 分类级结果改善，且规整黑白气泡基线不回退。

下一阶段只允许一个架构工作项：为 block 生成带置信度的 `containerMask`，并让修复与排版共同受其约束。
低置信度、开放边界或无法容纳粗描边译文时，端侧路线保留原画并返回可解释的“不安全替换”，由用户选择
云端整图路线；不得继续用扩大矩形、放宽相邻合并、缩小字号或新增单页文本规则强行覆盖。

该工作项在固定矩阵上只有两个结束状态：

- **通过**：上述视觉门全部满足，再继续比较修复模型、性能和更多语种；
- **不通过**：停止在当前启发式渲染器上追加参数，把文字贴图、开放边界和高密度复杂页明确列为端侧
  V1 不支持，由云端整图路线承担。

2026-07-24 为避免容器阶段继续依赖人工翻页，新增
[视觉回归工作流](../../research/manga-translation-visual-regression.md) 与本地
`scripts/comic_visual_regression.py`。首轮从已有真实 Reader 证据中筛出五组同页 source/candidate，
按漫画内容视口批量生成 JSON/HTML/overlay；相同输入的 baseline report 对比 delta 为 0。另一个文件名
标为同页的配对实际画面不同，像素变化达到 69%，已从基线排除。当前 smoke 可自动排序新增白区和纹理塌缩，
但尚未携带 `allowedMask`，因此只证明评测闭环可运行，不关闭视觉门。下一步先让真实设备运行导出稳定
source identity、analysis/translation、glyph/container mask 和 render 产物，再完成 Core-12 一次性标注。

2026-07-24 设备 recording v1 已打通：端侧后端通过默认空的 sink 导出 source、analysis、translation、
glyph mask、实际绘制布局、render 与阶段耗时；显式文件 recorder 只在测试/开发入口启用。设备 `237`
production-model 基准完成 3 组 recording，mask 尺寸与页面严格一致，拉回本地后可由
`comic_visual_regression.py --recording-dir` 直接生成带实现决策边界的报告。新增 sink 回归在目标测试组
通过，signed app 与 signed `entry@ohosTest` 均构建通过。

2026-07-26 的下一阶段把 developer recording 的分组来源从 detector 索引扩展为稳定
`sourceGroupIndexes`。索引在初始 OCR 分组后分配，并随纵向、横向和包含关系合并传播；它不进入
持久文档或 Reader 运行时协议。237 上四页公版回放的每个初始组均有且只有一个唯一索引，P12 两个
误合并大块分别精确追踪 10 和 9 个来源组，后者包含原先只能由文本长度反推的 detectorless 行。
可视回归报告优先消费精确索引，同时保留旧 recording 的兼容解析；候选分割仍只记录，不改生产分组。

同日完成第一轮独立设备 A/B：报告器只导出来源集合守恒的 `candidate-grouping-plan.json`，测试入口
通过 Base64 参数显式接收并校验页哈希、来源集合、矩形范围和唯一父块，再构造独立文档与模板。237
四页公版回放 1/1 通过；P12 从 7 个 render block 变为 11 个，两个误合并父块由 2 块拆成 6 块，
其他三页像素指标完全不变。P12 的 flatness collapse 降低 0.2197 个百分点，但 introduced white
增加 0.7324 个百分点。候选图确认字幕与对白可解除大框共享且未产生新重叠，但逐段原文/译文尚未进入
候选文档，当前重复校准占位文本不能作为生产质量证据。下一步先让精确来源组携带逐段文本并生成对应
translation block；在真实译文 A/B 通过前，Reader 继续使用现有生产分组。

下一小步已补齐逐段原文：developer recording 的每个初始来源组携带 `sourceText`，报告器按精确
`sourceGroupIndexes` 组合 segment，并分别区分几何安全与文档安全。237 新一轮四页回放中，P12
两个父块的 174/214 个非空白字符分别按 25+99+50 和 33+69+112 守恒；40 个初始来源组均无记录长度
不一致。候选回放的前六个 render block 在矩形和原文长度上与计划逐项一致，视觉后端 41/41、signed
app、signed `entry@ohosTest` 和本地 CI preflight 均通过。该结果关闭“候选几何无法生成独立原文块”
的缺口，但仍未提供每段真实译文；生产 Reader 分组保持不变。

该阶段关闭的是“真实运行产物无法稳定导出和离线评分”的工具缺口，不关闭视觉门。当前导出的
treatment/text rect 只能生成实现自身的 allowed mask，仍不是 `containerMask`。下一步先把用户授权真实
页面录成 Core-12 候选集并做一次性容器标注，再比较容器候选；不得继续凭单页截图追加渲染阈值。

2026-07-24 用户授权的五页真实输入完成 production-model 回放：一页规整黑白闭合气泡、两页彩色高密度
多列、两页彩色开放边界或文字贴图。总体改动 6.3929%，新增近白 3.9528%，纹理塌缩 1.6156%，实现决策
边界外改动为 0；分析加渲染中位数 5.339 秒，AOT 与 proposal CTD 是主要端侧耗时。该入口用既有译图 OCR
重建冻结译文，不调用 LLM，只衡量真实视觉下游，不能作为语义准确率证据。

同批页面的 glyph mask 叠图确认彩色源字的有色核心被 CTD 覆盖，但粗白描边经常留在 mask 外。扩大 mask
并追踪白描边可降低“新增近白”和“纹理塌缩”指标，却会迫使 256 px AOT 重建更大面积的真实背景，产生
明显涂抹、虚构纹理并增加约 12 秒/五页；继续切分 AOT 区域也未解决。两种候选均已否决，生产保持
profile v50 / AOT v29。该结果把下一步进一步收窄为独立 `containerMask` 加有界修复模型比较；不得再次
用扩大 glyph mask、分段 AOT 或单页阈值调整冒充质量收敛。

最终生产基线重新构建并安装到设备 `237` 后，`ComicLocalVisualBackend` 为 38/38；本地 CI preflight、
V2 inventory、signed app 和 signed `entry@ohosTest` 均通过。离线报告同时新增 glyph mask 规范化、叠图和
覆盖指标，后续真实样本扩充可以复用同一证据格式，不再依赖临时单页脚本。

2026-07-24 `containerMask` 进入“观测基础设施”阶段，但尚未接管生产修复或排版。现有闭合气泡 flood-fill
过去只在横排回填时运行；首轮五页 recording 因中文竖排绕过该分支而得到零容器，这已被作为架构缺口修正，
没有把零结果解释成真实模型能力。修正后仅在显式 recording 下使用独立宽搜索窗，竖排继续保留原
text rect、`shapeConstrained=false` 和既有最终译图。

同一真实五页第二轮得到 6 个容器候选：规整黑白页 5 个候选置信度 0.7082–0.7660，像素掩膜贴合闭合
白底气泡；彩色文字贴图页 1 个 0.5460 候选目视为相似颜色泄漏误检；其余三页没有候选。报告暂以 0.65
做分析分层，在当前样本上为 5 个高置信候选，但该阈值没有跨画廊校准，禁止直接写入生产接受规则。
最终译图总体改动 6.3929%、新增近白 3.9528%、纹理塌缩 1.6156%，与观测前完全一致。设备目标回归增至
39/39，signed app 与 signed `entry@ohosTest` 均构建通过。

该阶段关闭“无法从真实运行取得像素级容器候选和置信度”的工具缺口，不关闭容器质量门。下一阶段固定为：
补齐 Core-12 的四类样本和一次性容器标签，统计候选级 precision/recall、边界泄漏与置信度可分性；只有
跨画廊高置信度候选稳定后，才比较在该 mask 内的有界修复模型。低置信度、开放边界和文字贴图继续保留原画
或使用云端整图，不允许再以矩形扩张、glyph 膨胀或单页阈值强行覆盖。

同日 recording 增加候选标签图：每个检测容器使用独立 1–254 标签，layout 保存对应标签和置信度，离线
报告逐候选显示面积与置信度。真实第三轮回放中，黑白页 5 个候选分别占页
1.0176%–3.6792%，彩色误检候选占 2.1584%；最终译图指标仍与生产基线完全一致，设备回归为 39/39。
这使 Core-12 可以按候选标注 precision/recall 和泄漏，不再依赖 page-level 并集猜测。

历史真实产物已做资格盘点，发现的额外截图中存在英语旧链路、Reader chrome、视口不一致和未完成状态；
它们不满足同页身份与目标链路一致性，暂不计入 Core。下一轮样本扩展优先从原始 Reader 页面或稳定
recording 获取，不把旧截图裁切后未经身份校验就当作新样本。

2026-07-26 容器候选 recording 增加原框覆盖率、连通区紧致度、探测窗面积占比和内区亮度标准差，并让
离线报告同时显示候选/layout 观测比例。相同五页的 5 个正确黑白候选与 1 个彩色误检在四项特征上均有
间隔：正确候选的覆盖率/紧致度/面积占比分别不低于 0.7569/0.6488/0.2004、亮度标准差不高于 6.7691；
误检为 0.6169/0.4594/0.1472/9.8728。该结果只证明候选特征值得进入 Core-12 校准，不授权把这些边界或
0.65 分层写入生产。

本轮最终译图总体改动 6.3929%、新增近白 3.9528%、纹理塌缩 1.6156%，与此前完全一致；五页 recording
分析加渲染均值 6.6098 秒，与前一轮 6.6172 秒无可分辨回退。设备 `237` 的
`ComicLocalVisualBackend` 为 39/39，真实五页回放为 1/1，signed app 与 signed `entry@ohosTest`
构建通过。下一项仍是补齐跨画廊 Core-12 和独立人工容器标签；候选/layout 21.4286% 只是观测比例，
不得作为 recall。

2026-07-26 又从用户授权真实画廊取得连续七页彩页，并通过新增的
`comicContainerCalibration` 测试入口直接运行生产模型；固定占位译文只负责触发 recording，不作为
翻译结果。七页共 39 个 layout、8 个容器候选。已复核三页中，5 个彩色矩形旁白框置信度
0.7294–0.7984，3 个开放文字/相似背景误检为 0.5065–0.6279；同一旁白页另有 2 个浅色闭合框漏检。
与此前五页的已复核候选合并后，五个已标注页面共有 12 个目标容器、10 个真候选、4 个误检和 2 个漏检：
0.65 分层 precision 100%、recall 83.3333%。该分层尚无独立 holdout，不进入生产。

设备 `237` 七页回放为 1/1，通过 52.015 秒；逐页 analysis 均值 2.721 秒，render 均值 4.678 秒，总
耗时均值 7.399 秒、中位数 7.588 秒、范围 5.196–9.312 秒。render 占 63.2248%，下一性能工作项固定为
减少随 block 数增长的修复/mask/编码重复开销。下一质量工作项固定为补充浅色闭合矩形面板候选并完成剩余
页面人工标签，而不是降低 flood-fill 阈值。

离线报告已支持自动读取 `timings.tsv` 和可选 `--container-review`，旧 recording 保持兼容。当前 12 页
只达到候选矩阵的数值规模，尚未达到“全页人工标签 + 独立画廊 holdout”的 Core-12 关闭条件。

2026-07-26 容器探测补齐逐候选失败原因，并增加 recording-only 的轴对齐矩形边框分支。七页开发集原有
两个 `area_too_small` 漏检被精确补回，其余页面没有新增候选；已复核三页达到 7/7 召回，0.65 分层
precision/recall 均为 100%。另一段连续七页、43 个 layout 的硬负向留出中该分支新增候选为 0。
该证据只关闭当前规则的开发集漏检和同画廊负向特异性，不关闭跨画廊正向召回；矩形边框分支继续禁止接管
Reader 生产修复，下一质量工作项固定为另一视觉家族的正向矩形框 holdout。

另一视觉家族随后以四页公版老扫描彩漫完成独立正向留出：18 个目标中原 flood-fill 只命中 9 个，
precision/recall 为 42.8571%/50%。四边 0.9 的 recording 候选新增 0；四边得分诊断显示下边均值
0.3741、最弱边均值 0.2417。将最低四边覆盖门仅在 recording 中降到 0.5 后新增 1 真 1 假，整体
precision/recall 仅升至 43.4783%/55.5556%；同画廊七页硬负向的 11 次尝试仍新增 0。该结果否决
“继续降低统一四边阈值”作为生产方案，矩形分支继续 recording-only。下一质量工作项改为同一连通轮廓
上的短断边/平行边几何候选，并必须沿用这 4 页正向 + 7 页负向双门复核。

同一双门又否决了朴素四角硬门：最低边分数放到 0.3 后有 7 个正向候选进入角点检查，但要求四个局部角点
均不低于 0.35 时新增真候选为 0，硬负向仍为 0。该规则已撤回，角点只保留在 recording 诊断中，候选
接受恢复到最低边分数 0.5。下一工作项必须追踪同一连通轮廓上的短线段/平行边，不能再组合四条互不关联的
独立峰值。

同日 render recording 增加实际 AOT 调用次数。七页生产基线为 6.1 个 drawable group/页和
6.1 次 AOT/页，AOT 均值 3.2090 秒，占后端 render 的 64.2608%。跨独立气泡保守合并候选虽然把调用降到
5.0 次/页、render 由 5.0536 秒降到 4.8963 秒，但只取得约 3.1% 收益，并改变独立气泡的 AOT 输入域；
该候选已撤回，生产继续遵守 v29 的“仅同一已确认闭合气泡可合并”。下一性能工作项不得再放宽跨气泡合并，
应比较不改变输入域的模型/运行时成本，或处理仍占约 29% 的 layout 阶段。

2026-07-26 离线容器评审升级为目标框 schema v2：候选与目标做一对一空间匹配，并同时读取 recording 的
render source layout 与分析文档块，区分可渲染、渲染源合并、安全保留、仅文档有源和文档级真正缺失。
旧 schema v1 已用 final-v5 记录复跑，10 真/13 假的既有统计保持兼容。视觉复核同时删除一处错误真值：
`p13-top-left` 是包含人物和场景的整格画面边框，不是可擦除字幕容器。修正后公版四页共有 17 个目标，
其中 12 个有独立 render source layout、2 个 render source 合并、1 个分析文档块合并且因几何安全
策略保留、2 个在分析文档中也不存在；旧候选只命中 10/12 个 eligible 目标。

recording-only 边框搜索随后增加保守第二遍：先完整保留既有 3–64 px 规则，失败时才横向搜索 0–96 px、
纵向搜索 0–64 px；短 OCR 行高度下限为 12 px，`touches_probe_edge` 也可进入诊断。直接全局从 0 px
搜索曾破坏确定性描边矩形用例，已否决；两阶段版本设备目标回归恢复 40/40。四页真实正向重放补回两个
几何漏检，eligible recall 从 10/12 提高到 12/12；总体目标 recall 为 70.5882%，其余五个漏检全部是
上游 source 边界或召回问题。设备 `237` 的新鲜日志确认两个真正缺失目标既不在系统 OCR 中，也没有
YSGYolo/CTD proposal 覆盖；两个 render-source 合并目标原本存在逐行 detector 区域，但被后续横向
分组跨区合并；安全保留目标则与报纸内容共用一个文档块，并传播
`local_source_geometry_not_supported`。七页、43 个 layout 的真实彩页硬负向中矩形分支新增接受为
0，既有普通容器候选和最终译图指标不变。

该候选继续禁止接管 Reader：全部候选 precision 仍只有 46.1538%，而四条独立最高分边不保证来自同一
轮廓。下一质量阶段不再继续调统一阈值，拆为两个工作项：容器侧增加同轮廓/成对平行线证据以降低误检；
上游侧分别处理 2 个渲染源跨区合并、1 个安全保留的合并文档块和 2 个真正漏检。不得用统一横向间距
阈值强拆正常多行对白，任何整格画面边框也不得为了提高 recall 进入擦除范围。

2026-07-26 下一阶段先补齐真实分组来源链，而没有直接改生产阈值。显式 developer recording 现保存
`grouped`、`verticalMerged`、`horizontalMerged`、`output` 四个阶段，并为每个块记录完整 detector
region ids/labels、矩形、字符数、原始行尺度和保留标记；生产 ComicPageDocument 与 Reader 结果不变。
设备 `237` 的目标后端回归 40/40，四页公版重放 1/1。P12 右侧错误大块由 10 个
`changfangtiao` 行区横向合并而成，左侧由 8 个行区合并而成，已把后续工作精确定位到视觉分隔边界。
下一项是在同一正向/硬负向真实页上自动评分边界线证据，不得只凭 P12 写页面位置或 detector label
特例。

同日离线报告完成第一版横排分隔探针。四页公版有 249 个原始分组、44 个多 detector 横向合并块和
189 条候选边界；联合要求正间距至少 3 px、边界横跨最终合并宽度至少 50%、连续深线或垂直亮度边缘至少
0.55 后，只保留 P12 的 4 条“标题栏 / 正文框 / 气泡”边界。七页彩页硬负向有 174 个原始分组，但没有
多 detector 横排合并，探针数为 0；它不构成正常横排对白的负向证明。当前只提交 recording/report
能力，不改 Reader 文档分组。下一阶段必须补含普通英文多行对白的真实负向 holdout，再决定是否把视觉
分隔接入端侧分析。

同一公版漫画随后冻结连续五页普通英文横排段落作为独立负向留出。五页共有 237 个原始分组、
30 个多 detector 横向合并块和 165 条可评分边界；现有强门只产生 1 个候选。原图复核确认该候选位于红色
旁白框与下方对白气泡之间的真实黑色容器边界，正常旁白和气泡段落内部误报为 0。该结果让视觉分隔进入
recording-only 拆分 A/B，但仍不授权修改 Reader 生产分组。

回放同时暴露一处真实安全失败：低对比源字被 OCR 接受、却完全不在 CTD 文本掩膜中，旧代码会中止整页
渲染。生产降级现改为保留该源块、只继续修复和回填掩膜覆盖充分的其他组，禁止用颜色阈值猜测性擦除。
设备 `237` 的目标回归为 41/41，五页真实回放为 1/1；实际记录为 48 个可绘制组、1 个安全保留组，
分析加渲染均值 7.8378 秒，其中 AOT 均值 3.8770 秒、占后端 render 75.2494%。下一阶段仅实现开发记录
中的候选拆分和像素/分组 A/B；通过正向四页、该五页普通横排负向和七页彩页硬负向后，才能提出生产接入。

recording-only A/B 已把强边界转换成可审计拆分计划和原图叠图。四页公版正向中，P12 的两个错误大块分别
按两条真实容器边界拆成三个段，现状 2 块变为候选 6 块，净增 4 块；五页英文横排留出只把最后一页混合的
红色旁白与下方对白从 1 块拆成 2 块；七页彩页硬负向保持 0 个计划、0 个块变化。三组共 16 页没有出现
穿过普通段落的候选。三个计划均通过字符数和 detector 来源守恒；其中一行无 detector id 的 6 字符来源
通过唯一字符数子集归入第三段，避免几何正确却漏字。该阶段只证明候选几何可分、负向未触发，不代表候选
文本已经重新拼接或重新渲染。
下一步是在显式 developer recording 中运行候选分组，比较候选 document、译文适配和最终像素；生产
Reader 继续使用现状分组。

2026-07-26 真实译文 A/B 已完成。新增 debug-only 主进程桥，让 `ohosTest` 只导出页面请求，由主应用在
设置加载后用已配置 LLM 源翻译；凭据和 profile 不跨进程写盘，release build 不执行。设备 `237` 的 P12
现状为 7 block、provider 13.452 秒、analysis + render 6.730 秒；分段候选为 11 block、provider
14.629 秒、analysis + render 7.032 秒。模型返回了全部 11 段，但候选只有 10 个可绘制 layout：中央长
旁白因目标文本过长未通过 fit，安全保留原文。候选纹理塌缩下降 1.0220 个百分点，新增近白却增加
0.9769 个百分点；截图复核还确认同一父区域的子段各自扩框、独立字号后发生重叠和字号失控。

该结果否决“语义拆分后直接按子段独立修复/排版”，不否决视觉分隔和分段翻译。生产 Reader 继续保留
7-block 基线。下一阶段固定为 recording-only 的父区域重组：分段原文/译文继续独立进入 LLM，但返回后
按稳定父块合并，父区域只执行一次修复和一次统一排版；若统一文本不适配，必须在擦除前失败关闭，并评估
一次带确定长度预算的文本精炼。该父区域候选通过正向页视觉门和既有负向留出前，不得进入生产。

父区域重组已在设备 `237` 完成两轮真实译文验证。第一轮复用 11 段译文并收敛回 7 个原父块，彻底消除
子段各自扩框造成的互相覆盖；同时定位到一个独立排版缺陷：中文旁白中包含 `34,000` 时，旧 fit 逻辑按
空白切词，把整段中文错误地当成一个含拉丁数字的不可断词单词。修正为只测量实际连续拉丁/数字 token 后，
设备后端回归为 42/42，中央旁白恢复翻译，7/7 父块全部布局成功。该修正属于生产安全 bugfix，不改变
候选分组。

第二轮只在 recording 中增加保守的跨块断词续接：父框内必须恰有一个小型拉丁块、恰有一个子段以字母
连字符结尾，且候选位于该子段附近；存在多个候选或没有行尾连字符时一律不合并。真实 P12 将
`ANY SUGGES-` 与孤立的 `TIONS, GENTLE-MEN?` 合成 `ANY SUGGESTIONS, GENTLEMEN?` 后重新调用同一
LLM，翻译 block 从 11 降为 10，最终父渲染块从 7 降为 6，6/6 成功布局，重复的“有何建议”及其重叠
消失。目标页最大新增白块由 2.4064% 降至 2.2015%，纹理塌缩由 0.9011% 降至 0.7033%；新增近白由
6.2644% 升至 6.4088%，因此仍不能晋级生产。

该轮四页串跑总耗时异常升至 146.335 秒；异常从目标页之前即出现，第二页 render 已达 51.296 秒，之后
analysis 和 render 同时变慢，不可归因于只执行字符串和几何判断的续接规则，也不可与冷态基线直接比较。
下一阶段拆成两条：质量侧继续解决父框字号过大和空间利用率，但只允许在父区域统一 layout 内调整；性能侧
增加目标页单页冷态/热态基准，分开统计模型加载、analysis、layout、AOT 和编码，避免四页串跑尾页受热降频
污染。跨块续接继续 recording-only，必须扩充含行尾连字符但不应跨块合并的真实负向页后再讨论生产接入。

2026-07-26 单页隔离基准已完成。外部回放可按原始页码只运行目标页且不改变候选映射；同一 P12 冻结译文
三次总耗时为 6.460/5.464/5.652 秒，analysis 为 2.674/2.172/2.182 秒，render 为
3.786/3.292/3.470 秒。AOT 仍是后端最大阶段，为 2.739/2.214/2.448 秒；四页串跑的 38.179 秒
目标页据此判定为热态/资源污染，不能计入续接规则成本。

质量侧新增显式 recording-only 页面字号 profile，在最终 text rect 上取横排可用字号的下中位数作为
全页上限。目标页字号由 `24/22/18/17/16/13` 收敛为 `17/17/17/17/17/13`，顶部父区域不再异常
放大；changed/new-white/largest-white 改善为 10.9116%/4.0765%/1.1290%，但纹理塌缩从
0.7033% 升到 1.0769%。该候选暂不进入 Reader 默认 profile；下一阶段直接扩展冻结真实页 A/B，检查
小气泡、长旁白和不同页面字号分布。生产默认路径在匹配安装 signed 主 HAP 与 test HAP 后保持 42/42。

2026-07-26 横排真实译文字号阶段扩到四页。P10/P11/P13 分别执行一次真实 provider 请求
14.133/24.307/12.735 秒，P12 复用既有响应；每页默认/候选共用响应，未重复计费。硬中位数 v1 虽然
消除明显爆字，却在三页把 cap 压到 16/16/18px，并让纹理塌缩分别增加
0.2141/0.9584/0.3677 个百分点，已否决为生产候选。

outlier-only v2 使用“下中位数 + `max(2px, 20%)`”上限，四页 cap 为 19/19/18/22px。四页
changed 分别下降 0.9077/1.3489/0.3850/0.7127 个百分点，新增近白下降
0.9840/1.5324/0.4752/0.6839 个百分点；纹理塌缩仍增加
0.0535/0.5461/0.0549/0.0445 个百分点。截图复核显示字号离群得到抑制且空间利用率优于 v1，但四页
同属一个公版视觉家族，v2 继续 recording-only。另一组五页日文竖排真实译图在候选下逐像素不变；普通
横排留出的 `WHO..00-` 已作为数字尾连字符负例固定，禁止被续接规则吸收。下一质量门固定为跨画廊横排
真实译文，不再围绕当前四页继续调参。

2026-07-26 跨画廊门使用公版 `Turok, Son of Stone - Four Color Comics no. 596` 的 P5/P6/P9。
三页分别覆盖普通对白、长短混合和风格化对白；每页真实 provider 响应只生成一次，P9 的首次传输失败
只重试一次，全部后续 A/B 复用冻结响应。outlier-only v2 只在 P5/P6 各压低一个离群字号，P9 基本
不变；它没有破坏新家族，但收益也很小，因此继续 recording-only，尚不升入 Reader 默认 profile。

这组页面同时揭示比字号更严重的生产样式根因：旧描边以字符高度 50% 作为笔宽，填充后仍留下约 25% em
的可见外描边，导致相邻字形和破折号连成白条；老扫描棕褐纸色还会被误判为彩色文字墨色。生产样式改为
`clamp(4px, 16px, 0.32em)` 的描边笔宽，并要求彩色墨色候选与已采样背景至少有 48 的通道或亮度
对比，否则回退到暗色字干样本。

设备 `237` 复用相同译文复跑后，P5/P6/P9 的新增近白分别从
1.7942%/2.7323%/1.5874% 降为 1.4120%/2.0538%/1.1492%；changed 分别下降
0.0270/0.1496/0.0787 个百分点，纹理塌缩变化为 -0.0108/+0.0428/+0.0220 个百分点。三页截图
确认深色字干与白描边重新分离，P6 的连续白条可读性失败消失；AOT 输入和容器/布局数量均未改变。
设备定向回归为 43/43。

下一阶段不再继续调整描边宽度。真实结果中残留的异常长横线来自 OCR/译文 token，而不是笔画连接；
工作项固定为在冻结 P5/P6/P9 文档中追踪原 OCR、发送给 LLM 的 segment 和返回译文，建立只针对可证明
污染 token 的安全门，并用合法破折号、拟声词和短对白负例防止过度过滤。字号 v2 与污染过滤必须保持
两个独立候选，禁止为了改善截图把两者混入同一无法归因的改动。

2026-07-26 来源追踪已完成第一例生产安全门。P6 的 `---` 多为正常对白停顿，不能过滤；P9 的
`---AND G000' MMMMMM--- SZZZIP!` 则由 3 个数字混淆/重复拟声 token 主导，模型只会把坏输入扩写成
更多横线。端侧仅将“短 Latin transcript、至少 3 个噪声 token、噪声覆盖至少 2/3 字符、至少一处连续
重复字母”的块归为 SFX 并保留原画；单个 OCR 错字、普通数字、合法破折号和普通拉长语气均有定向负例。

文档 kind 改变后旧响应的 context fingerprint 被正确拒绝，因此只重新请求 P9 一次（provider 9.919 秒），
其余回放复用该响应。设备 `237` 最终 P9 从 10 个 layout 降为 9 个，坏块保留原始 artwork，其他九个
对白仍翻译；changed/new-white/flatness 为 3.0189%/0.9234%/0.4298%，新增近白比文字样式基线再降
0.2258 个百分点。`ComicLocalVisualBackend` 定向回归保持 43/43。

下个阶段切换为数据集与自动评测基线，不再把人工截图作为每次调参入口。第一版数据集必须保存真实页面
身份、冻结原图、OCR block、kind、翻译请求/响应指纹、最终 layout/修复记录和人工真值引用；按画廊而非
页面随机切分开发与 holdout。训练候选拆成 detector、OCR、mask/inpaint 和翻译评测四条，不允许用一个
混合分数掩盖任何一层退化。
