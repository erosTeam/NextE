...
- 2026-06-16 17:06:54 +0800 — P0 selector white-screen regression controller-verified for reachable cases. Build/security gates and V1 inventory passed; pagination UI acceptance relied on the captured device screenshots rather than the now-retired source-shape contract. New HAP listing confirmed no forbidden raw credential resource. Device screenshots: `/tmp/nexte_qa/qa02a.jpeg` Home source transient, `/tmp/nexte_qa/qa05a.jpeg` Toplist period transient, `/tmp/nexte_qa/qa09a.jpeg` Search submit transient, `/tmp/nexte_qa/qa11a.jpeg` Search filter transient; contact sheet `/tmp/nexte_qa/p0_selector_transients_sheet.jpg`. Favorites favcat/order and site 表/里 device cases remain `BLOCKED` by logged-out state after security remediation and must be re-device-verified after normal WebView login is available.
...
- 2026-06-18 07:45 +0800 — Mac-side false-404 logged-out smoke refreshed after auth-cookie-login-rebase merge. On Mate X7 emulator target `127.0.0.1:5555`, with every `hdc` command run outside the Codex sandbox, deep-linked `https://e-hentai.org/g/1/0000000000/`. The displayed error was localized parse failure, `无法解析此页面,应用可能需要更新。`, with `重试`; no user-facing `404` text appeared for this invalid public-gallery case. Evidence: `/private/tmp/nexte_lane_f_false_404_evidence/nexte_lane_f_false_404.png` and `/private/tmp/nexte_lane_f_false_404_evidence/nexte_lane_f_false_404_layout.json`. Limitation: bounded hilog tail did not capture a useful NextE classifier line, and this does not cover true-HTTP-404, ExHentai/Sad Panda, donor/permission, MaybeHidden, or logged-in auth matrix cases.
- 2026-06-18 08:03 +0800 — Gate V2 reachable-subtab Mac re-QA refreshed on Mate X7 emulator target `127.0.0.1:5555`; every `hdc` command ran outside the Codex sandbox. Home default list loaded real rows with no terminal empty/no-more copy; Home default → popular switch was captured immediately (`/private/tmp/nexte_lane_continuation_probe/nexte_req_home_hot_immediate.png`) and after settle (`/private/tmp/nexte_lane_continuation_probe/nexte_req_home_hot_after.png` + `nexte_req_home_hot_layout.json`), and the settled layout contained gallery rows with no `没有数据/没有更多了`. Toplist all period loaded real rows; Toplist all → year switch was captured immediately (`nexte_req_toplist_year_immediate.png`) and after settle (`nexte_req_toplist_year_after.png` + `nexte_req_toplist_year_layout.json`), and the settled layout contained gallery rows with no terminal empty/no-more copy. Gates rerun: `test_selector_reload_preserves_content_contract.mjs` PASS, `test_retained_tab_contract.mjs` PASS, V1 inventory PASS 0 file(s). Search landing and Favorites follow-up remain blocked by the system Xiaoyi IME first-run privacy page staying in the foreground; Codex clicked no `同意` button and typed no text.
- 2026-06-18 08:15 +0800 — Preview grid token correction. Controller/user clarified that `ThemeConstants.PREVIEW_THUMB_MIN_W=90` was intentional: it keeps the Mate X7 preview pane at 3 thumbnails per row while not making Mate 60 Pro jump to 4; 105 makes Mate X7 fall to 2 thumbnails and is not the target design. The later `0652a05` restoration to 105 incorrectly overrode that decision. Corrective lane restores the product token to 90 and updates `scripts/test_responsive_grid_contract.mjs` so it gates responsive derivation, stable tile framing, the X7/Mate60Pro 3-column breakpoint, and the 90vp token.
- 2026-06-18 08:20 +0800 — Preview grid token correction verified on Mate X7 emulator `127.0.0.1:5555`, with hdc outside the Codex sandbox. Gates passed: responsive-grid (55 assertions), thumbnail-mode, cover-presentation, detail-header-visual, V1 inventory 0, i18n parity, secret-safety, `git diff --check`, and official `scripts/build_hvigor_signed.sh` PackageHap + SignHap. Installed `entry-default-signed.hap`, opened public detail `https://e-hentai.org/g/3989982/16600a66e8/`; detail page loaded with no error and preview entry visible. Preview grid screenshot shows page labels 1-6 in the viewport after scrolling. Evidence: `/private/tmp/nexte_preview_min90_evidence/nexte_preview_min90_detail.png`, `nexte_preview_min90_detail_layout.json`, `nexte_preview_min90_grid.png`, `nexte_preview_min90_grid_layout.json`.
- 2026-06-18 08:30 +0800 — Horizontal preview mode Mac re-QA added on the same Mate X7 emulator and same installed `a253aff` build. `横向缩略图` was toggled on through Settings, verified as `checked=true`, and restored to `checked=false` after QA. Opened public detail `https://e-hentai.org/g/3989982/16600a66e8/`; horizontal preview row shows the visible `预览 (138)` header, `查看全部` entry, and a horizontal row with page labels 1/2/3. Tapping `查看全部` opened the AllThumbnails route with the `预览` title and a 3-column 1-12 grid. All hdc commands ran outside the Codex sandbox. Evidence: `/private/tmp/nexte_preview_min90_horizontal_evidence/horizontal_preview_row.png`, `horizontal_preview_row_layout.json`, `horizontal_all_thumbnails.png`, `horizontal_all_thumbnails_layout.json`, plus `settings_layout_after_toggle.json` and `settings_layout_restored.json` proving the temporary setting change was restored.
- 2026-06-18 08:44 +0800 — Hidden-inline preview mode Mac re-QA added on the same Mate X7 emulator and same installed `a253aff` build. `隐藏画廊缩略图` was toggled on through Settings, verified as `checked=true` while `横向缩略图` remained `checked=false`, and both switches were restored to `checked=false` after QA. Opened public detail `https://e-hentai.org/g/3989982/16600a66e8/`; preview card hid inline thumbnails while keeping the visible `查看全部` header entry and centered `更多预览` entry. Tapping `更多预览` opened the AllThumbnails route with the `预览` title and a 3-column 1-12 grid. All hdc commands ran outside the Codex sandbox. Evidence: `/private/tmp/nexte_preview_min90_hidden_evidence/hidden_preview_entry.png`, `hidden_preview_entry_layout.json`, `hidden_all_thumbnails.png`, `hidden_all_thumbnails_layout.json`, `settings_after_toggle.json`, and `settings_restored.json`.
- 2026-06-18 08:47 +0800 — Detail header/action Mac partial re-QA added on the same Mate X7 emulator and same installed `a253aff` build. Opened public detail `https://e-hentai.org/g/3989982/16600a66e8/`; current reachable state is not favorited and first-read. Header screenshot shows cover + title + JP title + uploader inside the card, with a compact right-bottom `阅读` capsule and no fake favcat heart/title block. Layout confirms `阅读` Button bounds `[768,805][1006,918]`, uploader is present/clickable, and page count/favorite-count metadata stays in InfoBar rather than the read CTA. All hdc commands ran outside the Codex sandbox. Evidence: `/private/tmp/nexte_detail_header_action_evidence/detail_header_action.png`, `detail_header_action_layout.json`. This does not cover favorited/unfavorited pair, resume label, or read-state matrix acceptance.
...
- 2026-06-18 10:00 +0800 — Gallery cover presentation dark-mode Mac partial re-QA added on Mate X7 emulator `127.0.0.1:5555`, same installed signed HAP. System Settings was used through visible UI to switch `浅色` -> `深色`; after QA it was restored to `浅色` and the restored layout confirms the light radio checked=true, dark radio checked=false, and `深色模式=已关闭`. Home default list in dark mode shows loaded covers as complete images over the dark distinct placeholder/backdrop, with layout Image backgrounds `#FF2E2E30`. Public detail `https://e-hentai.org/g/3989982/16600a66e8/` in dark mode shows the `Rockman: 10 Years History Book` header cover as a complete rounded cover in the dark card, not as a thin strip inside a grey block. All hdc commands ran outside the Codex sandbox. Evidence: `/private/tmp/nexte_cover_darkmode_evidence/system_restore_probe.json`, `dark_home_list.png`, `dark_home_list_layout.json`, `dark_detail_header.png`, `dark_detail_header_layout.json`, and `system_display_light_restored.json`. This covers loaded/dark Home list + loaded/dark detail header only; loading, error, dark grid-card, and controller visual acceptance remain open.
- 2026-06-18 10:14 +0800 — Dark grid-card intentional-cover behavior Mac partial re-QA added on Mate X7 emulator `127.0.0.1:5555`, same installed signed HAP. System Settings was switched to `深色`, NextE Settings `列表视图` was switched from `列表` -> `简洁` -> `网格`, Home default grid was captured, then NextE was restored to `列表` and system Settings was restored to `浅色`; all hdc commands ran outside the Codex sandbox. Evidence: `/private/tmp/nexte_cover_dark_grid_evidence/system_dark.json`, `settings_initial.json`, `settings_after_click1.json`, `settings_grid.json`, `dark_grid_home.png`, `dark_grid_home_layout.json`, `settings_restored.json`, and `system_light_restored.json`. Layout confirms Home `E-Hentai` default `Grid` with two visible `GalleryGridCard` cover Image nodes (`[62,439][1068,1875]` and `[1143,439][2149,1875]`) over `#FF2E2E30`; screenshot confirms grid-card covers intentionally fill their grid cells in dark theme. This covers loaded/dark Home grid-card behavior only; loading, error, and controller visual acceptance remain open.
- 2026-06-18 10:28 +0800 — Gallery cover loading/error fallback structural hardening added. `EhThumbnail` now tracks `loaded` and `failed` separately, resets both on URL reuse, keeps the centered spinner only for active loading, and shows a tertiary `sys.symbol.picture` marker on the distinct cover placeholder after image failure. All thumbnail branches route through the same combined `coverOverlay`. `scripts/test_cover_presentation_contract.mjs` now gates explicit loading/error fallback behavior and rejects direct `loaderOverlay` bypasses. Official signed build output was installed on Mate X7 emulator `127.0.0.1:5555`, launched successfully, and smoke-captured Home default list; all hdc commands ran outside the Codex sandbox. Evidence: `/private/tmp/nexte_cover_fallback_evidence/nexte_cover_fallback_loaded_smoke.png`, `nexte_cover_fallback_loaded_smoke_layout.json`, `nexte_cover_fallback_post_reinstall.png`, `nexte_cover_fallback_post_reinstall_layout.json`. Layout confirms foreground `com.erosteam.nexte` / `EntryAbility` / `pages/Index`, Home `E-Hentai` default list, and real gallery rows. This is implementation/contract/device-smoke evidence only; real loading placeholder screenshots, real error fallback screenshots, and controller visual acceptance remain open.
- 2026-06-18 10:43 +0800 — Gallery cover loading/error visual probe captured on Mate X7 emulator `127.0.0.1:5555`, installed from the current official signed HAP; all hdc commands ran outside the Codex sandbox. The current build adds an internal-only QA URI `nexte://qa/cover-fallback` (explicit `aa start` only; not listed in `module.json5` skills) that renders the real `EhThumbnail` component in forced loading and forced error states. Evidence: `/private/tmp/nexte_cover_fallback_probe_evidence/nexte_cover_fallback_probe.png`, `nexte_cover_fallback_probe_layout.json`. Layout confirms foreground `com.erosteam.nexte` / `EntryAbility` / `pages/Index`, visible `Cover fallback probe`, `Loading`, `Error`, one `LoadingProgress`, and one error-side `SymbolGlyph`; both forced-state cover slots use the light cover placeholder `#FFE6E8EB`. This satisfies deterministic component-state loading/error visual evidence. Controller visual acceptance remains open, and naturally network-triggered loading/error frames should be captured separately only if the controller rejects deterministic probe evidence.
- 2026-06-19 06:59 +0800 — Current-main reachable-subtab refresh ran on Mate X7 emulator target `127.0.0.1:5555`; hdc commands ran through the approved DevEco hdc path. V1 inventory and the official signed build passed. Current signed HAP was installed. Home default baseline, Home default -> popular immediate/settled, Toplist all baseline, and Toplist all -> year immediate/settled were captured. Layout inspection found real gallery rows and no terminal `没有数据/没有更多了` copy in both immediate and settled switch states. Evidence: `/private/tmp/nexte_gate_v2_current_main_evidence/` (`home.png/json`, `home_hot_immediate.png/json`, `home_hot_settled.png/json`, `toplist_all.png/json`, `toplist_year_immediate.png/json`, `toplist_year_settled.png/json`). Favorites favcat/order and Site 表/里 remain outside this refresh because they require safe logged-in/auth state. SearchFilter/Search chrome are current baseline and were not reopened. The former UI/source-shape contracts cited here are retired.
...
|---|---|---|
| P0 sub-tab switching white-screens / reloads | PARTIAL_REQA | 2026-06-19 current-main refresh again covers Home default->popular and Toplist all->year immediate/settled states without terminal empty/no-more copy. Favorites favcat/order and site switch remain open because they require safe logged-in/auth state; SearchFilter/Search chrome are current baseline and were not reopened in this refresh. |
| P0 false 404 on some galleries | NEEDS_CONTROLLER_REQA | Current Mac smoke confirms one logged-out invalid public-gallery deep link displays parse-failure copy rather than a false 404, but true-404, ExHentai/Sad Panda, donor/permission, MaybeHidden, and logged-in auth matrix cases still need controller-owned evidence. |
| P0 detail preview semantics collapsed | PARTIAL_REQA | Current Mac evidence covers default grid at the corrected 90vp token, horizontal preview row, horizontal `查看全部` -> AllThumbnails, hidden-inline preview entry, and hidden `更多预览` -> AllThumbnails route. Controller visual acceptance and any remaining disputed thumbnail presentation still need current acceptance. |
...
| P1 gallery cover presentation | PARTIAL_REQA | Current Mate X7 evidence covers loaded/light detail header, loaded/light Home default list, loaded/light Home grid-card intentional-cover behavior, loaded/dark Home default list, loaded/dark detail header, loaded/dark Home grid-card intentional-cover behavior, and deterministic `EhThumbnail` loading/error visual states via the internal probe. Controller visual acceptance remains open; naturally network-triggered loading/error frames are still separate if required by controller. |
...
- Historical note: this initially raised `ThemeConstants.RADIUS_SM` from 4 to 8 so shared small
  image/content corners no longer read as square on detail preview tiles, all-thumbnails tiles, header
  cover, and compact cover surfaces.
- Current baseline supersedes the token shape: `805496d` split thumbnail/cover roundness into
  `ThemeConstants.THUMB_RADIUS=8`, while ordinary `RADIUS_SM` returned to the compact 4vp role.
  Do not reopen this as a request to change product roundness unless a new visual defect is filed.
- This was a token-only visual hardening pass. It did not change preview column count, sprite crop math,
...
- Deterministic gate: `scripts/test_responsive_grid_contract.mjs` now locks both
  `PREVIEW_THUMB_MIN_W=90` and the current thumbnail/cover radius token `THUMB_RADIUS=8`.
