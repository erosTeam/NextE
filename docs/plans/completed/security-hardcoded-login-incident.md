...
2. `entry/src/main/resources/rawfile/dev_session.txt` exists locally and contains an EH auth cookie header shape:
...
3. The same rawfile is present in build intermediates:
   - `entry/build/default/intermediates/res/default/resources/rawfile/dev_session.txt`
...
4. The current local HAP artifacts contain the credential rawfile (verified with `unzip -l`, without reading values):
   - `entry/build/default/outputs/default/entry-default-signed.hap` contains `resources/rawfile/dev_session.txt` (96 bytes)
   - `entry/build/default/outputs/default/entry-default-unsigned.hap` contains `resources/rawfile/dev_session.txt` (96 bytes)
...
5. `.gitignore` currently ignores:
   - `.env.local`
   - `scripts/dev.env`
   - `entry/src/main/resources/rawfile/dev_session.txt`
   - `entry/build/`
   - `*.hap`
6. Full local git-history audit at 2026-06-16 after the user confirmed the HAP has not left the local-device boundary:
   ...
   - Result: no tracked `entry/src/main/resources/rawfile/dev_session.txt` filename in history.
...
8. `git ls-files` and tree search found no tracked `dev_session.txt` path and no tracked HAP output at this inspection.
9. The risky code path **was committed from the initial commit `6edd5cb`**:
   - `shared/src/main/ets/settings/CookieJarSettings.ets:114-134` reads `getRawFileContentSync('dev_session.txt')`, parses it, and saves it into app Preferences.
   - `shared/src/main/ets/settings/SettingsBootstrap.ets:27-29` calls `CookieJarSettings.injectDevSession(context)` on every startup after restore when no login exists.
...
12. Remediation completed after explicit path-level authorization:
   ...
   - removed app-startup dev-session injection from `CookieJarSettings.ets` and `SettingsBootstrap.ets`.
   ...
   - verified new `entry/build/default/outputs/default/entry-default-unsigned.hap` does **not** package `resources/rawfile/dev_session.txt`.
...
- A signed/unsigned HAP built with `entry/src/main/resources/rawfile/dev_session.txt` contains live EH session cookies.
...
- Do not print cookie values, account password, raw `.env.local`, raw `dev_session.txt`, or raw HAP contents.
...
Remove production/runtime code that reads bundled `dev_session.txt` and auto-injects cookies:

- Delete or hard-disable `CookieJarSettings.injectDevSession`.
...
- tracked tree contains `dev_session.txt`, `.env.local`, `scripts/dev.env`, HAP, signing material, or cookie fixture paths;
- source code calls `getRawFileContentSync('dev_session.txt')` or `injectDevSession` from app startup;
- resources contain `entry/src/main/resources/rawfile/dev_session.txt` at build/test time;
...
- concrete member IDs;
- “real account data” claims tied to auth validation;
...
- [x] `git grep` / source search shows no app-startup `injectDevSession` / `getRawFileContentSync('dev_session.txt')` path.
- [x] Secret gate passes on clean tree after quarantine and build.
- [x] Build output checked: no packaged `rawfile/dev_session.txt` in new debug unsigned HAP.
