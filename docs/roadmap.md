...
- `oh-package.json5`(modelVersion 6.1.1,name `nexte`,version 1.0.0)+ 各模块 `oh-package.json5`(`file:` 依赖);`hvigorfile.ts`/`hvigorconfig.ts`/`hvigor/hvigor-config.json5`/`.gitignore`
- `AppScope/app.json5`(bundleName `com.erosteam.nexte`,vendor `erosTeam`,versionCode 1,label `$string:app_name`,无云同步)
...
| 决策 | 采用默认 | 影响 |
|---|---|---|
| 包名/标识 | `com.erosteam.nexte`,vendor `erosTeam`,label NextE,v1.0.0,targetSdk 26.0.0 | 已写入 `AppScope/app.json5` |
...
| 云同步 | v1 本地化,去掉 `cloudStructuredDataSyncEnabled` + DISTRIBUTED_DATASYNC | 已去除 |
