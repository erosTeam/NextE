# ARCHIVED: /loop —— NextE 对标 eros_fe 的旧开发驱动提示词

> 本文件已归档，不再是项目规范。其中自动挑任务、自动提交、强制多智能体或“不得停止”等指令均已
> 失效。当前规则只从 `AGENTS.md` 和其分流文档读取。
...
参考物三方定位(分工不同,别混用):
- **eros_fe**(`/Users/honjow/git/eros_fe`,Flutter)= **规格 / SPEC**:要做什么、功能是否完整、页面结构与交互协调怎么安排、**比例/结构性的安排为什么这么定**。实现与处理的答案在它的源码里(但具体像素尺度按原生来,见 §2.3)。
- **V2Next**(`/Users/honjow/git/V2Next`,HarmonyOS)= **怎么用原生做 / HOW**:同一个东西在 ArkUI/HDS 里的原生写法、组件、占位、状态。**抄它,别从零自创。**
...
macOS 本机不要用 Linux `dev.sh` 驱动构建/签名。先用 `scripts/setup-local-build-profile.sh`
安装 ignored 的本地签名 profile,再用 `bash scripts/build_hvigor_signed.sh` 产出
`entry/build/default/outputs/default/entry-default-signed.hap`,随后用 DevEco/HDC 官方命令安装和启动。
...
- **Agent 控制 NextE 真机前必须先拿 lease**:读 `docs/device-lease.md`;会改变设备/UI 状态的 `hdc install` / `aa start` / `uitest` / 依赖前台状态的截图必须通过 `scripts/device-lease acquire` + `scripts/device-lease run --lease "$LEASE_ID" -- <command>` 执行;人工调试不受限制;无用户批准不得 `--force` 抢占。
